...
function add_key_pair_line {
  local FILE=$1
  local RSN=$2
  local PUBLIC=$3
  local PRIVATE=$4

  echo "principal_key_pairs {" >> $FILE
  echo "  principal: \"${RSN}\"" >> $FILE
  echo "  key_pairs {" >> $FILE
  echo "    public_key_file: \"${PUBLIC}\"" >> $FILE
  echo "    private_key_file: \"${PRIVATE}\"" >> $FILE
  echo "  }" >> $FILE
  echo "}" >> $FILE
}

function build_key_pair {
  rm -f /tmp/encryption_key_pair_config.textproto
  touch /tmp/encryption_key_pair_config.textproto
  cat >$REPORTING_KEY_PAIR <<HERE
...
HERE
  ...
  add_key_pair_line $REPORTING_KEY_PAIR "${HALO_MC}" "mc_enc_public.tink" "mc_enc_private.tink"
}

function add_mc_config_line {
  local FILE=$1
  local RSN=$2
  local API_KEY=$3
  local SIGNING_CERT=$4
  local SIGNING_PRIVATE_KEY=$5

  echo "configs {" >> $FILE
  echo "  key: \"${RSN}\"" >> $FILE
  echo "  value: {" >> $FILE
  echo "    api_key: \"${API_KEY}\"" >> $FILE
  echo "    signing_certificate_name: \"${SIGNING_CERT}\"" >> $FILE
  echo "    signing_private_key_path: \"${SIGNING_PRIVATE_KEY}\"" >> $FILE
  echo "  }" >> $FILE
  echo "}" >> $FILE
}

function build_mc_config {
  rm -f /tmp/measurement_consumer_config.textproto
  touch /tmp/measurement_consumer_config.textproto
  cat >$REPORTING_MC_CONFIG <<HERE
# proto-file: wfa/measurement/config/reporting/measurement_consumer_config.proto
# proto-message: MeasurementConsumerConfigs
HERE

  add_mc_config_line $REPORTING_MC_CONFIG "${HALO_MC}" "${HALO_MC_APIKEY}" "${HALO_MC_SIGNING_CERT}" "mc_cs_private.der"
}
...
function get_db_auth_secret {
  echo Please type password and Ctrl+D to finish

  kubectl create secret generic db-auth --type='kubernetes.io/basic/auth' \
  --append-hash --from-literal=password=password --from-literal=username=user

  export DB_AUTH_SECRETNAME=`kubectl get secrets | grep 'db-auth' | awk '{ print $1; }'`
}

function get_mc_config_secret {
  echo
  echo CREATE SECRET FOR REPORTING API SERVER
  echo
  kubectl create secret generic mc-config --append-hash --from-file=$REPORTING_MC_CONFIG

  export MC_CONFIG_SECRETNAME=`kubectl get secrets | grep 'mc-config' | awk '{ print $1; }'`
}

function deploy_reporting_server_db {
  echo
  echo DEPLOYING REPORTING SERVER POSTGRES DB
  echo
  tools/bazel-container-run //src/main/k8s/local:reporting_database_kind \
  --define=k8s_secret_name=$HALO_SECRETNAME \
  --define=k8s_db_secret_name=$DB_AUTH_SECRETNAME

  wait_for postgres 1/1Running
  sleep 60
}

function deploy_reporting_server {
  echo
  echo DEPLOY REPORTING SERVER
  echo
  tools/bazel-container-run //src/main/k8s/local:reporting_kind \
  --define=k8s_secret_name=$HALO_SECRETNAME \
  --define=k8s_db_secret_name=$DB_AUTH_SECRETNAME \
  --define=k8s_mc_config_secret_name=$MC_CONFIG_SECRETNAME
}
...
function setup_port_forwarding {
  ...
  postgres_port_forwarding_active=`ps ax | grep "kubectl port-forward postgres" | grep -v grep | wc -l`
  echo postgres_port_forwarding_active=$postgres_port_forwarding_active
  if [ $postgres_port_forwarding_active -eq 1 ]; then
    echo It appears that port forwarding for postgres is already active.
  else
    echo Turning on port forwarding ...
...
get_db_auth_secret
get_mc_config_secret
get_resource_names
deploy_reporting_server_db
deploy_reporting_server
