...
- **Cloud Functions** (SDK-based instrumentation with auto-instrumentation libraries):
  - Custom application metrics (e.g., `edpa.data_watcher.match_latency`, `edpa.event_group.sync_latency`) use OpenTelemetry SDK Meter API
  ...
  - **Note**: Native Cloud Functions metrics (`function.execution.latency`, `function.execution.count`) are available but custom EDPA metrics are used for SLOs to ensure consistency and control over measurements
...
- Cloud Functions: Initialize SDK in global scope (outside handler) to avoid cold start overhead
  - **Critical**: Call `EdpaTelemetry.flush()` in function handler's `finally` block to ensure metrics are exported before function returns
  - After function returns, background threads (including OpenTelemetry's periodic export) **cannot access CPU** and remain frozen even during warm period (5-15 minutes)
  - Without flush, all metrics recorded during function execution will be lost, as the periodic export (60s) cannot run after termination
...
5. **Event Group Sync Timeliness** (99.9%, 28d) - Event Group Sync function execution <20 minutes
...
- Window: Rolling 28 days
- Implementation:
  - Emit `edpa.data_availability.sync_duration` (histogram) in Data Availability Sync with value = `kingdom_update_complete_time - done_blob_creation_time`.
  - Data Availability Sync Function receives `doneBlobPath` via `X-DataWatcher-Path` header (line 82 in DataAvailabilitySyncFunction.kt).
  - Data Availability Sync queries GCS blob metadata to retrieve `timeCreated` for the done blob using the available `StorageClient`.
  - Upon successful Kingdom update, record histogram: `sync_duration = Instant.now() - blob.timeCreated`.
...
- Implementation:
  - **Two-level tracking:**
    - **Requisition-level**: Emit `edpa.results_fulfiller.requisition_fulfillment_latency` (histogram) with value = `fulfillment_complete_time - requisition.create_time`. Requisition metadata contains `create_time` field, available to Results Fulfiller when processing each requisition.
    - **Report-level**: Emit `edpa.results_fulfiller.report_fulfillment_latency` (histogram) with value = `report_complete_time - earliest_requisition_create_time`. Results Fulfiller computes this using the earliest `create_time` across all requisitions in the report (already queried during processing).
  - SLI: requisitions with `requisition_fulfillment_latency < 3600s` divided by total fulfilled.
  - **Debugging**: Use distributed tracing with span attributes (Section 4.4) for per-report analysis.
...
- Window: Rolling 28 days
- Implementation:
  - Compute SLI as ratio of aggregate counters: `edpa.results_fulfiller.requisitions_processed{status="success"}` divided by `edpa.data_watcher.queue_writes` over the rolling 28-day window.
  - Both metrics are simple counters incremented by their respective components.
  - No per-item correlation required - comparing total counts across the pipeline.
  - **Debugging**: When SLI drops, use distributed tracing (Section 4.4) with span attributes to identify specific failed work items.

### 3.5 SLO 5: Event Group Sync Timeliness
- Scope: Event Group Sync workflow (function invocation → Event Groups created in Kingdom).
- SLI: Proportion of Event Group Sync operations that complete successfully within 20 minutes.
- Target: 99.9%
- Window: Rolling 28 days
- Implementation:
  - Emit `edpa.event_group.sync_latency` (histogram) with value = `sync_complete_time - function_start_time`.
  - Event Group Sync Function measures execution time internally (from function entry to Kingdom sync completion).
  - Data Watcher trigger is immediate pass-through, so no separate tracking needed.
...
- **Resource attributes:** Tag every metric with `service.name`, `deployment.environment` (`dev`, `qa`, `prod`), `data_provider_name`, and `runtime` (`cloud_function`, `confidential_space`, `standalone`).
  - Configured in `Resource` object during SDK initialization (see Section 5.4)
...
- **Span attributes**: Added to spans for filtering in Cloud Trace console (see Section 4.4)
- **Logs**: Structured logging with business identifiers for correlation

**Not used for metric labels** due to high cardinality (unbounded values cause metric explosion).

### 4.4 Distributed Tracing Strategy

#### Approach: Span Attributes for Business Identifiers

Add business identifiers as span attributes to enable querying traces by known business entities. This follows OpenTelemetry best practices and allows filtering in Cloud Trace console.

#### Span Attribute Mapping

| Workflow | Span Attribute | Example Value |
|----------|----------------|---------------|
| Requisition Fulfillment | `report_id` | `measurementConsumers/mc123/reports/report456` |
| Event Group Sync | `event_group_reference_id` | `event-group-ref-789` |
| Data Availability Sync | `done_blob_path` | `gs://bucket/edp1/2025-01-15.done` |

##### Rationale

- Standard OpenTelemetry practice - business identifiers as span attributes
- Native Cloud Trace filtering - query by `report_id = "..."` in console
- Automatic trace context propagation - downstream calls (gRPC, HTTP) automatically inherit trace context

#### Implementation

Components add business identifiers as span attributes when creating spans:
...
Downstream calls automatically propagate trace context via gRPC interceptors (`GrpcTelemetry`) and HTTP client instrumentation.
...
1. Query Cloud Trace console with filter: `report_id = "measurementConsumers/mc123/reports/report456"`
2. View all spans across Requisition Fetcher, Results Fulfiller, Kingdom calls
3. Identify bottleneck span (e.g., `compute_frequency_vectors` took 85 minutes)
4. Cross-reference aggregated metrics: `edpa.results_fulfiller.frequency_vector_duration` percentiles
...
Shared OpenTelemetry infrastructure for EDPA components (Cloud Functions, Confidential Space MIG). Secure Computation API (Kubernetes) uses existing auto-instrumentation and is excluded.

#### Purpose

- SDK initialization (exporters, resource attributes, JVM metrics)
- Generic tracing utilities (span creation, context propagation)

#### Key classes

- `EdpaTelemetry` - SDK initialization, provides tracer/meter access
- `TracedOperation` - Generic span creation with attributes

#### Component responsibility

- Create component-specific metrics (e.g., `ResultsFulfillerMetrics.kt`)
- Add business identifiers as span attributes for tracing
- Instrument gRPC clients with `GrpcTelemetry` interceptors
...
object DataWatcherMetrics {
  private val meter = EdpaTelemetry.getMeter()
  val queueWrites = meter.counterBuilder("edpa.data_watcher.queue_writes").build()
}

// Tracing with business identifiers
TracedOperation.trace("report_fulfillment", mapOf("report_id" to reportId)) {
  // Processing logic
}
```
...
**Note:** Only Data Watcher needs HTTP client instrumentation because it calls Event Group Sync Cloud Function via HTTP POST (see `DataWatcher.kt` line 103). All other components use only gRPC for network communication.
...
- **Flush before exit (Cloud Functions)**: Always call `EdpaTelemetry.flush()` in function handler's `finally` block; background threads freeze after function returns (no CPU access even during warm period), so periodic export cannot run and metrics will be lost without explicit flush
  - **Latency tradeoff**: Flush adds 100-200ms typical latency (blocking network call to export metrics); this is included in function execution time and is billed
  - **Parallel flush**: Metrics and traces are flushed in parallel (not sequentially), so total time = max(metric_flush, trace_flush) rather than sum
  - **Optimization**: Reduce flush timeout for latency-sensitive functions (default 5s) or conditionally flush (e.g., sample 10% of invocations)
...
| Results Fulfiller | histogram | edpa.results_fulfiller.requisition_fulfillment_latency |
| Results Fulfiller | histogram | edpa.results_fulfiller.report_fulfillment_latency |
...
**Total**: 16 implemented (native platform metrics), 28 planned (custom EDPA metrics)
...
#### Metrics
- **End-to-end latency (two-level tracking):**
  - `edpa.results_fulfiller.requisition_fulfillment_latency` (histogram) - Requisition-level: Time from `requisition.create_time` (from requisition metadata) to fulfillment complete. Recorded per requisition.
  - `edpa.results_fulfiller.report_fulfillment_latency` (histogram) - Report-level: Time from earliest `requisition.create_time` across all requisitions in the report to report completion. Results Fulfiller computes this using the minimum create_time from all requisitions already queried during processing.
...
- **Dependencies**: Add OpenTelemetry SDK and auto-instrumentation library dependencies to the build
- **Terraform**: `src/main/terraform/gcloud/modules/edp-aggregator/` - Add `monitoring_iam.tf`, `dashboards.tf`, `alerts.tf`, `slos.tf`
- **SDK Initialization**: Create shared helper class (e.g., `OpenTelemetrySetup.kt`) for SDK initialization across all components
- **Cloud Functions**: Instrument `*Function.kt` files with SDK initialization (global scope) and custom metrics; add `GrpcTelemetry` interceptors to gRPC channels
- **Results Fulfiller**: Instrument [ResultsFulfiller.kt](https://github.com/world-federation-of-advertisers/cross-media-measurement/blob/main/src/main/kotlin/org/wfanet/measurement/edpaggregator/resultsfulfiller/ResultsFulfiller.kt) with SDK initialization and custom metrics; add `GrpcTelemetry` interceptors to Kingdom gRPC clients
- **gRPC Clients**: Add `GrpcTelemetry.create(openTelemetry).newClientInterceptor()` to all gRPC channel builders for automatic `rpc.client.duration` metrics
