...
def benchmark_setup():
    ...
    GET_HALO_MC_COMMAND = "kubectl logs jobs/resource-setup-job | grep 'Successfully created measurement consumer' | awk '{print $6;}' | head -1"
    ...
    GET_HALO_MC_APIKEY_COMMAND = "kubectl logs jobs/resource-setup-job | grep 'API key for measurement consumer' | awk '{print $8;}'"
    ...
    for EDP in EDPS:
        ...
        EG_command = f"kubectl logs {E}" + " | grep 'Successfully created eventGroup' | awk '{print $5}' | tr -d ."
        EG = subprocess.run(
                ["bash", "-c", EG_command],
                capture_output=True, 
                encoding="utf-8"
...
def wait_epds(edps):
    ...
    pod_processes = [
        subprocess.run(
            ["bash", "-c", "kubectl get pods | grep edp1-simulator"], 
            capture_output=True, 
            encoding="utf-8"
        )
        for edp in edps
    ...
    pod_names = [grep_output.split()[0] for grep_output in grep_outputs]
    check_commands = [
...
def get_all_mills():
    process = subprocess.run(
        ["bash", "-c", "kubectl get pods | grep 'mill' | awk '{print $1}'"], 
        capture_output=True, 
        encoding="utf-8"
    )
    return process.stdout.strip("\n").split("\n")

def store_pod_log(pod_name, number_publisher, demography_type, metric_type):
    log_path = f"{OUTPUT_DIR}/A{metric_type}-{number_publisher}{demography_type}-{pod_name}-log.txt"
    
    subprocess.run(
        ["bash", "-c", f"kubectl logs {pod_name} > {log_path}"] 
    )
...
for number_publisher in range(1, NUM_PUBLISHERS + 1):
    ...
    # Store logs from all mills
    for pod in get_all_mills():
        store_pod_log(pod, number_publisher, demography_type, metric_type)
