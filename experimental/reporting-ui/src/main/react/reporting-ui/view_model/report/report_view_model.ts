...
const filter = (data: ChartGroup[], filters: 'all'|string[] = 'all'): ChartGroup[] => {
  ...
  let filteredData = filters === 'all'
    ? data
    : data.filter(x => x.filters && x.filters.filter(y => filters.includes(y)).length > 0);
  ...
  for (let pub of pubs) {
    for (let date of dates) {
      ...
      consilidatedData.group = pub;
      filteredResults.push(consilidatedData);
    }
  }
