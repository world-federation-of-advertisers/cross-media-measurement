...
export class FakeApi implements ReportApi {
  reports: Report[];
  ...
  async _getReport(req: GetReportRequest, reports: Report[]): Promise<GetReportResponse> {
    return new Promise(r => {
      const rep = reports.find(x => x.id === req.id);
      r({
        report: rep,
      })
    });
