...
-- comment: Add ImpressionMetadataEntityKeys interleaved table to support filtering ImpressionMetadata by entity keys.
...
--
-- `EntityType` is interpreted per-EDP via configuration; no schema change is
-- needed to onboard a new EDP or to extend an EDP's taxonomy.
CREATE TABLE ImpressionMetadataEntityKeys (
  ...
  -- interpreted per-EDP via configuration.
  EntityType STRING(255) NOT NULL,
...
-- Non-unique index for finding ImpressionMetadata by entity key, used for List
-- filtering: given a (EntityType, EntityId) pair, find all parent
-- ImpressionMetadata blobs that include it. Non-unique by design — the same
-- (EntityType, EntityId) pair appears across multiple ImpressionMetadata
-- blobs (e.g. one per day).
CREATE INDEX ImpressionMetadataEntityKeysByTypeAndId
