...
```shell
bazel run //src/main/k8s/local:emulators_kind --define=k8s_secret_name=certs-and-configs-k8888kc6gg
```
...
```shell
bazel run //src/main/k8s/local:kingdom_kind --define=k8s_secret_name=certs-and-configs-k8888kc6gg
bazel run //src/main/k8s/local:resource_setup_kind --define=k8s_secret_name=certs-and-configs-k8888kc6gg
```
...
content below, substituting the appropriate resource names.
...
```shell
kubectl create configmap config-files --from-file=authority_key_identifier_to_principal_map.textproto --output=yaml --dry-run=client | kubectl replace -f -
```
...
```shell
bazel run //src/main/k8s/local:mc_frontend_simulator_kind --define=k8s_secret_name=certs-and-configs-bmt62gbkh5 --define=mc_name=measurementConsumers/FS1n8aTrck0
```
