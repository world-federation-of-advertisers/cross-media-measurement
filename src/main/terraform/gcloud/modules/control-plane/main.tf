...
resource "google_spanner_database_iam_member" "internal_server" {
  instance = google_spanner_database.control_plane_db.instance
  database = google_spanner_database.control_plane_db.name
  role     = "roles/spanner.databaseUser"
  member   = module.control_plane_internal.iam_service_account.member

  lifecycle {
    replace_triggered_by = [google_spanner_database.control_plane_db.id]
  }
}
