...
class PrincipalAuthInterceptor(
  private val openIdConnectAuth: OpenIdConnectAuthentication,
  ...
  override suspend fun <ReqT : Any, RespT : Any> interceptCallSuspending(
    ...
    return Contexts.interceptCall(
      Context.current()
        .withValue(PRINCIPAL_CONTEXT_KEY, principal)
        .withValue(SCOPES_CONTEXT_KEY, scopes),
      call,
      headers,
      next,
    )
  ...
  companion object {
    val PRINCIPAL_CONTEXT_KEY: Context.Key<Principal> = Context.key("access-principal")
    val SCOPES_CONTEXT_KEY: Context.Key<Set<String>> = Context.key("auth-scopes")
  }
}
