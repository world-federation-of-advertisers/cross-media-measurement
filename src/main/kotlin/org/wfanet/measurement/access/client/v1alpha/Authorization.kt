...
class Authorization(private val permissionsStub: PermissionsGrpcKt.PermissionsCoroutineStub) {
  ...
  suspend fun check(
    ...
    val principal: Principal =
      ContextKeys.PRINCIPAL.get()
        ?: throw Status.UNAUTHENTICATED.withDescription("Principal not found").asRuntimeException()
    ...
    checkPermissionsParallel(protectedResourceNames, principal, requiredPermissionIds).getOrThrow()
  }
  ...
  private fun buildPermissionDeniedException(
    ...
    return Status.PERMISSION_DENIED.withDescription(
        "Permission $missingPermissionId denied on resource $protectedResourceName (or it might not exist)"
      )
      .asRuntimeException()
  }
