...
/** Helpers for encoding Kotlin values as CEL value literals. */
object CelValueEncoding {
  /**
   * Encodes [value] as a CEL double-quoted string literal, escaping backslash, double-quote, and
   ...
   * literals -- rather than being re-escaped to `\uXXXX`. This keeps generated CEL readable in
   * server logs.
   */
  fun toCelStringLiteral(value: String): String {
  ...
  /**
   * Encodes [value] as a CEL numeric literal. Rejects `NaN` and infinities: CEL renders these as
   * bare identifiers (`NaN`, `Infinity`, `-Infinity`) rather than numeric literals, so Kotlin's
   * `Float.toString()` output would produce a filter that fails to compile. Semantically, `NaN ==
   * NaN` is `false` in IEEE 754 -- a comparison against `NaN` is meaningless -- so rejecting
   ...
   */
  fun toCelNumericLiteral(value: Float): String {
    require(value.isFinite()) {
      "Cannot encode non-finite float $value as a CEL numeric literal; NaN and infinities are " +
