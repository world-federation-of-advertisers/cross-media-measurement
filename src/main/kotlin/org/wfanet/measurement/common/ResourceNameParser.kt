...
class ResourceNameParser(pattern: String) {
  ...
  init {
    val parts: List<String> = pattern.split('/')
    val segments: MutableList<Segment> = ArrayList(parts.size)
    for (part in parts) {
  ...
  fun parseIdSegments(relativeResourceName: String): Map<String, String>? {
    val parts: List<String> = relativeResourceName.split('/')
    if (parts.size != segments.size) {
      return null
    }

    val idSegments: MutableMap<String, String> = mutableMapOf()
    for ((part: String, segment: Segment) in parts.zip(segments)) {
      when (segment) {
        is LiteralSegment -> {
  ...
  fun assembleName(idSegments: Map<String, String>): String {
    val parts: MutableList<String> = ArrayList(segments.size)
    for (segment: Segment in segments) {
      val part: String =
        when (segment) {
          is LiteralSegment -> segment.value
          is VariableSegment ->
            requireNotNull(idSegments[segment.name]) { "Missing value for ${segment.name}" }
        }
      parts.add(part)
    }
