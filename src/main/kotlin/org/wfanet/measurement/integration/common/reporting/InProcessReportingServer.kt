...
class InProcessReportingServer(
  ...
  private val publicKingdomCertificatesMock: CertificatesCoroutineImplBase = mockService {
  ...
  private val publicKingdomMeasurementConsumersMock: MeasurementConsumersCoroutineImplBase =
    mockService {
      onBlocking { getMeasurementConsumer(any()) }.thenReturn(MEASUREMENT_CONSUMER)
    }
  private val publicKingdomMeasurementsMock: MeasurementsCoroutineImplBase = mockService {
  ...
  private val publicKingdomMeasurementConsumersClient by lazy {
    PublicKingdomMeasurementConsumersCoroutineStub(publicKingdomServer.channel)
  }
  private val publicKingdomMeasurementsClient by lazy {
    PublicKingdomMeasurementsCoroutineStub(publicKingdomServer.channel)
  }
  private val publicKingdomCertificatesClient by lazy {
    PublicKingdomCertificatesCoroutineStub(publicKingdomServer.channel)
  }
  private val publicKingdomDataProvidersClient by lazy {
    PublicKingdomDataProvidersCoroutineStub(publicKingdomServer.channel)
  }
  private val publicKingdomEventGroupsClient by lazy {
    PublicKingdomEventGroupsCoroutineStub(publicKingdomServer.channel)
  }
  private val publicKingdomEventGroupMetadataDescriptorsClient by lazy {
    PublicKingdomEventGroupMetadataDescriptorsCoroutineStub(publicKingdomServer.channel)
  }
    ...
    GrpcTestServerRule(logAllRequests = verboseGrpcLogging) {
      ...
      val encryptionKeyPairStore =
        InMemoryEncryptionKeyPairStore(
          mapOf(MEASUREMENT_CONSUMER.publicKey.data to MC_PRIVATE_KEY_HANDLE)
        )
      ...
      listOf(
          EventGroupsService(
            ...
            .withMetadataPrincipalIdentities(config),
          ReportingSetsService(internalReportingSetsClient).withMetadataPrincipalIdentities(config),
          ReportsService(
  ...
  override fun apply(statement: Statement, description: Description): Statement {
    return chainRulesSequentially(internalDataServer, publicKingdomServer, publicApiServer)
  ...
  companion object {
    ...
    private val MC_PRIVATE_KEY_HANDLE: TinkPrivateKeyHandle =
