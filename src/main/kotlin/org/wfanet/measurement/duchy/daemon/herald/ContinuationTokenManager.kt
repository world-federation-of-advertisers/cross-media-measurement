...
class ContinuationTokenManager(
  ...
  suspend fun markTokenProcessed(continuationToken: String) {
    var lastProcessedTokenInSequence = ""

    synchronized(this) {
      continuationTokens[continuationToken] = State.PROCESSED

      val removableTokens = mutableListOf<String>()
      for (item in continuationTokens) {
        if (item.value == State.PENDING) {
          break
        }
        lastProcessedTokenInSequence = item.key
      }
      removableTokens.add(lastProcessedTokenInSequence)
      latestContinuationToken = lastProcessedTokenInSequence

      // Remove processed tokens before lastProcessedTokenInSequence
      for (token in removableTokens) {
        continuationTokens.remove(token)
      }
    }

    if (lastProcessedTokenInSequence != "") {
