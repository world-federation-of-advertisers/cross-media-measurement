...
class HonestMajorityShareShuffleMill(
  ...
  /**
   * Verify the secret seed against the data provider certificate and the duchy encryption key.
   *
   * @throws [IllegalArgumentException] if fail to retrieve data provider certificate or the duchy
   *   encryption key.
   * @throws [CertPathValidatorException] if data provider certificate is invalid
   * @throws [SignatureException] if the signature is invalid
   * @throws [InvalidProtocolBufferException] if fail to parse or decrypt protobuf message
   * @throws [CertificateException] if fail to read certificate
   */
  private suspend fun verifySecretSeed(
    ...
    val dataProviderCertificate =
      try {
        ...
        throw IllegalArgumentException(
          "Fail to get certificate for $dataProviderCertificateName",
          e,
        )
      }
    ...
    val request = completeShufflePhaseRequest {
      ...
      for (requisition in requisitions) {
        ...
        if (blob != null) {
          ...
          val seed =
            try {
              verifySecretSeed(secretSeed, hmss.encryptionKeyPair.privateKeyId)
            } catch (e: Exception) {
              throw PermanentErrorException("Fail to verify secret seed for $requisitionId", e)
            }
