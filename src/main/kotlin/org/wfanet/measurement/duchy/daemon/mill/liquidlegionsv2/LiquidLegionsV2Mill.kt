...
class LiquidLegionsV2Mill(
  ...
  /**
   * Verifies the Edp signature and the local existence of requisitions. Returns an error message if
   * anything is wrong. Otherwise return an empty string.
   */
  private fun verifyEdpSignatureAndRequisition(requisition: RequisitionMetadata): String {
    ...
    if (requisition.details.externalFulfillingDuchyId == duchyId && requisition.path.isBlank()) {
      errorList.append(
        "Missing expected data for requisition ${requisition.externalRequisitionId}.\n"
      )
    }
  ...
  /**
   * Verifies the duchy Elgamal Key signature. Returns an error message if anything is wrong.
   * Otherwsie return an empty string.
   */
  private fun verifyDuchySignature(duchy: ComputationParticipant): String {
    val errorList = StringBuilder()
    // TODO(wangyaopw): verify el_gamal_public_key_signature and update errorMessage if necessary.
    if (!workerStubs.containsKey(duchy.duchyId)) {
      errorList.append("Unrecognized duchy ${duchy.duchyId}.\n")
    }
    return errorList.toString()
  }
  ...
  private suspend fun confirmationPhase(token: ComputationToken): ComputationToken {
    val errorList = StringBuilder()
    token.requisitionsList.forEach { errorList.append(verifyEdpSignatureAndRequisition(it)) }
    token.computationDetails.liquidLegionsV2.participantList.forEach {
      errorList.append(verifyDuchySignature(it))
    }
    return if (errorList.isEmpty()) {
      passConfirmationPhase(token)
    } else {
      failComputationAtConfirmationPhase(token, errorList.toString())
    }
  }
