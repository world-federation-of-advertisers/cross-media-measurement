...
class Herald(
  ...
  private val rpcDispatcher = Dispatchers.IO
  ...
  suspend fun syncStatuses(continuationToken: String): String {
    ...
    systemComputationsClient
      ...
      // conditions the computation should just be failed so the Herald doesn't get stuck on one
      // that cannot succeed.
   ...
   *
   * [block] is dispatched with [rpcDispatcher].
   *
  ...
  private suspend fun <R> runWithRetries(
    ...
    val finalResult =
      (1..maxAttempts).fold(Result.failure<R>(IllegalStateException())) { previous, attemptNumber ->
        ...
        val result = runCatching { withContext(rpcDispatcher) { block(systemComputation) } }
        if (result.isFailure) {
          delay(timeMillis = minOf((1L shl attemptNumber) * 1000L, 60_000L))
        }
        result
      }
    if (finalResult.isFailure) {
      logger.severe("[id=$globalId] Giving up after $maxAttempts attempts")
    }
