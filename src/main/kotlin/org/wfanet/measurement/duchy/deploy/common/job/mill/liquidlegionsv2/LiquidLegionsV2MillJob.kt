...
import org.wfanet.measurement.duchy.mill.GracePeriod
...
abstract class LiquidLegionsV2MillJob : Runnable {
  ...
  protected fun run(storageClient: StorageClient) {
    ...
    val gracePeriod = GracePeriod(flags.gracePeriod)
    val gracePeriodBackoff = ExponentialBackoff()
    runBlocking(CoroutineName("Mill $millId")) {
      while (isActive && !gracePeriod.ended) {
        val workExhausted =
        ...
        if (!workExhausted) {
          gracePeriod.reset()
        } else if (!gracePeriod.started) {
          gracePeriod.start()
        }

        if (gracePeriod.started) {
          val delayDuration =
            gracePeriodBackoff.durationForAttempt(gracePeriod.getAndIncrementAttempt())
          logger.fine {
            "@Mill $millId: No work available. Next attempt in ${delayDuration.toHumanFriendlyDuration()}"
          }
          delay(delayDuration)
        }
      }
