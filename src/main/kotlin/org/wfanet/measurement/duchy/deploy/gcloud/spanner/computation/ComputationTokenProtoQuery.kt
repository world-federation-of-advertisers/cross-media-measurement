...
import org.wfanet.measurement.internal.duchy.RequisitionBlobMetadata
  ...
  companion object {
    private val parameterizedQueryUsingGlobalComputationIdString =
      ...
      SELECT c.ComputationId,
             ...
             ARRAY(
               SELECT AS STRUCT r2.ExternalRequisitionId, IFNULL(r2.PathToBlob, "") AS PathToBlob
               FROM Requisitions AS r2
               WHERE c.ComputationId = r2.ComputationId
                 AND LENGTH(PathToBlob) > 0
             ) AS Requisitions
  ...
  override fun asResult(struct: Struct): ComputationToken {
    ...
    val requisitions =
      struct
        ...
        .map {
          RequisitionBlobMetadata.newBuilder()
            .apply {
    ...
    return ComputationToken.newBuilder()
      .apply {
        ...
        if (requisitions.isNotEmpty()) {
          addAllLocalRequisitionBlobs(requisitions)
        }
      }
      .build()
  }
