...
fun InternalComputationStage.toSystemComputationStage(): ComputationStage {
  ...
  return when (stageCase) {
    InternalComputationStage.StageCase.LIQUID_LEGIONS_SKETCH_AGGREGATION_V2 -> {
      liquidLegionsSketchAggregationV2.toSystemComputationStage()
    }
    InternalComputationStage.StageCase.REACH_ONLY_LIQUID_LEGIONS_SKETCH_AGGREGATION_V2 -> {
      reachOnlyLiquidLegionsSketchAggregationV2.toSystemComputationStage()
    }
    InternalComputationStage.StageCase.HONEST_MAJORITY_SHARE_SHUFFLE -> {
      honestMajorityShareShuffle.toSystemComputationStage()
    }
    InternalComputationStage.StageCase.STAGE_NOT_SET -> error("Invalid internal ComputationStage.")
  }
}

private fun InternalLlv2.Stage.toSystemComputationStage(): ComputationStage {
  return when (this) {
    ...
    InternalLlv2.Stage.INITIALIZATION_PHASE ->
      LiquidLegionsV2Stage.Stage.INITIALIZATION_PHASE.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_REQUISITIONS_AND_KEY_SET ->
      LiquidLegionsV2Stage.Stage.WAIT_REQUISITIONS_AND_KEY_SET.toProtocolComputationStage()
    InternalLlv2.Stage.CONFIRMATION_PHASE ->
      LiquidLegionsV2Stage.Stage.CONFIRMATION_PHASE.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_TO_START ->
      LiquidLegionsV2Stage.Stage.WAIT_TO_START.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_SETUP_PHASE_INPUTS ->
      LiquidLegionsV2Stage.Stage.WAIT_SETUP_PHASE_INPUTS.toProtocolComputationStage()
    InternalLlv2.Stage.SETUP_PHASE ->
      LiquidLegionsV2Stage.Stage.SETUP_PHASE.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_EXECUTION_PHASE_ONE_INPUTS ->
      LiquidLegionsV2Stage.Stage.WAIT_EXECUTION_PHASE_ONE_INPUTS.toProtocolComputationStage()
    InternalLlv2.Stage.EXECUTION_PHASE_ONE ->
      LiquidLegionsV2Stage.Stage.EXECUTION_PHASE_ONE.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_EXECUTION_PHASE_TWO_INPUTS ->
      LiquidLegionsV2Stage.Stage.WAIT_EXECUTION_PHASE_TWO_INPUTS.toProtocolComputationStage()
    InternalLlv2.Stage.EXECUTION_PHASE_TWO ->
      LiquidLegionsV2Stage.Stage.EXECUTION_PHASE_TWO.toProtocolComputationStage()
    InternalLlv2.Stage.WAIT_EXECUTION_PHASE_THREE_INPUTS ->
      LiquidLegionsV2Stage.Stage.WAIT_EXECUTION_PHASE_THREE_INPUTS.toProtocolComputationStage()
    InternalLlv2.Stage.EXECUTION_PHASE_THREE ->
      LiquidLegionsV2Stage.Stage.EXECUTION_PHASE_THREE.toProtocolComputationStage()
    InternalLlv2.Stage.COMPLETE -> LiquidLegionsV2Stage.Stage.COMPLETE.toProtocolComputationStage()
  }
}

private fun LiquidLegionsV2Stage.Stage.toProtocolComputationStage() = computationStage {
  liquidLegionsV2Stage = this@toProtocolComputationStage
}

private fun InternalRoLlv2.Stage.toSystemComputationStage(): ComputationStage {
  return when (this) {
    ...
    InternalRoLlv2.Stage.INITIALIZATION_PHASE ->
      ReachOnlyLiquidLegionsV2Stage.Stage.INITIALIZATION_PHASE.toProtocolComputationStage()
    ...
    InternalRoLlv2.Stage.CONFIRMATION_PHASE ->
      ReachOnlyLiquidLegionsV2Stage.Stage.CONFIRMATION_PHASE.toProtocolComputationStage()
    InternalRoLlv2.Stage.WAIT_TO_START ->
      ReachOnlyLiquidLegionsV2Stage.Stage.WAIT_TO_START.toProtocolComputationStage()
    InternalRoLlv2.Stage.WAIT_SETUP_PHASE_INPUTS ->
      ReachOnlyLiquidLegionsV2Stage.Stage.WAIT_SETUP_PHASE_INPUTS.toProtocolComputationStage()
    InternalRoLlv2.Stage.SETUP_PHASE ->
      ReachOnlyLiquidLegionsV2Stage.Stage.SETUP_PHASE.toProtocolComputationStage()
    InternalRoLlv2.Stage.WAIT_EXECUTION_PHASE_INPUTS ->
      ReachOnlyLiquidLegionsV2Stage.Stage.SETUP_PHASE.toProtocolComputationStage()
    InternalRoLlv2.Stage.EXECUTION_PHASE ->
      ReachOnlyLiquidLegionsV2Stage.Stage.SETUP_PHASE.toProtocolComputationStage()
    InternalRoLlv2.Stage.COMPLETE ->
      ReachOnlyLiquidLegionsV2Stage.Stage.COMPLETE.toProtocolComputationStage()
  }
}

private fun ReachOnlyLiquidLegionsV2Stage.Stage.toProtocolComputationStage() = computationStage {
  reachOnlyLiquidLegionsV2Stage = this@toProtocolComputationStage
}

private fun InternalHmss.Stage.toSystemComputationStage(): ComputationStage {
  return when (this) {
    ...
    InternalHmss.Stage.INITIALIZED ->
      HonestMajorityShareShuffleStage.Stage.INITIALIZED.toProtocolComputationStage()
    InternalHmss.Stage.WAIT_TO_START ->
      HonestMajorityShareShuffleStage.Stage.WAIT_TO_START.toProtocolComputationStage()
    InternalHmss.Stage.WAIT_ON_SHUFFLE_INPUT_PHASE_ONE ->
      HonestMajorityShareShuffleStage.Stage.WAIT_ON_SHUFFLE_INPUT_PHASE_ONE
        .toProtocolComputationStage()
    InternalHmss.Stage.SETUP_PHASE ->
      HonestMajorityShareShuffleStage.Stage.SETUP_PHASE.toProtocolComputationStage()
    ...
    InternalHmss.Stage.SHUFFLE_PHASE ->
      HonestMajorityShareShuffleStage.Stage.SHUFFLE_PHASE.toProtocolComputationStage()
    InternalHmss.Stage.AGGREGATION_PHASE ->
      HonestMajorityShareShuffleStage.Stage.AGGREGATION_PHASE.toProtocolComputationStage()
    InternalHmss.Stage.COMPLETE ->
      HonestMajorityShareShuffleStage.Stage.COMPLETE.toProtocolComputationStage()
  }
}

private fun HonestMajorityShareShuffleStage.Stage.toProtocolComputationStage() = computationStage {
