...
class AsyncComputationControlService(
  ...
  private suspend fun advanceComputationInternal(
    ...
    if (stages.expectStageInput(computationStage, token)) {
      val computationDetails = stages.updateComputationDetails(token.computationDetails, request.computationStageInput)
      computationsClient.updateComputationDetails(updateComputationDetailsRequest {
        this.token = token
