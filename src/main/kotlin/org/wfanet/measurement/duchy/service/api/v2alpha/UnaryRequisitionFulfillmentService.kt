...
import org.wfanet.measurement.api.v2alpha.UnaryFulfillRequisitionRequest
import org.wfanet.measurement.api.v2alpha.UnaryFulfillRequisitionResponse
import org.wfanet.measurement.api.v2alpha.UnaryRequisitionFulfillmentGrpcKt.UnaryRequisitionFulfillmentCoroutineImplBase
...
private val FULFILLED_RESPONSE =
  unaryFulfillRequisitionResponse { state = Requisition.State.FULFILLED }

/** Implementation of `wfa.measurement.api.v2alpha.UnaryRequisitionFulfillment` gRPC service. */
class UnaryRequisitionFulfillmentService(
...
) : UnaryRequisitionFulfillmentCoroutineImplBase() {

  override suspend fun fulfillRequisition(
    request: UnaryFulfillRequisitionRequest
  ): UnaryFulfillRequisitionResponse {
    val key =
    ...
    val requisitionMetadata =
      verifyRequisitionFulfillment(computationToken, externalRequisitionKey, request.nonce)
