...
/**
 * Blob storage for requisitions.
 *
 * @param storageClient the blob storage client.
 * @param generateBlobKey a function to generate unique blob keys.
 */
// TODO: Merge this class with the ComputationStore since most codes of them are the same.
...
private constructor(
  ...
  /**
   * Writes a requisition as a new blob with the specified content.
   *
  ...
  /**
   * Returns a [Blob] for the requisition with the specified blob key, or `null` if the requisition
   * isn't found.
   */
  ...
  companion object {
    fun forTesting(storageClient: StorageClient, generateBlobKey: BlobKeyGenerator) =
      RequisitionStore(storageClient, generateBlobKey)
  }
}
