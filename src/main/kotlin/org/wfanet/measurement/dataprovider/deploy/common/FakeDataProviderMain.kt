...
class EmptySketchGenerator : EncryptedSketchGenerator {
  ...
  override fun generate(
    ...
    return sketchEncrypter.encrypt(sketch, encryptionKey)
  }
}
...
private fun run(@CommandLine.Mixin flags: Flags) {
  val elGamalPublicKeyStore = GrpcElGamalPublicKeyCache(flags.combinedPublicKeysStub)
  val requisitionFulfiller = GrpcRequisitionFulfiller(flags.requisitionFulfillmentStub)
  val unfulfilledRequisitionProvider =
    GrpcUnfulfilledRequisitionProvider(
      ApiId(flags.externalDataProviderId).externalId,
      flags.requisitionsStub
    )
  val sketchConfigStore = GrpcSketchConfigCache(flags.sketchConfigsStub)

  val throttler = MinimumIntervalThrottler(Clock.systemUTC(), flags.throttlerMinimumInterval)

  val fakeDataProvider =
    FakeDataProvider(
      EmptySketchGenerator(),
