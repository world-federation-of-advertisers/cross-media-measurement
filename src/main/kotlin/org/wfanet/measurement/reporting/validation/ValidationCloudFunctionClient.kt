 ...
 * function endpoint, and deserializes the [DataProviderImpressionQueryResponse]. Handles timeouts
...
class ValidationCloudFunctionClient(
  ...
  fun call(
    ...
    for (attempt in 0..maxRetries) {
      ...
      try {
        ...
        if (lastStatus in 200..299) {
          val response = DataProviderImpressionQueryResponse.parseFrom(httpResponse.body())
          return CallResult(response = response, error = null, httpStatus = lastStatus)
        }

        if (lastStatus in 500..599) {
          lastException = Exception("Cloud function returned HTTP $lastStatus")
    ...
    return CallResult(
      response = null,
      error =
        "Cloud function call failed after ${maxRetries + 1} attempts: ${lastException?.message}",
      httpStatus = lastStatus,
    )
  }
