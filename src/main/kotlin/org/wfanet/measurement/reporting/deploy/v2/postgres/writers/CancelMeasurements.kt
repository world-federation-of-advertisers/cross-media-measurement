...
/**
 * Updates the Measurement State to CANCELLED for Measurements in the database.
 *
 ...
 */
class CancelMeasurements(private val request: BatchCancelMeasurementsRequest) :
  ...
  override suspend fun TransactionScope.runTransaction(): List<Measurement> {
    ...
    val statement =
      boundStatement(
        """
      UPDATE Measurements SET State = $1
      WHERE MeasurementConsumerId = $2 AND CmmsMeasurementId = $3
      """
      ) {
        val state = Measurement.State.CANCELLED
        request.cmmsMeasurementIdsList.forEach {
          addBinding {
            bind("$1", state)
    ...
    if (result.numRowsUpdated < request.cmmsMeasurementIdsList.size) {
    ...
    request.cmmsMeasurementIdsList.forEach { measurements.add(idMap[it]!!) }
