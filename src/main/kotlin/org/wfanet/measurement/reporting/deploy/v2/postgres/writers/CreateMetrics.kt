...
class CreateMetrics(private val requests: List<CreateMetricRequest>) :
  ...
  override suspend fun TransactionScope.runTransaction(): List<Metric> {
     ...
     * DB error, Only extract the external IDs that haven't been queried yet.
     */
    val requestIdsMap: Map<String, String> =
      requests
        .mapNotNull {
        ...
        .associate { it.externalMetricId to it.requestId }

    if (requestIdsMap.isNotEmpty()) {
      val batchGetMetricsRequest: BatchGetMetricsRequest = batchGetMetricsRequest {
        ...
        externalMetricIds += requestIdsMap.keys
      }
      existingMetricsMap +=
        MetricReader(transactionContext)
          ...
          .associate { result ->
            val requestId =
              requestIdsMap[result.metric.externalMetricId] ?: throw MetricNotFoundException()
            /**
             * If the request ID is blank or the request IDs are not matched, throw ALREADY EXISTS
             */
