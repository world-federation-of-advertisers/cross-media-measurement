    ...
    if (basicReportsEnabled) {
      if (
        ...
        throw CommandLine.MissingParameterException(
          spec.commandLine(),
          spec.args(),
          "--spanner-project, --spanner-instance, and --spanner-database are all required if " + "--basic-reports-enabled is true",
        )
      }
      ...
      if (!::impressionQualificationFilterConfigFile.isInitialized) {
        throw CommandLine.MissingParameterException(
          spec.commandLine(),
          spec.args(),
          "--impression-qualification-filter-config-file is required if " + "--basic-reports-enabled is true",
        )
      }
      ...
      if (!::eventMessageDescriptorSetFiles.isInitialized || !::eventMessageTypeUrl.isInitialized) {
        throw CommandLine.MissingParameterException(
          spec.commandLine(),
          spec.args(),
          "--event-message-descriptor-set and --event-message-type-url are required if " + "--basic-reports-enabled is true",
        )
      }
    }
    validated = true
