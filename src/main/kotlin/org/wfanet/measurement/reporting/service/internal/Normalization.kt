...
object Normalization {
  ...
  private val eventTemplateFieldComparator: Comparator<EventTemplateField> =
  ...
  /**
   * Returns a normalized (sorted) copy of [terms].
   *
   * @param terms terms of a disjunction
   */
  @JvmName("normalizeEventTemplateFields")
  fun normalize(terms: Iterable<EventTemplateField>): List<EventTemplateField> {
    return terms.sortedWith(eventTemplateFieldComparator)
  }

  /** Returns a normalized copy of [eventFilters]. */
  @JvmName("normalizeEventFilters")
  fun normalize(eventFilters: Iterable<EventFilter>): List<EventFilter> {
    return eventFilters
      .map {
        it.copy {
          val normalizedTerms = normalize(terms)
          terms.clear()
