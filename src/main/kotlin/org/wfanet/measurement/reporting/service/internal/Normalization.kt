...
object Normalization {
  ...
  fun computeFingerprint(dimension: ReportingSetResult.Dimension): Long {
    val serialization =
      FingerprintSerialization.serialize {
        field(ReportingSetResult.Dimension.EXTERNAL_REPORTING_SET_ID_FIELD_NUMBER) {
          value(dimension.externalReportingSetId)
        }
        field(ReportingSetResult.Dimension.VENN_DIAGRAM_REGION_TYPE_FIELD_NUMBER) {
          value(dimension.vennDiagramRegionTypeValue)
        }
        if (dimension.custom) {
          field(ReportingSetResult.Dimension.CUSTOM_FIELD_NUMBER) { value(dimension.custom) }
        } else {
          field(
            ReportingSetResult.Dimension.EXTERNAL_IMPRESSION_QUALIFICATION_FILTER_ID_FIELD_NUMBER
          ) {
            value(dimension.externalImpressionQualificationFilterId)
          }
        }
        field(ReportingSetResult.Dimension.METRIC_FREQUENCY_SPEC_FIELD_NUMBER) {
          message { value(dimension.metricFrequencySpec) }
        }
        field(ReportingSetResult.Dimension.GROUPING_FIELD_NUMBER) {
          message { value(dimension.grouping) }
        }
        field(ReportingSetResult.Dimension.EVENT_FILTERS_FIELD_NUMBER) {
          list(dimension.eventFiltersList)
        }
      }
    return fingerprint(serialization)
  }
