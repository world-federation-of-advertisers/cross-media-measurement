...
class BasicReportsService(
  ...
  private val internalImpressionQualificationFiltersStub: ImpressionQualificationFiltersCoroutineStub,
      ...
      buildList {
        for (impressionQualificationFilter in request.basicReport.impressionQualificationFiltersList) {
          if (impressionQualificationFilter.hasImpressionQualificationFilter()) {
            val key =
              ImpressionQualificationFilterKey.fromName(impressionQualificationFilter.impressionQualificationFilter)
            try {
              add(internalImpressionQualificationFiltersStub.getImpressionQualificationFilter(
                getImpressionQualificationFilterRequest {
                  externalImpressionQualificationFilterId = key!!.impressionQualificationFilterId
                }).toImpressionQualificationFilter().filterSpecsList)
            } catch (e: StatusException) {
              throw when (InternalErrors.getReason(e)) {
                InternalErrors.Reason.IMPRESSION_QUALIFICATION_FILTER_NOT_FOUND ->
                  InvalidFieldValueException("basic_report.impression_qualification_filters.impression_qualification_filter", e) { "ImpressionQualificationFilter ${impressionQualificationFilter.impressionQualificationFilter} not found" }
                    .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
   ...
   */
  private suspend fun validateCreateBasicReportRequest(request: CreateBasicReportRequest): ReportingSet {
    ...
    val campaignGroupKey = ReportingSetKey.fromName(request.basicReport.campaignGroup)
    ...
    val campaignGroup: ReportingSet =
      try {
        ...
        ).reportingSetsList.first().toReportingSet()
      } catch (e: StatusException) {
        throw when (e.status.code) {
          Status.Code.NOT_FOUND -> throw InvalidFieldValueException("basic_report.campaign_group") { "Not found" }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
    ...
    if (request.basicReport.reportingInterval.reportStart.year == 0 ||
    ...
    if (request.basicReport.reportingInterval.reportEnd.year == 0 ||
    ...
    for (impressionQualificationFilter in request.basicReport.impressionQualificationFiltersList) {
      if (impressionQualificationFilter.hasImpressionQualificationFilter()) {
        ImpressionQualificationFilterKey.fromName(impressionQualificationFilter.impressionQualificationFilter)
          ?: throw InvalidFieldValueException("basic_report.impression_qualification_filters.impression_qualification_filter")
            .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
      } else if (impressionQualificationFilter.hasCustom()) {
        if (impressionQualificationFilter.custom.filterSpecList.isEmpty()) {
          throw RequiredFieldNotSetException("basic_report.impression_qualification_filters.custom.filter_spec")
            .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }
        ...
        buildSet<MediaType> {
          for (filterSpec in impressionQualificationFilter.custom.filterSpecList) {
            if (this.contains(filterSpec.mediaType)) {
              throw InvalidFieldValueException("basic_report.impression_qualification_filters.custom.filter_spec") {
            ...
            if (filterSpec.filtersList.isEmpty()) {
              throw RequiredFieldNotSetException("basic_report.impression_qualification_filters.custom.filter_spec.filters")
                .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
            }

            for (eventFilter in filterSpec.filtersList) {
              if (eventFilter.termsList.size != 1) {
                throw RequiredFieldNotSetException("basic_report.impression_qualification_filters.custom.filter_spec.filters.terms")
                  .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
              }

              for (eventTemplateField in eventFilter.termsList) {
                if (eventTemplateField.path.isEmpty()) {
                  throw RequiredFieldNotSetException("basic_report.impression_qualification_filters.custom.filter_spec.filters.terms.path")
                    .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
                }

                if (eventTemplateField.value.selectorCase == EventTemplateField.FieldValue.SelectorCase.SELECTOR_NOT_SET) {
                  throw RequiredFieldNotSetException("basic_report.impression_qualification_filters.custom.filter_spec.filters.terms.value")
                    .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
                }
              }
            }
    ...
    for (resultGroupSpec in request.basicReport.resultGroupSpecsList) {
      ...
      if (resultGroupSpec.dimensionSpec.hasGrouping() && resultGroupSpec.dimensionSpec.grouping.eventTemplateFieldsList.isEmpty()) {
        throw RequiredFieldNotSetException("basic_report.result_group_specs.dimension_spec.grouping.event_template_fields")
          .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
      }

      for (eventFilter in resultGroupSpec.dimensionSpec.filtersList) {
        if (eventFilter.termsList.size != 1) {
          throw InvalidFieldValueException("basic_report.result_group_specs.dimension_spec.filters.terms")
            .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }

        for (eventTemplateField in eventFilter.termsList) {
          if (eventTemplateField.path.isEmpty()) {
            throw RequiredFieldNotSetException("basic_report.result_group_specs.dimension_spec.filters.terms.path")
              .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
          }

          if (eventTemplateField.value.selectorCase == EventTemplateField.FieldValue.SelectorCase.SELECTOR_NOT_SET) {
            throw RequiredFieldNotSetException("basic_report.result_group_specs.dimension_spec.filters.terms.value")
              .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
          }
        }
      }
      ...
      if (resultGroupSpec.resultGroupMetricSpec.hasComponentIntersection()) {
        throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.component_intersection") {
          "Not supported at this time"
        }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
      }

      if (resultGroupSpec.metricFrequency.selectorCase == MetricFrequencySpec.SelectorCase.SELECTOR_NOT_SET) {
      ...
      if (resultGroupSpec.metricFrequency.selectorCase == MetricFrequencySpec.SelectorCase.TOTAL) {
        if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.hasNonCumulative()) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.reporting_unit.non_cumulative") {
        ...
        if (resultGroupSpec.resultGroupMetricSpec.component.hasNonCumulative()) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.component.non_cumulative") {
            "non_cumulative cannot be specified when metric_frequency is total"
          }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }
      }
      ...
      if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.nonCumulative.percentKPlusReach) {
        if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.nonCumulative.kPlusReach <= 0) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.reporting_unit.non_cumulative.k_plus_reach") {
      ...
      if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.cumulative.percentKPlusReach) {
        if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.cumulative.kPlusReach <= 0) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.reporting_unit.cumulative.k_plus_reach") {
            "percent_k_plus_reach requires k_plus_reach to be positive"
          }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }
      }
      ...
      if (resultGroupSpec.resultGroupMetricSpec.reportingUnit.stackedIncrementalReach &&
        ...
        throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.reporting_unit.stacked_incremental_reach") {
      ...
      if (resultGroupSpec.resultGroupMetricSpec.component.nonCumulative.percentKPlusReach) {
        if (resultGroupSpec.resultGroupMetricSpec.component.nonCumulative.kPlusReach <= 0) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.component.non_cumulative.k_plus_reach") {
            "percent_k_plus_reach requires k_plus_reach to be positive"
          }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }
      }
      ...
      if (resultGroupSpec.resultGroupMetricSpec.component.cumulative.percentKPlusReach) {
        if (resultGroupSpec.resultGroupMetricSpec.component.cumulative.kPlusReach <= 0) {
          throw InvalidFieldValueException("basic_report.result_group_specs.result_group_metric_spec.component.cumulative.k_plus_reach") {
            "percent_k_plus_reach requires k_plus_reach to be positive"
          }.asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
        }
      }
