...
private fun MutableMap<
  ReportingSet,
  MutableMap<MetricCalculationSpecBuilderKey, MetricCalculationSpecBuilder>,
>
  .computeReportingUnitMetricSetSpecTransformation(
  ...
  if (reportingUnitMetricSetSpec.hasNonCumulative()) {
    val key = createMetricCalculationSpecBuilderKey(filter, groupings, false, metricFrequencySpec)
  ...
  if (reportingUnitMetricSetSpec.hasCumulative()) {
    val key = createMetricCalculationSpecBuilderKey(filter, groupings, true, metricFrequencySpec)
    metricCalculationSpecBuilderMap
      .computeIfAbsent(key) { MetricCalculationSpecBuilder() }
  ...
  if (reportingUnitMetricSetSpec.stackedIncrementalReach) {
    val firstMetricCalculationSpecBuilderMap =
      computeIfAbsent(
        dataProviderPrimitiveReportingSetMap.getValue(reportingUnit.componentsList.first())
      ) {
        mutableMapOf()
      }
    val firstKey =
      createMetricCalculationSpecBuilderKey(filter, groupings, true, metricFrequencySpec)
...
>
  ...
  for (primitiveReportingSet in primitiveReportingSets) {
    val metricCalculationSpecBuilderMap = computeIfAbsent(primitiveReportingSet) { mutableMapOf() }
  ...
  if (
    ...
    if (componentMetricSetSpec.hasNonCumulativeUnique()) {
      val firstKey =
        createMetricCalculationSpecBuilderKey(filter, groupings, false, metricFrequencySpec)
      firstMetricCalculationSpecBuilderMap
        .computeIfAbsent(firstKey) { MetricCalculationSpecBuilder() }
