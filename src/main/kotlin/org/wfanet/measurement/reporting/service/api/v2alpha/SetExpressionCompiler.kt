 ...
 */
private typealias MutablePrimitiveRegionToUnionSetCoefficientMap =
  ConcurrentMap<PrimitiveRegion, UnionSetCoefficientMap>
...
class SetExpressionCompiler {
   ...
   */
  private val primitiveRegionCache:
    ConcurrentMap<ReportingSetCount, MutablePrimitiveRegionToUnionSetCoefficientMap> =
    ConcurrentHashMap()
  /**
  ...
  private fun convertPrimitiveRegionsToUnionSetCoefficientMap(
    ...
    val primitiveRegionsToUnionSetCoefficients: Map<PrimitiveRegion, UnionSetCoefficientMap> =
      primitiveRegionCache.compute(reportingSetCount) { _, previousValue ->
        (previousValue ?: ConcurrentHashMap()).also { primitiveRegionsToUnionSetCoefficients ->
          for (region in primitiveRegions) {
            primitiveRegionsToUnionSetCoefficients.compute(region) {
              _,
              previousCoefficientMap: UnionSetCoefficientMap? ->
              if (previousCoefficientMap != null) {
                // Skip region that has already been computed.
                null
              } else {
