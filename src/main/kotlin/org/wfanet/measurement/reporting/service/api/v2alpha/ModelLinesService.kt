...
class ModelLinesService(
  ...
  override suspend fun listValidModelLines(request: ListValidModelLinesRequest): ListValidModelLinesResponse {
    ...
    if (Timestamps.compare(request.timeInterval.startTime, request.timeInterval.endTime) >= 0) {
      throw InvalidFieldValueException("time_interval") { fieldName -> "$fieldName must have a start_time that is before the end_time" }
        .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
    }
    ...
    for (eventGroup in request.eventGroupsList) {
      ...
      throw InvalidFieldValueException("event_groups") {
        fieldName -> "$eventGroup in $fieldName is invalid"
      }
        .asStatusRuntimeException(Status.Code.INVALID_ARGUMENT)
    }
      ...
      buildList {
        for (eventGroupName in request.eventGroupsList) {
          val kingdomEventGroup =
            try {
              kingdomEventGroupsStub.getEventGroup(getEventGroupRequest {
                name = eventGroupName
              })
          ...
          val dataProviderName = KingdomEventGroupKey.fromName(kingdomEventGroup.name)!!.parentKey.toName()
          if (!dataProvidersSet.contains(dataProviderName)) {
            dataProvidersSet.add(dataProviderName)
            val kingdomDataProvider =
              try {
                kingdomDataProvidersStub.getDataProvider(getDataProviderRequest {
    ...
    val modelLines: List<ModelLine> =
      try {
        kingdomModelLinesStub.listModelLines(listModelLinesRequest {
          parent = modelSuite
        }).modelLinesList
      } catch (e: StatusException) {
        throw when (e.status.code) {
          ...
          if (request.timeInterval.isFullyContainedWithin(modelLine)) {
            if (dataAvailabilityIntervals.areFullyContainedWithin(modelLine)) {
              val listModelRolloutsResponse =
                try {
                  kingdomModelRolloutsStub.listModelRollouts(listModelRolloutsRequest {
   ...
   */
  private fun modelLineComparator() = Comparator<ModelLine> { a, b ->
    when {
      a.type == b.type -> {
