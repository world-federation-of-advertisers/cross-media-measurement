...
abstract class ReportsServiceTest<T : ReportsCoroutineImplBase> {
  ...
  @Test
  fun `createReport returns report after report metrics have been created`() = runBlocking {
    createMeasurementConsumer(CMMS_MEASUREMENT_CONSUMER_ID, measurementConsumersService)
    val createdMetricCalculationSpec =
      createMetricCalculationSpec(CMMS_MEASUREMENT_CONSUMER_ID, metricCalculationSpecsService)
    val createdReportingSet = createReportingSet(CMMS_MEASUREMENT_CONSUMER_ID, reportingSetsService)

    val groupingPredicatesList: List<List<String>> =
      listOf(listOf("predicate1"), listOf("predicate2"))
    ...
    for (entry in createdReport.reportingMetricEntriesMap.entries) {
      for (metricCalculationSpecReportingMetrics in
        entry.value.metricCalculationSpecReportingMetricsList) {
        metricCalculationSpecReportingMetrics.reportingMetricsList.forEachIndexed {
          index,
          reportingMetric ->
          val externalMetricId = "externalMetricId$index"
          val request =
    ...
    assertThat(createdReportReusingMetrics)
      .ignoringFields(Report.EXTERNAL_REPORT_ID_FIELD_NUMBER, Report.CREATE_TIME_FIELD_NUMBER)
