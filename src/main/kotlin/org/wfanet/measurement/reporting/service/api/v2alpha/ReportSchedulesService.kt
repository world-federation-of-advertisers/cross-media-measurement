...
class ReportSchedulesService(
  ...
  override suspend fun createReportSchedule(request: CreateReportScheduleRequest): ReportSchedule {
    ...
    grpcRequire(request.reportScheduleId.matches(RESOURCE_ID_REGEX)) { "report_schedule_id is invalid." }
    ...
    val internalReportingSets: List<InternalReportingSet> = getInternalReportingSets(request.reportSchedule.reportTemplate, parentKey.measurementConsumerId, internalReportingSetsStub)
    checkDataAvailability(request.reportSchedule, eventStartTimestamp, internalReportingSets, dataProvidersStub, eventGroupsStub)

    val internalReportSchedule = try {
      internalReportSchedulesStub.createReportSchedule(createReportScheduleRequest {
  ...
  private suspend fun ReportSchedule.toInternal(measurementConsumerKey: MeasurementConsumerKey): InternalReportSchedule {
    val source = this

    val eventStart = source.eventStart
    grpcRequire(eventStart.year > 0 && eventStart.month > 0 && eventStart.day > 0 && (eventStart.hasUtcOffset() || eventStart.hasTimeZone())) {
    ...
    if (source.hasEventEnd()) {
      ...
      grpcRequire(
        eventEnd.year > 0 && eventEnd.month > 0 && eventEnd.day > 0
      ) {
    ...
    return internalReportSchedule {
      ...
      nextReportCreationTime =
        try {
          ...
          throw Status.INVALID_ARGUMENT.withDescription("event_start.time_zone.id is invalid").asRuntimeException()
        } catch (e: DateTimeException) {
          throw Status.INVALID_ARGUMENT.withDescription("event_start.utc_offset is not in valid range.")
            .asRuntimeException()
        }
    }
  }
  ...
  /**
   * Converts a public [Report] used as a template to an internal [InternalReport] used as a template.
   */
  private suspend fun Report.toInternal(measurementConsumerKey: MeasurementConsumerKey): InternalReport {
    val source = this
    ...
    val reportingMetricEntries: Map<String, InternalReport.ReportingMetricCalculationSpec> = source.reportingMetricEntriesList.associate { reportingMetricEntry ->
      if (reportingMetricEntry.value.metricCalculationSpecsList.isEmpty()) {
        throw Status.INVALID_ARGUMENT.withDescription("Entry in reporting_metric_entries has value with empty metric_calculation_specs").asRuntimeException()
      }
      ...
      grpcRequire(reportingSetKey.cmmsMeasurementConsumerId == measurementConsumerKey.measurementConsumerId) {
        ...
        ReportKt.reportingMetricCalculationSpec {
          metricCalculationSpecReportingMetrics +=
            reportingMetricEntry.value.metricCalculationSpecsList.map { metricCalculationSpecName ->
              ...
              grpcRequire(metricCalculationSpecKey.cmmsMeasurementConsumerId == measurementConsumerKey.measurementConsumerId) {
    ...
    val internalMetricCalculationSpecs: List<InternalMetricCalculationSpec> = getInternalMetricCalculationSpecs(metricCalculationSpecIds, measurementConsumerKey)
    for (internalMetricCalculationSpec in internalMetricCalculationSpecs) {
      if (internalMetricCalculationSpec.details.cumulative) {
        throw Status.INVALID_ARGUMENT.withDescription("metric_calculation_spec cannot have cumulative set to true.").asRuntimeException()
      }
    }
  ...
  private suspend fun getInternalMetricCalculationSpecs(externalMetricCalculationSpecIds: List<String>, measurementConsumerKey: MeasurementConsumerKey): List<InternalMetricCalculationSpec> {
    ...
    return submitBatchRequests(externalMetricCalculationSpecIds.distinct().asFlow(), BATCH_GET_METRIC_CALCULATION_SPECS_LIMIT, callRpc) { response ->
  ...
  companion object {
    ...
    /**
     * Converts a public [ReportSchedule.Frequency] to an internal [InternalReportSchedule.Frequency].
     */
      ...
      return InternalReportScheduleKt.frequency {
        when (source.frequencyCase) {
          ...
          ReportSchedule.Frequency.FrequencyCase.WEEKLY -> {
            grpcRequire(source.weekly.dayOfWeek != DayOfWeek.DAY_OF_WEEK_UNSPECIFIED && source.weekly.dayOfWeek != DayOfWeek.UNRECOGNIZED) {
    ...
    /**
     * Converts a public [ReportSchedule.ReportWindow] to an internal [InternalReportSchedule.ReportWindow].
     */
    private fun ReportSchedule.ReportWindow.toInternal(eventStart: DateTime): InternalReportSchedule.ReportWindow {
      val source = this
      @Suppress("WHEN_ENUM_CAN_BE_NULL_IN_JAVA") // Proto enum fields are never null.
        ...
        when (source.windowCase) {
          ReportSchedule.ReportWindow.WindowCase.TRAILING_WINDOW -> {
            ...
            trailingWindow = InternalReportScheduleKt.ReportWindowKt.trailingWindow {
              count = source.trailingWindow.count
              increment = when (source.trailingWindow.increment) {
                ReportSchedule.ReportWindow.TrailingWindow.Increment.DAY -> InternalReportSchedule.ReportWindow.TrailingWindow.Increment.DAY
                ReportSchedule.ReportWindow.TrailingWindow.Increment.WEEK -> InternalReportSchedule.ReportWindow.TrailingWindow.Increment.WEEK
                ReportSchedule.ReportWindow.TrailingWindow.Increment.MONTH -> InternalReportSchedule.ReportWindow.TrailingWindow.Increment.MONTH
                ...
                ReportSchedule.ReportWindow.TrailingWindow.Increment.INCREMENT_UNSPECIFIED ->
                  throw Status.INVALID_ARGUMENT.withDescription("increment in trailing_window is not specified.").asRuntimeException()
              }
            }
          }

          ReportSchedule.ReportWindow.WindowCase.FIXED_WINDOW -> {
            grpcRequire(source.fixedWindow.year > 0 && source.fixedWindow.month > 0 && source.fixedWindow.day > 0) {
              "fixed_window in report_window is not a full date."
            }
            grpcRequire(source.fixedWindow.isBefore(date {
              year = eventStart.year
              month = eventStart.month
              day = eventStart.day
            })) {
    ...
    private suspend fun getInternalReportingSets(reportTemplate: Report, cmmsMeasurementConsumerId: String, reportingSetsStub: ReportingSetsCoroutineStub): List<InternalReportingSet> {
      val reportingSetIds: Set<String> = reportTemplate.reportingMetricEntriesList.map { reportingMetricEntry ->
     ...
     */
    private suspend fun checkDataAvailability(reportSchedule: ReportSchedule, eventTimestamp: Timestamp, internalReportingSets: List<InternalReportingSet>, dataProvidersStub: DataProvidersCoroutineStub, eventGroupsStub: EventGroupsCoroutineStub) {
        ...
        if (internalReportingSet.hasPrimitive()) {
          for (eventGroupKey in internalReportingSet.primitive.eventGroupKeysList) {
            if (!eventGroupMap.containsKey(eventGroupKey)) {
              val eventGroupName = EventGroupKey(dataProviderId = eventGroupKey.cmmsDataProviderId, eventGroupId = eventGroupKey.cmmsEventGroupId).toName()
              val eventGroup =
                try {
                  eventGroupsStub.getEventGroup(
              ...
              if (Timestamps.compare(windowStart, eventGroup.dataAvailabilityInterval.startTime) < 0) {
                throw Status.FAILED_PRECONDITION.withDescription("ReportSchedule event_start is invalid due to the data_availability_interval of the EventGroup with name ${eventGroup.name}").asRuntimeException()
              }
            }
            ...
            if (!dataProviderMap.containsKey(eventGroupKey.cmmsDataProviderId)) {
              ...
              if (Timestamps.compare(windowStart, dataProvider.dataAvailabilityInterval.startTime) < 0) {
                throw Status.FAILED_PRECONDITION.withDescription("ReportSchedule event_start is invalid due to the data_availability_interval of the DataProvider with name ${dataProvider.name}").asRuntimeException()
              }
            }
          }
        }
    ...
    private fun Date.isBefore(other: Date): Boolean {
      return LocalDate.of(this.year, this.month, this.day).isBefore(LocalDate.of(other.year, other.month, other.day))
    }
  }
}
