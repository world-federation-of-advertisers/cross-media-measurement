...
abstract class ReportResultsServiceTest {
  ...
  @Test
  fun `batchCreateReportingSetResults throws when non-cumulative result missing for cumulative window`() {
    ensureMeasurementConsumer()
    val reportResultsStub = ReportResultsGrpc.newBlockingV2Stub(grpcChannel)
    val reportResult: ReportResult =
      reportResultsStub.createReportResult(CREATE_REPORT_RESULT_REQUEST)
    ...
    assertThat(exception.errorInfo)
      .isEqualTo(
        errorInfo {
          ...
          metadata[Errors.Metadata.REPORTING_WINDOW_NON_CUMULATIVE_START.key] = "null"
