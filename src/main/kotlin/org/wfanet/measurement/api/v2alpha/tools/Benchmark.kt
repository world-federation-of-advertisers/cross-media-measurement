...
class Benchmark(
  ...
  private fun collectCompletedTasks(
    ...
    while (iTask < taskList.size) {
      ...
      if(measurement.state == Measurement.State.COMPUTING && task.requisitionFulfilledTime == null) {
    ...
    File(flags.outputFile).printWriter().use { out ->
      ...
      for (task in completedTasks) {
        ...
        if(task.requisitionFulfilledTime != null) {
          out.print(",${(task.requisitionFulfilledTime!!.toEpochMilli() - firstInstant.toEpochMilli()) / 1000.0}")
        } else {
          out.print(",-1")
        }
