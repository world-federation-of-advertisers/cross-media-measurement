    ...
    .also {
      it.name =
        RequisitionKey(
          ...
          .toName()
      it.dataProvider =
        when (details.apiVersion.toPublicApiVersion()) {
...
fun InternalMeasurement.toSystemComputation(): Computation {
  return Computation.newBuilder()
    .also {
      ...
      it.protocolConfig =
        when (details.apiVersion.toPublicApiVersion()) {
          PublicApiVersion.V2_ALPHA -> ProtocolConfigKey(externalProtocolConfigId).toName()
        }
