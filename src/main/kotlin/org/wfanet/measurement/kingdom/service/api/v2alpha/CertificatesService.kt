...
import org.wfanet.measurement.api.v2alpha.certificate
import org.wfanet.measurement.api.v2alpha.dataProviderCertificateName
import org.wfanet.measurement.api.v2alpha.duchyCertificateName
import org.wfanet.measurement.api.v2alpha.measurementConsumerCertificateName
import org.wfanet.measurement.api.v2alpha.modelProviderCertificateName
  ...
  val name =
    ...
    when (parentCase) {
      InternalCertificate.ParentCase.EXTERNAL_MEASUREMENT_CONSUMER_ID ->
        measurementConsumerCertificateName(
          externalIdToApiId(externalMeasurementConsumerId),
          certificateApiId
        )
      InternalCertificate.ParentCase.EXTERNAL_DATA_PROVIDER_ID ->
        dataProviderCertificateName(externalIdToApiId(externalDataProviderId), certificateApiId)
      InternalCertificate.ParentCase.EXTERNAL_DUCHY_ID ->
        duchyCertificateName(externalDuchyId, certificateApiId)
      InternalCertificate.ParentCase.EXTERNAL_MODEL_PROVIDER_ID ->
        modelProviderCertificateName(externalIdToApiId(externalModelProviderId), certificateApiId)
