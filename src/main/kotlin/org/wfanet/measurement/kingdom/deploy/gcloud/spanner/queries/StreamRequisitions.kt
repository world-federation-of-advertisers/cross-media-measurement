...
class StreamRequisitions(requestFilter: StreamRequisitionsRequest.Filter, limit: Int = 0) :
  SpannerQuery<Requisition, Requisition>() {
  override val reader: BaseSpannerReader<Requisition> =
    RequisitionReader(Requisition.View.FULL).fillStatementBuilder {
  ...
  override fun Flow<Requisition>.transform(): Flow<Requisition> = this
}
...
private fun Statement.Builder.appendWhereClause(filter: StreamRequisitionsRequest.Filter) {
  ...
  if (filter.measurementStatesList.isNotEmpty()) {
