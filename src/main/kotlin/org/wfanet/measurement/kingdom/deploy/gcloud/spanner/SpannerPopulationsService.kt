...
class SpannerPopulationsService(
  ...
  override suspend fun getPopulation(request: GetPopulationRequest): Population {
    return PopulationReader().readByExternalPopulationId(client.singleUse(), ExternalId( request.externalDataProviderId), ExternalId( request.externalPopulationId))?.population?: failGrpc(Status.NOT_FOUND) { "Population not found" }
  }
