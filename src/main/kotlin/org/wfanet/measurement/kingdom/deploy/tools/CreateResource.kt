...
private abstract class CreatePrincipalCommand : Callable<Int> {
  ...
  @Option(
    names = ["--public-key-file"],
    description = ["Principal's serialized public key"],
    required = true
  )
  private lateinit var publicKeyFile: File

  protected val serializedPublicKey: ByteString by lazy { publicKeyFile.readBytes().toByteString() }

  @Option(
    names = ["--public-key-signature-file"],
    description = ["Principal's public key's signature"],
    required = true
  )
  private lateinit var publicKeySignatureFile: File

  protected val publicKeySignature: ByteString by lazy {
    publicKeySignatureFile.readBytes().toByteString()
  }
}
    ...
    val dataProvider = dataProvider {
      ...
      details =
        DataProviderKt.details {
          ...
          publicKeySignature = this@CreateDataProviderCommand.publicKeySignature
        }
    }
    ...
    val modelProvider = modelProvider {
      ...
      details =
        ModelProviderKt.details {
          apiVersion = API_VERSION
          publicKey = serializedPublicKey
          publicKeySignature = this@CreateModelProviderCommand.publicKeySignature
        }
    }
