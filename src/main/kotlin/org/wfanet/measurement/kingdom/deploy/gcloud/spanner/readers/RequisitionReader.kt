...
import org.wfanet.measurement.internal.kingdom.Requisition.View
import org.wfanet.measurement.internal.kingdom.RequisitionKt.Dsl as RequisitionDsl
...
private val COMMON_SELECT_EXPRESSIONS =
  listOf(
    "Requisitions.*",
...
private val NORMALIZED_SELECT_EXPRESSIONS = COMMON_SELECT_EXPRESSIONS
private val FULL_SELECT_EXPRESSIONS = COMMON_SELECT_EXPRESSIONS.plus("MeasurementDetails")
...
class RequisitionReader(private val view: View) : BaseSpannerReader<Requisition>() {
  override val builder: Statement.Builder = initBuilder(view)

  override suspend fun translate(struct: Struct): Requisition {
    return when (view) {
      View.FULL -> buildFullRequisition(struct)
      View.NORMALIZED -> buildNormalizedRequisition(struct)
      View.UNRECOGNIZED -> throw IllegalArgumentException("Unrecognized Requisition.View")
    }
  }
  ...
  companion object {
    /** Builds a [Requisition] from [struct] with [View.FULL]. */
    fun buildFullRequisition(struct: Struct): Requisition = requisition {
      fillRequisitionCommon(struct)
      parentMeasurement =
        parentMeasurement {
    ...
    /** Builds a [Requisition] from [struct] with [View.NORMALIZED]. */
    fun buildNormalizedRequisition(struct: Struct): Requisition = requisition {
      fillRequisitionCommon(struct)
    }
  }
}

private fun RequisitionDsl.fillRequisitionCommon(struct: Struct) {
  ...
  for (duchyStruct in struct.getStructList("Duchies")) {
    ...
    val externalDuchyId =
      checkNotNull(DuchyIds.getExternalId(duchyId)) { "External ID not found for Duchy $duchyId" }
...
private fun initBuilder(view: View): Statement.Builder {
  val sqlBuilder = StringBuilder("SELECT\n")
  when (view) {
    View.FULL -> FULL_SELECT_EXPRESSIONS
    View.NORMALIZED -> NORMALIZED_SELECT_EXPRESSIONS
    View.UNRECOGNIZED -> throw IllegalArgumentException("Unrecognized Requisition.View")
  }.joinTo(sqlBuilder, ", ")
