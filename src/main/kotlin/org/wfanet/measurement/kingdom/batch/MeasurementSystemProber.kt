...
class MeasurementSystemProber(
  ...
  private val measurementConsumersService: MeasurementConsumersGrpcKt.MeasurementConsumersCoroutineStub,
  private val publicMeasurementsService: org.wfanet.measurement.api.v2alpha.MeasurementsGrpcKt.MeasurementsCoroutineStub,
  ...
  fun run() {
    ...
    for (dataProviderName in dataProviderNames) {
      ...
      val dataProvider = runBlocking {
        try {
          dataProvidersService.withAuthenticationKey(apiAuthenticationKey)
      ...
      val eventGroups = runBlocking {
        eventGroupsService.withAuthenticationKey(apiAuthenticationKey)
          .listEventGroups(listEventGroupsRequest)
      }.eventGroupsList.toList()

      if (eventGroups.size != 1) {
        logger.warning("There should be exactly 1:1 mapping between a data provider and an event group, but data provider $dataProvider is related to ${eventGroups.size} event groups")
        return
      }
    ...
    val measurement = measurement {
      ...
      dataProviders += dataProviderNames.map {
   ...
   *
   * case 1: no previous measurements -> YES
   * case 2: previous measurements exist
   *   a) at least 1 finished a long time ago -> Yes
   *   b) finished ones are finished too soon -> No
   *   c) no finished measurements -> no
   */
  private suspend fun shouldCreateNewMeasurement(): Boolean {
    ...
    val oldFinishedMeasurements: List<Measurement> = internalMeasurementsService
      .streamMeasurements(
        streamMeasurementsRequest {
          filter =
