...
suspend fun AsyncDatabaseClient.TransactionContext.batchCreateImpressionMetadata(
  ...
  val creations: List<ImpressionMetadata> =
      requests
          ...
          .map { request ->
            if (
                existingBlobUriToImpressionMetadata.containsKey(request.impressionMetadata.blobUri)
            ) {
              throw ImpressionMetadataAlreadyExistsException(
...
fun AsyncDatabaseClient.ReadContext.readImpressionMetadata(
  ...
  val query =
      statement(sql) {
        ...
        if (filter.hasIntervalOverlaps()) {
          bind("intervalOverlapsStartTime")
              .to(filter.intervalOverlaps.startTime.toGcloudTimestamp())
