...
class VidLabelingDispatcherFunction : HttpFunction {
  ...
  override fun service(request: HttpRequest, response: HttpResponse) {
    try {
      ...
      val batchMaxSizeBytes: Long =
        if (config.hasMaxBatchSizeBytes()) config.maxBatchSizeBytes
  ...
  companion object {
    ...
    // TODO(world-federation-of-advertisers/cross-media-measurement#3584): Add
     ...
     *
     * Converts config-layer types to v1alpha types for the TEE application. Per-batch fields
     * (`input_blob_uris` and `batch_index`) are set by [VidLabelingDispatcher] for each batch.
     *
