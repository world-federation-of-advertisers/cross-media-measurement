...
class DashboardComplianceCheck : Runnable {
  ...
  private val edps by lazy {
    listOf(
      ...
      EdpConfig("comscore", "d9hrk_MNong", "edp-comscore-dashboard@$project.iam.gserviceaccount.com"),
  ...
  private val credentials: GoogleCredentials by lazy {
    val creds = GoogleCredentials.getApplicationDefault()
      .createScoped("https://www.googleapis.com/auth/bigquery", "https://www.googleapis.com/auth/cloud-platform")
    creds
  }
  ...
  private fun bigQueryAsEdp(edp: EdpConfig): BigQuery {
    ...
    val impersonatedCredentials =
      ImpersonatedCredentials.create(
        ...
        edp.saEmail,
        null,
        listOf("https://www.googleapis.com/auth/bigquery", "https://www.googleapis.com/auth/cloud-platform"),
        300,
      )
  ...
  private fun checkDataIsolation() {
    ...
    for (edp in edps) {
      ...
      try {
        ...
        val resourceIds = result.iterateAll().map { it["DataProviderResourceId"].stringValue }.toSet()
  ...
  private fun checkDriftDetection() {
    ...
    val forbiddenColumns = setOf("TotalMcs", "CoveragePercent", "EdpCount", "cross_publisher", "edp_distribution", "data_provider_keys", "result_group_specs")
      ...
      for (row in result.iterateAll()) {
        ...
        for (pattern in forbiddenPatterns) {
          if (pattern.matches(columnName)) {
            violations.add("$tableName: column '$columnName' matches forbidden pattern '${pattern.pattern}'")
          }
        }
      }
    ...
    val expectedColumns = mapOf(
      "requisition_overview" to setOf(
        "DataProviderResourceId", "Report", "CmmsMeasurementConsumer", "RequisitionState",
        "CmmsCreateTime", "FulfilledTime", "FulfillmentDurationSeconds", "ReportState",
        "ReportStartYear", "ReportStartMonth", "ReportStartDay",
        "ReportEndYear", "ReportEndMonth", "ReportEndDay", "ImpressionQualificationFilters",
      ),
      "mc_details_edp" to setOf(
        "CmmsMeasurementConsumer", "CmmsDataProvider", "EventGroupCount",
        "EventGroupIds", "CampaignNames", "BrandNames", "AccountIds",
      ),
      "report_detail_edp" to setOf(
        "BasicReportId", "CmmsDataProvider", "EventGroupCount",
        "EventGroupIds", "CampaignNames", "BrandNames",
      ),
      "mc_details" to setOf(
        "CmmsMeasurementConsumer", "CmmsDataProvider", "EventGroupCount",
        "EventGroupIds", "CampaignNames", "BrandNames", "AccountIds",
      ...
      "report_detail" to setOf(
        "BasicReportId", "CmmsDataProvider", "EventGroupCount",
        "EventGroupIds", "CampaignNames", "BrandNames", "EdpCount",
      ),
    )
