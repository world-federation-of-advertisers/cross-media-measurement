...
data class RawImpressionUploadKey(
  ...
  companion object FACTORY : ResourceKey.Factory<RawImpressionUploadKey> {
    const val PATTERN =
      "${DataProviderKey.PATTERN}/rawImpressionUploads/{raw_impression_upload}"
...
/** [ResourceKey] of a RawImpressionUploadModelLine. */
data class RawImpressionUploadModelLineKey(
  override val parentKey: RawImpressionUploadKey,
  val rawImpressionUploadModelLineId: String,
) : ChildResourceKey {
  constructor(
    dataProviderId: String,
    rawImpressionUploadId: String,
    rawImpressionUploadModelLineId: String,
  ) : this(
  ...
  override fun toName(): String {
    return parser.assembleName(
      mapOf(
        IdVariable.DATA_PROVIDER to dataProviderId,
        IdVariable.RAW_IMPRESSION_UPLOAD to rawImpressionUploadId,
        IdVariable.RAW_IMPRESSION_UPLOAD_MODEL_LINE to rawImpressionUploadModelLineId,
      )
    )
  }

  companion object FACTORY : ResourceKey.Factory<RawImpressionUploadModelLineKey> {
    const val PATTERN =
      "${RawImpressionUploadKey.PATTERN}/rawImpressionUploadModelLines/{raw_impression_upload_model_line}"
    private val parser = ResourceNameParser(PATTERN)

    override fun fromName(resourceName: String): RawImpressionUploadModelLineKey? {
      val idVars: Map<IdVariable, String> = parser.parseIdVars(resourceName) ?: return null
      return RawImpressionUploadModelLineKey(
        idVars.getValue(IdVariable.DATA_PROVIDER),
        idVars.getValue(IdVariable.RAW_IMPRESSION_UPLOAD),
        idVars.getValue(IdVariable.RAW_IMPRESSION_UPLOAD_MODEL_LINE),
      )
    }
  }
}
