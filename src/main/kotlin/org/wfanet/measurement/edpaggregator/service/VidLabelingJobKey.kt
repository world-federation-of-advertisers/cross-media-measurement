...
/** [ResourceKey] of a VidLabelingJob. */
data class VidLabelingJobKey(
  override val parentKey: RawImpressionUploadKey,
  val vidLabelingJobId: String,
) : ChildResourceKey {
  constructor(
    dataProviderId: String,
    rawImpressionUploadId: String,
    vidLabelingJobId: String,
  ) : this(RawImpressionUploadKey(dataProviderId, rawImpressionUploadId), vidLabelingJobId)
  ...
  override fun toName(): String {
    return parser.assembleName(
      mapOf(
        IdVariable.DATA_PROVIDER to dataProviderId,
        IdVariable.RAW_IMPRESSION_UPLOAD to rawImpressionUploadId,
        IdVariable.VID_LABELING_JOB to vidLabelingJobId,
      )
    )
  }

  companion object FACTORY : ResourceKey.Factory<VidLabelingJobKey> {
    const val PATTERN = "${RawImpressionUploadKey.PATTERN}/vidLabelingJobs/{vid_labeling_job}"
    private val parser = ResourceNameParser(PATTERN)

    override fun fromName(resourceName: String): VidLabelingJobKey? {
      val idVars: Map<IdVariable, String> = parser.parseIdVars(resourceName) ?: return null
      return VidLabelingJobKey(
        idVars.getValue(IdVariable.DATA_PROVIDER),
        idVars.getValue(IdVariable.RAW_IMPRESSION_UPLOAD),
        idVars.getValue(IdVariable.VID_LABELING_JOB),
      )
    }
  }
}
