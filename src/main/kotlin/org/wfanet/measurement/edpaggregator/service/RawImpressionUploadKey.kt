...
/** [ResourceKey] of a RawImpressionUploadFile. */
data class RawImpressionUploadFileKey(
  ...
  constructor(
    ...
    rawImpressionUploadId: String,
    fileId: String,
  ) : this(RawImpressionUploadKey(dataProviderId, rawImpressionUploadId), fileId)
  ...
  companion object FACTORY : ResourceKey.Factory<RawImpressionUploadFileKey> {
    const val PATTERN = "${RawImpressionUploadKey.PATTERN}/files/{file}"
    private val parser = ResourceNameParser(PATTERN)

    override fun fromName(resourceName: String): RawImpressionUploadFileKey? {
      val idVars: Map<IdVariable, String> = parser.parseIdVars(resourceName) ?: return null
      return RawImpressionUploadFileKey(
        idVars.getValue(IdVariable.DATA_PROVIDER),
        idVars.getValue(IdVariable.RAW_IMPRESSION_UPLOAD),
