...
abstract class PoolAssignmentJobServiceTest {
  ...
  /**
   * Creates a parent [RawImpressionUploadModelLine] row (empty pool offsets, no max event date) for
   * the given (upload, model line). MarkSucceeded requires this parent row so it can merge each
   * shard's pool offsets / max event date into it.
   */
