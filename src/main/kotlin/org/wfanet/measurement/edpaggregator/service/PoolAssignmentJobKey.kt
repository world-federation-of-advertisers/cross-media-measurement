...
/** [ResourceKey] of a PoolAssignmentJob. */
data class PoolAssignmentJobKey(
  override val parentKey: RawImpressionUploadKey,
  val poolAssignmentJobId: String,
) : ChildResourceKey {
  constructor(
    dataProviderId: String,
    rawImpressionUploadId: String,
    poolAssignmentJobId: String,
  ) : this(RawImpressionUploadKey(dataProviderId, rawImpressionUploadId), poolAssignmentJobId)
  ...
  override fun toName(): String {
    return parser.assembleName(
      mapOf(
        ...
        IdVariable.POOL_ASSIGNMENT_JOB to poolAssignmentJobId,
      )
    )
  }

  companion object FACTORY : ResourceKey.Factory<PoolAssignmentJobKey> {
    const val PATTERN = "${RawImpressionUploadKey.PATTERN}/poolAssignmentJobs/{pool_assignment_job}"
    private val parser = ResourceNameParser(PATTERN)

    override fun fromName(resourceName: String): PoolAssignmentJobKey? {
      val idVars: Map<IdVariable, String> = parser.parseIdVars(resourceName) ?: return null
      return PoolAssignmentJobKey(
        idVars.getValue(IdVariable.DATA_PROVIDER),
        idVars.getValue(IdVariable.RAW_IMPRESSION_UPLOAD),
        idVars.getValue(IdVariable.POOL_ASSIGNMENT_JOB),
      )
    }
  }
}
