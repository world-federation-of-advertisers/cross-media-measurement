...
class EdpValidationPostProcessor(
  ...
  enum class ReportValidationResult {
    ...
    FAILED,
    /**
     * Configured DataProviders were queried, but no query produced a verdict (all errored/skipped).
     */
    SKIPPED,
    /** No row matched any DataProvider validation config — nothing was validated. */
    NOT_CONFIGURED,
  }
  ...
  fun validate(rows: List<ImpressionDataRow>): ReportValidationResult {
    ...
    for ((dataProviderName, dataProviderRows) in rowsByDataProvider) {
      ...
      when (validateDataProvider(dataProviderName, config, dataProviderRows)) {
        ...
        // validateDataProvider only ever returns PASSED/FAILED/SKIPPED.
        ReportValidationResult.NOT_CONFIGURED -> {}
      }
    }
    ...
    val result =
      when {
        ...
        anyConfigured -> ReportValidationResult.SKIPPED
        else -> ReportValidationResult.NOT_CONFIGURED
      }
    metrics.reportResultsCounter.add(1, Attributes.of(EdpValidationMetrics.RESULT_KEY, result.name))
    return result
  }

  /**
   * Deterministic request ID for a query, so retries of the same logical query are not seen as new
   * requests by the publisher cloud function (idempotency) and can be correlated across logs.
   *
   ...
   * @return the deterministic request ID
   */
  private fun stableRequestId(dataProviderName: String, row: ImpressionDataRow): String {
    val key =
      "$dataProviderName|${row.entityType}|${row.entityId}|${row.startTimeSeconds}|" +
        "${row.endTimeSeconds}"
  ...
  private fun validateDataProvider(
    ...
    for (row in rows) {
      val request = dataProviderImpressionQueryRequest {
        requestId = stableRequestId(dataProviderName, row)
      ...
      val timeout =
        if (config.hasRequestTimeoutOverride()) {
          config.requestTimeoutOverride.toDuration()
        } else {
          DEFAULT_TIMEOUT
        }
      ...
      when {
        ...
        response.hasResult() -> {
          val publisherCount = response.result.value
          // An unset `minimum_impression_count` (optional) means "apply the default floor"; an
          // explicit value (including 0, which disables the floor) is honored as-is. The proto
          // default for an unset optional is 0, so gate on presence before falling back.
          val minimumImpressionCount =
            if (config.tolerance.hasMinimumImpressionCount())
              config.tolerance.minimumImpressionCount
            else DEFAULT_MIN_IMPRESSION_COUNT
          if (publisherCount < minimumImpressionCount) {
            // Publisher count is below the configured floor; skip validation without comparing.
            logger.info(
              "Validation SKIPPED for $dataProviderName entity ${row.entityId}: publisher count " +
                "$publisherCount below minimum $minimumImpressionCount"
            )
            metrics.skippedQueriesCounter.add(
              1,
              Attributes.of(
            ...
            // Scale the publisher count to the EDPA's measured population before comparing.
            val expectedCount = (publisherCount * row.vidSamplingWidth).toLong()
            val evaluation =
            ...
            when (evaluation.verdict) {
              ToleranceEvaluator.Verdict.SKIPPED -> {
                // No expected impressions to compare against (e.g. the sampling width rounds the
                // expected count to zero); count it as a skip and emit no deviation or verdict.
                logger.info(
                  "Validation SKIPPED for $dataProviderName entity ${row.entityId}: " +
                    "no expected impressions"
                )
                metrics.skippedQueriesCounter.add(
                  1,
                  Attributes.of(
                    ...
                    dataProviderName,
                    EdpValidationMetrics.SKIP_REASON_KEY,
                    // Distinct from the publisher-floor skip ("below_minimum_count"): this fires
                    // when
                    // the scaled expected count rounds to zero, so dashboards can tell them apart.
                    "no_expected_impressions",
                  ),
                )
              }
                  ...
                  ToleranceEvaluator.Verdict.FAIL -> {
                    logger.log(
                      ...
                      "Validation FAILED for $dataProviderName entity ${row.entityId}: " +
                        "reported=${evaluation.reportedCount}, publisher=$publisherCount, " +
                        "expected=${evaluation.expectedCount}, deviation=${evaluation.deviationFraction}, " +
                        "tolerance=${evaluation.effectiveTolerance}",
                    )
                    anyFailed = true
                  }
                  ToleranceEvaluator.Verdict.WARNING ->
  ...
  companion object {
    ...
    // Default impression-count floor applied when `ToleranceConfig.minimum_impression_count` is
    // unset, per its proto contract. An explicit 0 disables the floor (always validate).
    private const val DEFAULT_MIN_IMPRESSION_COUNT = 1000L
  }
}
