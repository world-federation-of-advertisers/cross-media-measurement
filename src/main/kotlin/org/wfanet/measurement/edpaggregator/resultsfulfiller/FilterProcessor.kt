...
class FilterProcessor(
   ...
   */
  private var program: Program = if (filterSpec.celExpression.isEmpty()) {
  ...
  private fun batchTimeRangeOverlaps(batch: EventBatch): Boolean {
    ...
    val overlapStart = if (batch.minTime.isBefore(startInstant)) startInstant else batch.minTime
    val overlapEnd = if (batch.maxTime.isBefore(endInstant)) batch.maxTime else endInstant
