 ...
 *
 * Error handling:
 * - @throws [ImpressionReadException] on storage errors.
 ...
 */
class EventProcessingOrchestrator(private val privateEncryptionKey: PrivateKeyHandle) {
  ...
  suspend fun run(
    ...
    logger.info(
      "Starting EventProcessingOrchestrator.runWithRequisitions with ${requisitions.size} requisitions"
    )
   ...
   */
  private fun createPipeline(config: PipelineConfiguration): EventProcessingPipeline {
