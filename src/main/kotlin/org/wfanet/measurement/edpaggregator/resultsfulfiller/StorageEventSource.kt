...
class StorageEventSource(
      ...
      return eventGroupDetailsList
        .flatMap { eventGroupDetails ->
          eventGroupDetails.collectionIntervalsList.flatMap { interval ->
            val startDate = LocalDate.ofInstant(interval.startTime.toInstant(), zoneId)
            // One second is subtracted since end of interval is exclusive just in case the time
            // is midnight to avoid fetching an extra day.
            val endDate = LocalDate.ofInstant(interval.endTime.toInstant().minusSeconds(1), zoneId)

            // Validate date range
            if (startDate.isEqual(endDate)) {
              throw IllegalArgumentException(
                "Start date and end date are the same ($startDate) for event group ${eventGroupDetails.eventGroupReferenceId}. " +
                "Collection intervals must span at least one full day."
              )
            }
            if (startDate.isAfter(endDate)) {
              throw IllegalArgumentException(
                "Start date ($startDate) is after end date ($endDate) for event group ${eventGroupDetails.eventGroupReferenceId}. " +
                "Collection intervals must have a valid date range."
              )
            }
            ...
            startDate.datesUntil(endDate.plusDays(1)).asSequence().map { date ->
              EventGroupDate(
  ...
  constructor(
    ...
    DefaultEventPathResolver(
      impressionsMetadataBucketUri = impressionsDekBucketUri
    ),
