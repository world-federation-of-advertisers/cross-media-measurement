...
class StorageEventSource(
  ...
  override suspend fun generateEventBatches(dispatcher: CoroutineContext): Flow<EventBatch> {
    ...
    return channelFlow {
      ...
      val processingJobs =
        eventReaders.map { eventReader ->
          launch(dispatcher) {
            val (batchCount, eventCount) =
              processEventReader(eventReader) { eventBatch -> send(eventBatch) }
  ...
  private suspend fun createEventReaders(): List<StorageEventReader> {
    val allSources = eventGroupDetailsList.flatMap { details ->
      details.collectionIntervalsList.flatMap { interval ->
        impressionMetadataService.listImpressionDataSources(
          modelLine = modelLine,
          eventGroupReferenceId = details.eventGroupReferenceId,
          period = interval,
        )
      }
    }
    ...
    return uniqueSources.map { source ->
      StorageEventReader(
        blobDetails = source.blobDetails,
        kmsClient = kmsClient,
  ...
  private suspend fun processEventReader(
    ...
    logger.fine(
      "Reading events from ${blobDetails.blobUri}"
    )
    ...
    logger.fine(
      "Read $eventCount events in $batchCount batches for ${blobDetails.blobUri}"
    )
