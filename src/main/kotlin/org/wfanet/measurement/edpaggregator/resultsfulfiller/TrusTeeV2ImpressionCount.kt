 ...
 * carries no sampling error. [frequencyVector] already holds `min(f, 127)` per VID over that
 * population, so the count reads off an array the fulfiller has built.
 *
...
fun buildTrusTeeV2FulfillmentDetails(
  ...
  val frequencyData: IntArray = frequencyVector.getByteArray().map { it.toInt() }.toIntArray()
    ...
    impression =
      when (params.capMode) {
        ...
        ImpressionCapMode.CUSTOM_CAP -> {
          ...
          impression {
            value = frequencyData.sumOf { minOf(it, cap).toLong() }
            noiseMechanism = ProtocolConfig.NoiseMechanism.NONE
