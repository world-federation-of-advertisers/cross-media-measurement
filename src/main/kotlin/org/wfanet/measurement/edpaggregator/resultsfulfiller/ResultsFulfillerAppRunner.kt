...
class ResultsFulfillerAppRunner : Runnable {
      ...
      val kmsConfig =
        GCloudWifCredentials(
          ...
          serviceAccountImpersonationUrl =
            EDP_TARGET_SERVICE_ACCOUNT_FORMAT.format(edpConfig.serviceAccount),
        )
  ...
  fun saveEdpsCerts() {
    edpConfigs.configsList.forEach { edpConfig ->
      ...
      val edpPrivateDer = accessSecretBytes(googleProjectId, edpConfig.encPrivateDerSecretId, SECRET_VERSION)
      saveByteArrayToFile(edpPrivateDer, edpConfig.encPrivateDerLocalPath)
      val edpEncPrivate = accessSecretBytes(googleProjectId, edpConfig.encPrivateSecretId, SECRET_VERSION)
      saveByteArrayToFile(edpEncPrivate, edpConfig.encPrivateLocalPath)
      val edpTlsKey = accessSecretBytes(googleProjectId, edpConfig.tlsKeySecretId, SECRET_VERSION)
      saveByteArrayToFile(edpTlsKey, edpConfig.tlsKeyLocalPath)
      val edpTlsPem = accessSecretBytes(googleProjectId, edpConfig.tlsPemSecretId, SECRET_VERSION)
      saveByteArrayToFile(edpTlsPem, edpConfig.tlsPemLocalPath)
    }
  }
