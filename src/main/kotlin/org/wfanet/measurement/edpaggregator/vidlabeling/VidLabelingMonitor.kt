...
class VidLabelingMonitor(
  ...
  /**
   * Surfaces uploads stuck in a non-terminal state past [stalenessThreshold] (logged at `SEVERE`)
   * and model lines in `FAILED` (via [VidLabelingMonitorMetrics.failedUploadsGauge]) for alerting.
   *
  ...
  private suspend fun checkFailuresAndStaleness(): Pair<List<String>, List<String>> {
    ...
    val failedModelLines: List<String> =
      (createdUploads + activeUploads + failedUploads).flatMap { upload ->
        listUploadModelLines(upload.name)
          .filter { it.state == RawImpressionUploadModelLine.State.FAILED }
          .map { it.name }
      }
    // failedUploadsGauge is the alerting signal (alert on gauge > 0 with a duration window, which
    // dedupes naturally). No per-tick SEVERE log here, which would re-page on every Cloud Scheduler
    // tick for the same FAILED model line until manual recovery.
