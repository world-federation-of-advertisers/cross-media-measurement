...
object Tracing {
  ...
  /**
   * Executes a block within a new span using pre-built span attributes, handling both
   * suspendable and non-suspendable code.
   *
   * @param spanName Human-readable span name
   * @param attributes Span attributes (low-cardinality only)
   * @param block Code to execute
  ...
  /**
   * Installs W3C trace context for the current thread using the provided headers.
   *
   * Use this overload for purely synchronous work. For suspending work, prefer the suspend overload
   * to ensure coroutine context propagation.
   */
  inline fun <T> withW3CTraceContext(
    headers: Map<String, List<String>>?,
    block: () -> T,
  ): T {
    val parentContext = extractW3CContext(headers)
  ...
  @PublishedApi internal fun extractW3CContext(headers: Map<String, List<String>>?): Context {
    if (headers.isNullOrEmpty()) {
      return Context.current()
    }
    return w3cPropagator.extract(Context.current(), headers, HttpHeaderGetter)
  }
}
...
private object HttpHeaderGetter : TextMapGetter<Map<String, List<String>>> {
  override fun keys(carrier: Map<String, List<String>>): Iterable<String> = carrier.keys

  override fun get(carrier: Map<String, List<String>>?, key: String): String? {
    carrier ?: return null
    val entry =
      carrier.entries.firstOrNull { entry -> entry.key.equals(key, ignoreCase = true) }
        ?: return null
