...
object Tracing {
   ...
   *
   * This is an extension on [CoroutineContext] so that suspend callers prefer this overload over the
   ...
   *
   * @receiver Coroutine context that launches the traced suspending work
   *
   * @param spanName Human-readable span name
   * @param attributes Span attributes (low-cardinality only)
   * @param block Code to execute
   */
  suspend fun <T> CoroutineContext.trace(
    spanName: String,
    attributes: Map<String, String> = emptyMap(),
    block: suspend () -> T,
  ): T {
  ...
  @PublishedApi internal fun extractW3CContext(request: HttpRequest): Context {
    return w3cPropagator.extract(
      Context.current(),
      request,
      CloudFunctionsHttpRequestGetter,
    )
  }
}
 ...
 * Executes a synchronous block within a new span.
 *
 * For suspending code, use the [CoroutineContext.trace] extension instead.
 */
inline fun <T> trace(
  spanName: String,
  attributes: Map<String, String> = emptyMap(),
  crossinline block: () -> T,
): T = Tracing.trace(spanName, attributes, block)
