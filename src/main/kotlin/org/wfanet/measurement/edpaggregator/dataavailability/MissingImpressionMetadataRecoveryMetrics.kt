...
class MissingImpressionMetadataRecoveryMetrics(meter: Meter) {
  ...
  /** Deleted resources that could not be restored in the current run. */
  val failedUndeletesGauge: LongGauge =
    meter
      .gaugeBuilder("edpa.data_availability_recovery.failed_undeletes")
      .setDescription("Deleted ImpressionMetadata resources that failed restoration")
      .setUnit("{resource}")
      .ofLongs()
      .build()
