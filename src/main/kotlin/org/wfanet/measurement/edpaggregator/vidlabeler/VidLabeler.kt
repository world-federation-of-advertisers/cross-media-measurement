...
class VidLabeler<E : ParquetRawEvent>(
  ...
  suspend fun label(): Set<LocalDate> {
    ...
    val contexts =
      specs.map { spec ->
        ModelLineContext(
          ...
          rawImpressionUpload = spec.rawImpressionUpload,
    ...
    //   is_last_shard=true response signals the service-side flip (LABELING -> COMPLETED) per
    //   design doc section 388; without it an upload that reaches LABELING stays there forever.
    // TODO(world-federation-of-advertisers/cross-media-measurement#4010): On is_last_shard=true,
 ...
 * @property rawImpressionUpload source upload recorded in output metadata sidecars.
