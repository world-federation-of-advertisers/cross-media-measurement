 ...
 * which reads the Riegeli-encoded [CompiledNode] list (the `model_blob_path` payload resolved by
 * the `VidLabelingDispatcher`).
 */
class VirtualPeopleVidAssigner(private val labeler: Labeler) : VidAssigner {
  ...
  companion object {
    /**
     * Builds a [VirtualPeopleVidAssigner] from a compiled VID model blob.
     *
     ...
     * overload rather than parsed as a single root node.
     */
    fun fromCompiledNodeBlob(modelBlob: ByteString): VirtualPeopleVidAssigner {
      val nodes: List<CompiledNode> =
        Riegeli.readRecords(modelBlob.newInput())
          ...
          .toList()
      return VirtualPeopleVidAssigner(Labeler.build(nodes))
    }
  }
}
 ...
 *
 * @param loadAssigner builds a [VidAssigner] for a model blob URI — typically reads the
 *   Riegeli-encoded `CompiledNode` list from storage and calls
