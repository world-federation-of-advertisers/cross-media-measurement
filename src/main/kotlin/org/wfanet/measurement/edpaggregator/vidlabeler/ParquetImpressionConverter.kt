...
class ParquetImpressionConverter(
  ...
  override fun convert(
    ...
    // Left as a built Message: PopulationAttributeWriter sets the assigned VID's population
    // attributes on its builder and packs once, so the event is serialized a single time per
    // impression rather than packed here and re-parsed there.
    val eventMessage = mappers.messageMapper.project(event.row)
