...
class VidLabelingSink(
    ...
    for (digestedEvent in events) {
      for (context in modelLineContexts) {
        ...
        val eventTimestamp = timestamp {
          ...
          nanos =
            (Math.floorMod(converted.eventTimeMicros, MICROS_PER_SECOND) * NANOS_PER_MICRO).toInt()
        }
  ...
  private suspend fun writeGroup(key: OutputGroupKey, impressions: List<LabeledImpression>) {
    ...
    logger.info(
      "Wrote ${impressions.size} labeled impressions for ${key.modelLine} to $outputBlobUri"
    )
  }
