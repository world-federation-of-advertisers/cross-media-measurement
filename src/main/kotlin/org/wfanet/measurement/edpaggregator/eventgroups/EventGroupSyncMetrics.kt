 ...
 *
 * Tracks sync operations, success rates, and latency for syncing event groups
 * between the EDP and CMMS.
 *
 * Attributes:
...
class EventGroupSyncMetrics(meter: Meter) {
   ...
   *
   * Incremented when an event group sync fails due to validation errors,
   ...
   *
   * Records the time taken to complete a single event group sync operation,
   * from validation through Kingdom API call completion.
   */
