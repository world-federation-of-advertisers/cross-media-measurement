...
class EventGroupSync(
  ...
  private fun metricAttributes() = Attributes.of(DATA_PROVIDER_NAME_KEY, edpName)

  suspend fun sync(): Flow<MappedEventGroup> = flow {
    inEventGroupSyncSpan {
  ...
  private suspend fun syncEventGroupItem(
    ...
    return try {
      inEventGroupSyncItemSpan(eventGroupRefId) {
  ...
  private suspend fun <T> inEventGroupSyncSpan(block: suspend () -> T): T =
    withSpan(
      tracer,
      SPAN_EVENT_GROUP_SYNC,
      Attributes.of(DATA_PROVIDER_NAME_KEY, edpName),
      errorMessage = "EventGroupSync failed",
    ) { _ ->
      block()
    }

  private suspend fun <T> inEventGroupSyncItemSpan(
    eventGroupReferenceId: String,
    block: suspend () -> T,
  ): T =
    withSpan(
      tracer,
      SPAN_EVENT_GROUP_SYNC_ITEM,
      Attributes.of(
        EVENT_GROUP_REFERENCE_ID_KEY,
        eventGroupReferenceId,
        DATA_PROVIDER_NAME_KEY,
        edpName,
      ),
  ...
  companion object {
    ...
    private val DATA_PROVIDER_NAME_KEY = AttributeKey.stringKey("data_provider_name")
    private val EVENT_GROUP_REFERENCE_ID_KEY = AttributeKey.stringKey("event_group_reference_id")

    private const val SPAN_EVENT_GROUP_SYNC = "EventGroupSync"
    private const val SPAN_EVENT_GROUP_SYNC_ITEM = "EventGroupSync.Item"
