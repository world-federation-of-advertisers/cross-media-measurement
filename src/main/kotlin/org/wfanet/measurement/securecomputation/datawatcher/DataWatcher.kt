          ...
          val sinkType = config.sinkConfigCase.name
            Tracing.trace(
              ...
              val processingDuration =
                blobCreateTime?.let {
                  val deltaMillis = Instant.now().toEpochMilli() - it.toEpochMilli()
                  deltaMillis.coerceAtLeast(0).toDouble() / 1000.0
                }
                  ?: (processingStartTime.elapsedNow().inWholeMilliseconds / 1000.0)
              metrics.recordMatchLatency(
                sinkType = sinkType,
                latencySeconds = processingDuration,
              )
          }
        } else {
