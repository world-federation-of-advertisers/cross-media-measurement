...
class FrequencyVectorBuilder(
  ...
  init {
    ...
    if (overrideImpressionMaxFrequencyPerUser != null) {
      require(overrideImpressionMaxFrequencyPerUser >= 1 || overrideImpressionMaxFrequencyPerUser == -1) {
        "overrideImpressionMaxFrequencyPerUser must be >= 1 or -1 (uncapped)"
      }
    }

    maxFrequency =
      if (measurementSpec.hasImpression() && overrideImpressionMaxFrequencyPerUser != null && overrideImpressionMaxFrequencyPerUser != -1) {
