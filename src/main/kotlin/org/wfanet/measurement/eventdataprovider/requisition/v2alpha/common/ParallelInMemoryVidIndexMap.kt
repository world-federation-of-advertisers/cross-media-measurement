  ...
  companion object {
    ...
    fun build(
      ...
      return buildInternal(
        populationSpec,
        VidIndexMapCommon::hashVidToLongWithFarmHash,
        partitionCount,
      )
    }
    ...
    fun generateHashes(
      ...
      val vids = VidIndexMapCommon.collectVids(populationSpec)
      if (vids.isEmpty()) {
        return emptyArray()
      }
      ...
      applyPartitioned(vids.size, boundedPartitionCount) { partition ->
        for (i in partition.startIndex until partition.endIndexExclusive) {
          val vidInt = vids[i]
          val hash = hashFunction(vidInt.toLong(), VidIndexMapCommon.HASH_SALT)
