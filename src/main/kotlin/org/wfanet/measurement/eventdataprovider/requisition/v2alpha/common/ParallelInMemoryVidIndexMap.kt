 ...
 * workers while storing results in a FastUtil primitive map.
 *
...
private constructor(
  ...
  companion object {
    ...
    /** Computes VID/hash pairs using parallel partitions. */
    @VisibleForTesting
    fun generateHashes(
      populationSpec: PopulationSpec,
      hashFunction: (Long, ByteString) -> Long,
    ...
    /** Executes [task] in parallel for each partition of `[0, totalElements)`. */
    fun applyPartitioned(
      totalElements: Int,
      desiredPartitions: Int,
      task: (PartitionBounds) -> Unit,
    ) {
      if (totalElements == 0) {
        return
      }
      ...
      partitions.parallelStream().forEach { bounds -> task(bounds) }
    }
