...
class RawImpressionsWriter(
   ...
   * under `<blobKeyPrefix>/<event_date>/<blobKeyId>`, and returns the blob keys written (in date
   * order) so the caller can register them as `RawImpressionUploadFile`s.
   *
  ...
  companion object {
    ...
    // Footer key-value metadata key the reader requires; must equal
    // org.wfanet.measurement.edpaggregator.vidlabeler.FileEntityKeys.EVENT_DATE_KEY.
