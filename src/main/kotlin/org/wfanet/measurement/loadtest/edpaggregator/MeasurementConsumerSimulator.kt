...
class MeasurementConsumerSimulator(
        ...
        .flatMap { (syntheticEventGroupSpec, expression, collectionInterval) ->
          runBlocking {
            ...
            SyntheticDataGeneration.generateEvents(
              ...
              .filter { impression ->
                impression.eventTime.toInstant() >= collectionInterval.startTime.toInstant() &&
                  impression.eventTime.toInstant() < collectionInterval.endTime.toInstant()
              }
          }
        }
        .map { it.vid }
  ...
  private suspend fun executeReachOnly(
    runId: String,
    requiredCapabilities: DataProvider.Capabilities =
      DataProvider.Capabilities.getDefaultInstance(),
    vidSamplingInterval: VidSamplingInterval = DEFAULT_VID_SAMPLING_INTERVAL,
  ...
  private suspend fun createMeasurement(
    ...
    val eventGroups: List<EventGroup> =
      listEventGroups(measurementConsumer.name)
        .filter { it.eventGroupReferenceId in syntheticEventGroupMap.keys }
        .toList()
                  ...
                  collectionInterval = interval {
                    ...
                    val durationMillis =
                      Duration.between(
                        ...
                        .toMillis() * percentage
                    logger.info("START TIME: ${eventGroup.dataAvailabilityInterval.startTime}")
                    logger.info("END TIME ${(eventGroup.dataAvailabilityInterval.startTime
                      .toInstant()
  ...
  companion object {
    ...
    private const val CONFIDENCE_INTERVAL_MULTIPLIER = 6.0
