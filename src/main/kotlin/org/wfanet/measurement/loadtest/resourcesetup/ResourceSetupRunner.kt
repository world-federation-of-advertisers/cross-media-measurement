...
@CommandLine.Command(
  name = "run_resource_setup_job",
  mixinStandardHelpOptions = true,
  showDefaultValues = true
)
private fun run(@CommandLine.Mixin flags: ResourceSetupFlag) {
  val clientCerts =
  ...
  val v2alphaPublicApiChannel: ManagedChannel =
    buildMutualTlsChannel(
      flags.kingdomV2alphaPublicApiFlags.target,
      clientCerts,
      flags.kingdomV2alphaPublicApiFlags.certHost
    )
  ...
  runBlocking {
    ...
    ResourceSetupImpl(inMemoryKeyStore, dataProvidersStub, measurementConsumersStub, runId)
