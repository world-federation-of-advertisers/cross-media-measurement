...
import org.wfanet.measurement.api.v2alpha.ListModelRolloutsRequestKt.filter as modelRolloutsRequestFilter
...
class PdpSimulator(
  ...
  private suspend fun getEventGroupByReferenceId(eventGroupReferenceId: String): EventGroup? {
    ...
    check(response.nextPageToken.isEmpty()) {
      "Too many EventGroups for ${pdpData.name} and $measurementConsumerName"
    }
  ...
  suspend fun executeRequisitionFulfillingWorkflow() {
    ...
    for (requisition in requisitions) {
      try {
        ...
        val directProtocolConfig = requireNotNull(
          requisition.protocolConfig.protocolsList.first().direct
        ) {
          "Requisition must have a direct protocol config."
        }
        ...
        fulfillPopulationMeasurement(requisition, requisitionSpec, measurementSpec,
          populationBucketsList, modelRelease, requisitionFilterExpression, directProtocol)
  ...
  private suspend fun getModelRelease(
      ...
      try {
        modelRolloutsStub.listModelRollouts(
          listModelRolloutsRequest {
            parent = measurementSpecModelLineName
          }
        )
      } catch (e: StatusException) {
        throw when (e.status.code) {
          Status.Code.NOT_FOUND -> InvalidSpecException("ModelLine $measurementSpecModelLineName not found", e)
    ...
    return try {
        modelReleasesStub.getModelRelease(
  ...
  private suspend fun fulfillPopulationMeasurement(
    ...
    val populationSum = getTotalPopulation(
      requisitionSpec,
  ...
  private suspend fun getTotalPopulation(
    requisitionSpec: RequisitionSpec,
    populationBucketsList: List<PopulationBucket>,
    modelRelease: ModelRelease,
    filterExpression: String,
    ...
    val validPopulationBucketsList = populationBucketsList.filter { it.modelReleasesList.contains(modelRelease.name)
      // find better way to do this
      && Timestamp.parseFrom(it.validStartTime.toByteString()).seconds >= requisitionIntervalStartTime.seconds
      && Timestamp.parseFrom(it.validEndTime.toByteString()).seconds >= requisitionIntervalEndTime.seconds
    }.distinct()
  ...
  /**
   * Calculate direct reach and frequency for measurement with single PDP by summing up VIDs
    ...
    return when (measurementSpec.measurementTypeCase) {
      ...
      MeasurementSpec.MeasurementTypeCase.DURATION -> {
        val externalDataProviderId =
          apiIdToExternalId(DataProviderKey.fromName(pdpData.name)!!.dataProviderId)
        MeasurementKt.result {
  ...
  companion object {
    ...
    private fun getEventGroupReferenceIdPrefix(pdpDisplayName: String): String {
      return "$SIMULATOR_EVENT_GROUP_REFERENCE_ID_PREFIX-${pdpDisplayName}"
    }

    fun getEventGroupReferenceIdSuffix(eventGroup: EventGroup, pdpDisplayName: String): String {
      val prefix = getEventGroupReferenceIdPrefix(pdpDisplayName)
