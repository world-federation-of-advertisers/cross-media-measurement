...
class BigQueryEventQuery(
  ...
  override fun getUserVirtualIds(
    ...
    val resultJob: Job? = queryJob.waitFor()
    if (resultJob == null || resultJob.status.error != null) {
      throw Exception("Error running query.")
    }
    ...
    return testEvent {
      ...
      videoAd = testVideoTemplate {
        if (ageGroup == Person.AgeGroup.YEARS_18_TO_34) {
          age =
            TestVideoTemplateKt.ageRange { value = TestVideoTemplate.AgeRange.Value.AGE_18_TO_34 }
        }
      }
    }
  }
