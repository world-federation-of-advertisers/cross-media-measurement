...
class ReadEventGroups private constructor() : Runnable {
  ...
  private fun list(
    ...
    runBlocking {
      ...
      // Print stats even if SIGTERM is sent to terminate the process early.
      val statCollector = StatCollector()
      Runtime.getRuntime()
        .addShutdownHook(
          object : Thread() {
            override fun run() {
              statCollector.print(System.err)
            }
          }
        )
      latencies.collect { statCollector.recordLatency(it) }
    }
  }
  ...
  private class StatCollector {
    ...
    fun recordLatency(latency: kotlin.time.Duration) {
      latenciesMillis.add(latency.inWholeMilliseconds.toDouble())
    }
  ...
  private fun PrintStream.printStats(latenciesMillis: Collection<Long>) {
    val stats = Stats.of(latenciesMillis)
    val mean: kotlin.time.Duration = stats.mean().milliseconds
    val standardDeviation: kotlin.time.Duration = stats.populationStandardDeviation().milliseconds
    val p50: kotlin.time.Duration =
