...
class ReadEventGroups private constructor() : Runnable {
      ...
      val latencies = flow {
        while (coroutineContext.isActive) {
          val request = listEventGroupsRequest {
            ...
            if (legacy) {
              filter = mediaTypes.map { "${it.number} in media_types" }.joinToString(" || ")
            } else {
              orderBy =
              ...
              structuredFilter =
                ListEventGroupsRequestKt.filter { mediaTypesIntersect += mediaTypes }
            }
          }
  ...
  private fun outputStats(latenciesMillis: Collection<Long>) {
    ...
    println("Mean: $mean")
    println("Variance: ${stats.populationVariance()}(ms)^2")
