 ...
 *
 * The clever design allows MP to decrypt event data after matching by:
...
class PreprocessSourceEventsTask(private val cipher: DeterministicCommutativeCipher) :
  ...
  companion object {
    ...
    private const val GCM_IV_LENGTH = 12 // 96 bits as recommended
  ...
  private suspend fun processEventsAsStream(
    ...
    withContext(Dispatchers.IO) {
      ...
      val rawData = rawEventsBlob.toByteString()
      val request = PreprocessEventsRequest.parseFrom(rawData.toByteArray())
      ...
      events.chunked(CIPHER_BATCH_SIZE).forEach { batch ->
        ...
        val validEvents =
          mutableListOf<org.wfanet.panelmatch.client.eventpreprocessing.UnprocessedEvent>()
        val plaintextKeys = mutableListOf<ByteString>()

        batch.forEach { event ->
          val plaintextKey = event.id.toStringUtf8()
          val eventDataBytes = event.data.toByteArray()

          if (plaintextKey.isNotBlank() && eventDataBytes.isNotEmpty()) {
            validEvents.add(event)
            plaintextKeys.add(ByteString.copyFromUtf8(plaintextKey))
          }
        }

        if (plaintextKeys.isEmpty()) return@forEach

        // Step 2: Batch encrypt all keys with single JNI call (1000x speedup)
        ...
        validEvents.zip(encryptedKeys).forEach { (event, encryptedKey) ->
          val encryptedKeyBytes = encryptedKey.toByteArray()
          val eventDataBytes = event.data.toByteArray()

          // Encrypt event data using AES-GCM
          val encryptedEventData = encryptEventData(eventDataBytes, encryptedKeyBytes)

          // Create and emit encrypted event
          val encryptedEvent = encryptedMatchedEvent {
            encryptedJoinKey = ByteString.copyFrom(encryptedKeyBytes)
  ...
  /**
   * Encrypt event data using AES-GCM with the encrypted key as the AES key source.
   *
   * Security design:
   * - Each event gets a unique IV for semantic security
   * - AES key is derived from the encrypted join key using SHA-256
   * - GCM provides authentication to detect tampering
   * - Only parties with the matching encrypted key can decrypt
   *
   * @param eventData The raw event data bytes to encrypt
   * @param encryptedKey The raw encrypted key bytes to use for AES key derivation
   * @return Encrypted data in format: IV (12 bytes) + ciphertext + auth tag (16 bytes)
   */
  private fun encryptEventData(eventData: ByteArray, encryptedKey: ByteArray): ByteArray {
    // Derive AES key from encrypted join key
    val aesKey = deriveAesKey(encryptedKey)
    ...
    // Encrypt the data
    val ciphertext = cipher.doFinal(eventData)
  ...
  /**
   * Derive a fixed-size AES key from the encrypted join key.
   *
   * Uses SHA-256 to:
   * - Ensure consistent 256-bit key regardless of input size
   * - Provide one-way derivation (cannot recover encrypted key from AES key)
   * - Mix the entropy across all key bits
   *
   * @param encryptedKey The encrypted join key bytes
   * @return 256-bit AES key
   */
  private fun deriveAesKey(encryptedKey: ByteArray): ByteArray {
    val digest = MessageDigest.getInstance("SHA-256")
    return digest.digest(encryptedKey)
  }
