...
import org.wfanet.panelmatch.common.sha256
...
internal constructor(
  ...
  private suspend fun processJoinKeys(blob: StorageClient.Blob): BigQueryStreamingStatus {
    ...
    // Filter out entries with empty keys or empty IDs
    ...
    // Validate that all keys and identifiers are distinct in the filtered list
    ...
    // Process in chunks using the filtered list
    for (batch in validJoinKeyAndIds.chunked(BATCH_SIZE)) {
      val bigQueryRows =
        batch.map { joinKeyAndId ->
          // Hash the G(Key) to get H(G(Key)) for joining in authorized view workflow
          val hashedJoinKey = joinKeyAndId.joinKey.key.sha256()

          // Convert JoinKeyAndId to BigQuery row with hashed key
          val row =
            mapOf(
              keyColumnName to hashedJoinKey.toBase64(), // Efficient Base64 encoding
              dateColumnName to exchangeDate.toIsoDateString(),
            )
          val insertId = row.generateInsertId()
          insertId to row
        }
  ...
  private suspend fun processEncryptedEvents(blob: StorageClient.Blob): BigQueryStreamingStatus {
    ...
    // Read, parse, and filter delimited EncryptedMatchedEvent protos lazily
    ...
    for (batch in encryptedEventsSequence.chunked(BATCH_SIZE)) {
      ...
      val bigQueryRows =
        batch.map { encryptedEvent ->
          ...
          val row =
            mapOf(
              keyColumnName to encryptedEvent.encryptedJoinKey.toBase64(),
  ...
  companion object {
    ...
    private fun Map<String, Any>.generateInsertId(): String {
      val contentHash = this.hashCode()
      return "${System.currentTimeMillis()}_${contentHash}_${UUID.randomUUID()}"
    }
  ...
  /** private data class for tracking streaming statistics. */
