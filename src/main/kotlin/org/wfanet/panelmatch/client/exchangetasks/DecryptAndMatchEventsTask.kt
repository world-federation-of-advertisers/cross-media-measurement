 ...
 *
 * Reads encrypted events containing H(G(Key)), maps them to G(Key) and plaintext keys using
 * JoinKeyAndIdCollections, decrypts event data using AES-GCM, and outputs
 * KeyedDecryptedEventDataSet.
 *
 * Security: Events contain H(G(Key)) for joining, but decryption requires G(Key). Only parties with
 * G(Key) can decrypt.
 */
class DecryptAndMatchEventsTask : ExchangeTask {
  ...
  private suspend fun processEncryptedEvents(
    ...
    for (encryptedEvent in encryptedEvents) {
      ...
      val decryptedData =
        AesGcmCryptoHelper.decrypt(encryptedEvent.encryptedEventData, keyEntry.gKey)
      val output = keyedDecryptedEventDataSet {
        plaintextJoinKeyAndId = keyEntry.plaintextJoinKeyAndId
        decryptedEventData += plaintext { payload = decryptedData }
      }
