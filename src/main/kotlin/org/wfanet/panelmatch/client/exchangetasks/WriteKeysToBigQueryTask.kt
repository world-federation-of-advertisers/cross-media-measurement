...
/**
 * Writes JoinKeyAndId protos to BigQuery keys table using the streaming API.
 *
 * This task:
 * 1. Reads a stream of length-delimited JoinKeyAndId protos
 * 2. Converts each proto to a BigQuery row with encrypted_join_key and exchange_date
 * 3. Batches rows and writes them using BigQuery's streaming insertAll API
 * 4. Returns a status message upon completion
 *
 * The task is designed for the BigQuery authorized view workflow where encrypted keys need to be
 * stored in BigQuery for efficient JOINs with encrypted events.
 *
 * Input: Stream of JoinKeyAndId protos (from StreamJoinKeysTask) Output: Status message indicating
 * success/failure
 */
class WriteKeysToBigQueryTask(
  ...
  companion object {
    private val logger = Logger.getLogger(WriteKeysToBigQueryTask::class.java.name)
  ...
  override suspend fun execute(
    ...
    withContext(Dispatchers.IO) {
      logger.info("Starting WriteKeysToBigQueryTask for $projectId.$datasetId.$tableId")

      val inputBlob = requireNotNull(input[INPUT_LABEL]) { "Missing required input: $INPUT_LABEL" }

      val stats = processJoinKeys(inputBlob)
  ...
  /** Process a stream of JoinKeyAndId protos and write them to BigQuery. */
  ...
  private fun buildStatusMessage(stats: StreamingStats): String {
    return buildString {
      appendLine("BigQuery Streaming Complete for Keys Table")
