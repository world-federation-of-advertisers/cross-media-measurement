...
class KingdomlessApiClient(
   ...
   * return a [DatedExchangeWorkflow]. For each recurring exchange ID, this iterator returns one
   * dated workflow for every date beginning from today's date and walking back until the first
   * exchange date (but no further than [lookbackWindow] days ago).
   *
      ...
      sequence {
          ...
          while (true) {
            for (recurringExchangeId in recurringExchangeIds) {
              var date = clock.instant().toLocalDate()
              val minDate = clock.instant().minus(lookbackWindow).toLocalDate()
              while (date >= minDate) {
                yield(ExchangeDateKey(recurringExchangeId, date))
