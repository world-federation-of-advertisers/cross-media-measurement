...
class KingdomlessApiClient(
  ...
  private inner class CheckpointManager(private val datedWorkflow: DatedExchangeWorkflow) {
    ...
    suspend fun loadCheckpoints(
      ...
      dataProviderCheckpoint =
        retry(retryAttempts, retryBackoff) {
  ...
  /**
   * Executes [block] and returns the result. If [block] throws an exception, the exception is
   * suppressed and [block] is retried according to the specified [attempts] and [backoff]. If
   * [block] does not succeed after retrying, the last exception is rethrown.
   */
