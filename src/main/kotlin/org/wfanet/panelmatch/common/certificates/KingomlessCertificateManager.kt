...
class KingdomlessCertificateManager(
  ...
  override suspend fun createForExchange(exchange: ExchangeDateKey): String {
    ...
    val certName = CertificateKey(identity.id, clock.instant()).certName
  ...
  private data class CertificateKey(val ownerId: String, val createTime: Instant) {

    val certName: String = listOf(ownerId, createTime.epochSecond).joinToString(SEPARATOR)

    companion object {

      private const val OWNER_ID_INDEX = 1
      private const val CREATE_TIME_INDEX = 2
      ...
      fun fromCertName(certName: String): CertificateKey? {
        ...
        return CertificateKey(ownerId, createTime)
      }
    }
  }
