...
def get_report_summary_v2_from_report_result(
  ...
  """
    ...
    for group_key, results_for_group in grouped_results.items():
        ...
        # Creates a descriptive string for each term in the group key (e.g.,
        # "path=value").
        ...
        for reporting_set_result_entry in results_for_group:
            ...
            match reporting_set_result_key.WhichOneof(
                ...
                case "external_impression_qualification_filter_id":
                    report_summary_set_result.impression_filter = (
                        reporting_set_result_key.
                        external_impression_qualification_filter_id)
            ...
            elif (reporting_set_result_key.venn_diagram_region_type ==
            ...
            # Sorts the reporting window results by end dates.
            sorted_window_results = sorted(
                reporting_set_result_value.reporting_window_results,
                key=lambda window_entry: _proto_date_to_datetime(
            ...
            for window_entry in sorted_window_results:
                ...
                # Processes cumulative results.
                if noisy_values.HasField("cumulative_results"):
                    report_summary_window_result = (
                        report_summary_set_result.cumulative_results.add())
                    _copy_window_results(
                        # All cumulative results are weekly.
                ...
                # Processes non-cumulative results.
                if noisy_values.HasField("non_cumulative_results"):
                    metric_frequency_spec = reporting_set_result_key.metric_frequency_spec.WhichOneof(
                        "selector"
                    )
                    if metric_frequency_spec == "total":
                        whole_campaign_non_cumulative_results.append((
                            noisy_values.non_cumulative_results,
                            window_key.end,
                        ))
                    elif metric_frequency_spec == "weekly":
                        report_summary_window_result = (
                            report_summary_set_result.
                            non_cumulative_results.add())
                        _copy_window_results(
                            ReportSummaryWindowResult.MetricFrequencyType.WEEKLY,
                            noisy_values.non_cumulative_results,
                            report_summary_window_result,
            ...
            # Processes the deferred whole-campaign results last.
            for result, end_date in whole_campaign_non_cumulative_results:
                report_summary_window_result = (
                    report_summary_set_result.non_cumulative_results.add())
                _copy_window_results(
