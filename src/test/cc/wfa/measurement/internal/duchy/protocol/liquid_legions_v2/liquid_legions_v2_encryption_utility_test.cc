// Copyright 2020-2023 The Cross-Media Measurement Authors
//
...
class TestData {
 public:
  ...
  absl::StatusOr<MpcResult> GoThroughEntireMpcProtocol(
      ...
      NoiseMechanism noise_mechanism, int max_frequency) {
    ...
    *complete_execution_phase_one_at_aggregator_request
    ...
    complete_execution_phase_one_at_aggregator_request.set_noise_mechanism(
        noise_mechanism);
    ...
    *complete_execution_phase_two_request_1
    ...
    complete_execution_phase_two_request_1.set_noise_mechanism(noise_mechanism);
    ...
    *complete_execution_phase_two_request_2
    ...
    complete_execution_phase_two_request_2.set_noise_mechanism(noise_mechanism);
    ...
    *complete_execution_phase_two_at_aggregator_request
    ...
    complete_execution_phase_two_at_aggregator_request.set_noise_mechanism(
        noise_mechanism);
    ...
    *complete_execution_phase_three_at_aggregator_request
    ...
    complete_execution_phase_three_at_aggregator_request.set_noise_mechanism(
        noise_mechanism);
...
TEST(EndToEnd, SumOfCountsShouldBeCorrect) {
  ...
  ASSERT_OK_AND_ASSIGN(MpcResult result_with_geometric_noise,
                       test_data.GoThroughEntireMpcProtocol(
                           encrypted_sketch, /*reach_noise=*/nullptr,
                           /*frequency_noise=*/nullptr,
                           NoiseMechanism::GEOMETRIC, kMaxFrequency));
  ...
  ASSERT_OK_AND_ASSIGN(MpcResult result_with_gaussian_noise,
                       test_data.GoThroughEntireMpcProtocol(
                           encrypted_sketch, /*reach_noise=*/nullptr,
                           /*frequency_noise=*/nullptr,
                           NoiseMechanism::DISCRETE_GAUSSIAN, kMaxFrequency));
...
TEST(EndToEnd, SumOfCountsShouldBeCappedbyMaxFrequency) {
  ...
  EXPECT_NEAR(result_with_gaussian_noise.frequency_distribution[kMaxFrequency],
              0.5, 0.001);
}
...
TEST(ReachEstimation, NonDpNoiseShouldNotImpactTheResultWithMaxFrequencyOne) {
  ...
  *reach_noise_parameters.mutable_composite_el_gamal_public_key() =
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      test_data.GoThroughEntireMpcProtocol(
          encrypted_sketch, &reach_noise_parameters,
          /*frequency_noise=*/nullptr, NoiseMechanism::GEOMETRIC,
          /*max_frequency = */ 1));
  EXPECT_EQ(result_with_geometric_noise.reach, expected_reach);
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      test_data.GoThroughEntireMpcProtocol(
          encrypted_sketch, &reach_noise_parameters,
          /*frequency_noise=*/nullptr, NoiseMechanism::DISCRETE_GAUSSIAN,
          /*max_frequency = */ 1));
  EXPECT_EQ(result_with_gaussian_noise.reach, expected_reach);
}
...
TEST(FrequencyNoise, DeterministicNoiseShouldHaveNoImpact) {
  ...
  *frequency_noise_params.mutable_dp_params() =
  ...
  EXPECT_EQ(result_with_geometric_noise.frequency_distribution[2], 0.25);
  EXPECT_EQ(result_with_geometric_noise.frequency_distribution[3], 0.75);
  ...
  EXPECT_EQ(result_with_gaussian_noise.frequency_distribution[2], 0.25);
  EXPECT_EQ(result_with_gaussian_noise.frequency_distribution[3], 0.75);
}
...
TEST(FrequencyNoise, NonDeterministicNoiseShouldRandomizeTheResult) {
  ...
  *frequency_noise_params.mutable_dp_params() =
  ...
  EXPECT_NE(result_with_geometric_noise.frequency_distribution[2], 0.25);
  EXPECT_NE(result_with_geometric_noise.frequency_distribution[3], 0.75);

  double total_p = 0;
  for (auto x : result_with_geometric_noise.frequency_distribution) {
    total_p += x.second;
  }
  ...
  ASSERT_THAT(result_with_gaussian_noise.frequency_distribution,
              testing::Not(SizeIs(2)));
  EXPECT_NE(result_with_gaussian_noise.frequency_distribution[2], 0.25);
  EXPECT_NE(result_with_gaussian_noise.frequency_distribution[3], 0.75);

  total_p = 0;
  for (auto x : result_with_gaussian_noise.frequency_distribution) {
    total_p += x.second;
  }
  EXPECT_NEAR(total_p, 1, 0.0001);
}
...
TEST(EndToEnd, CombinedCasesWithDeterministicReachAndFrequencyDpNoises) {
  ...
  *frequency_noise_params.mutable_dp_params() =
  ...
  EXPECT_EQ(result_with_geometric_noise.reach, expected_reach);
  EXPECT_THAT(
      result_with_geometric_noise.frequency_distribution,
  ...
  EXPECT_EQ(result_with_gaussian_noise.reach, expected_reach);
  EXPECT_THAT(
      result_with_gaussian_noise.frequency_distribution,
      UnorderedElementsAre(Pair(3, DoubleNear(0.25, 0.001)),
                           Pair(6, DoubleNear(0.5, 0.001)),
                           Pair(kMaxFrequency, DoubleNear(0.25, 0.001))));
}
