...
class KingdomlessCertificateManagerTest {
  ...
  fun getCertificateUsesCertNameAsCacheKey() = runBlockingTest {
    val otherCertName = "some-other-cert"
    prepareRootCertificate(MODEL_PROVIDER_ID)
    prepareSharedStorageCertificate(EXCHANGE_DATE_KEY)
  ...
  companion object {
    ...
    private val STORED_CERTIFICATE by lazy {
      certificate {
        ownerId = MODEL_PROVIDER_ID
        x509Der = CERTIFICATE.encoded.toByteString()
      }
    }

    private val CURRENT_TIME = Instant.now()
    private val DATE = CURRENT_TIME.atZone(ZoneOffset.UTC).toLocalDate()
    ...
    private val CERT_NAME = "mp-certificate-${CURRENT_TIME.epochSecond}"
  }
}
