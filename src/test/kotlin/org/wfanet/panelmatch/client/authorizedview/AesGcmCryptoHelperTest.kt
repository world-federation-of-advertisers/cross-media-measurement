...
class AesGcmCryptoHelperTest {
  ...
  fun `encrypt with key longer than 32 bytes uses only first 32 bytes`() {
    ...
    val encryptedWithLong = AesGcmCryptoHelper.encrypt(plaintext, longKey)
    val encryptedWithTruncated = AesGcmCryptoHelper.encrypt(plaintext, truncatedKey)

    // Can decrypt with either the long key or the truncated key (since only first 32 bytes are
    // used)
    val decryptedWithLong = AesGcmCryptoHelper.decrypt(encryptedWithLong, longKey)
    val decryptedWithTruncated = AesGcmCryptoHelper.decrypt(encryptedWithLong, truncatedKey)
