...
import org.wfanet.panelmatch.client.exchangetasks.remote.aws.EmrServerlessClient
...
class RemoteRemoteTaskOrchestratorTest {

  @Mock
  private lateinit var mockStorageClient1: StorageClient

  @Mock
  private lateinit var emrServerlessClient1: EmrServerlessClient

  @Mock
  private lateinit var mockStorageClient2: StorageClient

  @Mock
  private lateinit var emrServerlessClient2: EmrServerlessClient
  ...
  fun setUp() {
    ...
    runBlocking {
      ...
      emrExchangeTaskRunner = EmrRemoteTaskOrchestrator(
  ...
  @Test
  fun testOrchestrateTaskFor() {
    runBlocking {
      whenever(emrServerlessClient1.startOrStopApplication(any(), any())).thenReturn(true)
      whenever(emrServerlessClient1.startAndWaitJobRunCompletion(any(), any(), any())).thenReturn(true)
      ...
      verify(emrServerlessClient1, times(2)).startOrStopApplication(any(), any())
  ...
  fun testOrchestrateTaskThrowsExceptionWhenStartApplicationFails() {
    runBlocking {
      whenever(emrServerlessClient1.startOrStopApplication(any(), eq(true))).thenReturn(false)
  ...
  fun testRunPanelExchangeStepOnEmrAppThrowsExceptionWhenRunApplicationFails() {
    runBlocking {
      whenever(emrServerlessClient1.startOrStopApplication(any(), any())).thenReturn(true)
      whenever(emrServerlessClient1.startAndWaitJobRunCompletion(any(),  any(), any())).thenReturn(false)
  ...
  fun testOrchestrateTaskThrowsExceptionWhenStopApplicationFails() {
    runBlocking {
      whenever(emrServerlessClient1.startOrStopApplication(any(), eq(true))).thenReturn(true)
      whenever(emrServerlessClient1.startAndWaitJobRunCompletion(any(),  any(), any())).thenReturn(true)
      whenever(emrServerlessClient1.startOrStopApplication(any(), eq(false))).thenReturn(false)
