...
class PreprocessEventsTaskTest {
  ...
  fun `successfully preprocesses single event`() {
    ...
    val decryptedBytes = AesGcmCryptoHelper.decrypt(
  ...
  fun `groups multiple events with same key into single output`() {
    ...
    val decryptedBytes = AesGcmCryptoHelper.decrypt(event.encryptedEventData, expectedEncryptedKey)
    val decryptedEventData = DecryptedEventData.parseFrom(decryptedBytes)
  ...
  fun `handles large batch with grouped events`() {
    ...
    val decryptedBytes = AesGcmCryptoHelper.decrypt(user1Event!!.encryptedEventData, user1Key)
    val decryptedEventData = DecryptedEventData.parseFrom(decryptedBytes)
  ...
  fun `skips events with empty data`() {
    ...
    val decryptedEventData = DecryptedEventData.parseFrom(decryptedBytes)
    assertThat(decryptedEventData.eventDataList).hasSize(1)
    assertThat(decryptedEventData.eventDataList[0]).isEqualTo("non-empty-data".toByteStringUtf8())
  }
  ...
  fun `mixed keys with some having multiple events`() {
    ...
    val user1Decrypted = DecryptedEventData.parseFrom(
      AesGcmCryptoHelper.decrypt(user1Event!!.encryptedEventData, user1Key)
    )
    ...
    val user2Decrypted = DecryptedEventData.parseFrom(
      AesGcmCryptoHelper.decrypt(user2Event!!.encryptedEventData, user2Key)
    )
    ...
    val user3Decrypted = DecryptedEventData.parseFrom(
      AesGcmCryptoHelper.decrypt(user3Event!!.encryptedEventData, user3Key)
    )
