...
class ReadEncryptedEventsFromBigQueryTaskTest {
  ...
  fun `task uses default column names when not specified`() = runBlockingTest {
    ...
    setupMockBigQueryResponse(
      ...
      eventDataColumnName = ReadEncryptedEventsFromBigQueryTask.DEFAULT_ENCRYPTED_EVENT_DATA_COLUMN_NAME,
    )
  ...
  @Test
  fun `validateSchema fails when join key column missing`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
    ...
    // When
    val result = task.execute(emptyMap())
    val events = result["encrypted-events"]!!.toList()
  ...
  @Test
  fun `validateSchema fails when event data column missing`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `validateSchema fails when join key column wrong type`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `validateSchema fails when event data column wrong type`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `parseRowToEncryptedMatchedEvent fails on null join key`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `parseRowToEncryptedMatchedEvent fails on null event data`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  fun `streamAsProto handles single page of results`() = runBlockingTest {
    ...
    setupMockBigQueryResponse(rows = rows, hasNextPage = false)
    ...
    // When hasNextPage is false, getNextPage should not be called
    verify(mockTableResult, times(0)).getNextPage()
    verify(mockTableResult, times(1)).hasNextPage()
  }

  @Test
  fun `streamAsProto handles multiple pages`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
    ...
    setupMockBigQueryResponseWithPages(listOf(rows1, rows2))
    ...
    events.forEachIndexed { index, event ->
      ...
      assertThat(event.encryptedJoinKey.toStringUtf8()).isEqualTo("key-$expectedIndex")
      assertThat(event.encryptedEventData.toStringUtf8()).isEqualTo("data-$expectedIndex")
    }
  }
  ...
  @Test
  fun `execute handles BigQuery query timeout`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `execute handles BigQuery permission errors`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `execute handles network errors`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
  ...
  @Test
  fun `handles large result sets efficiently`() = runBlockingTest {
    // Given
    val task =
      ReadEncryptedEventsFromBigQueryTask(
        projectId = PROJECT_ID,
    ...
    val pageSize = 1000
    val pages = mutableListOf<List<FieldValueList>>()

    for (pageNum in 0 until (numberOfRows / pageSize)) {
      val pageRows =
        ((pageNum * pageSize) until ((pageNum + 1) * pageSize)).map { i ->
          createMockRow(
            ByteString.copyFromUtf8("key-$i").toByteArray(),
            ByteString.copyFromUtf8("data-$i").toByteArray(),
          )
    ...
    setupMockBigQueryResponseWithPages(pages)
  ...
  private fun setupMockBigQueryResponse(
    ...
    // Setup table result
    whenever(mockTableResult.getSchema()).thenReturn(schema)
    whenever(mockTableResult.getValues()).thenReturn(rows)
    whenever(mockTableResult.hasNextPage()).thenReturn(hasNextPage)
    whenever(mockTableResult.getNextPage()).thenReturn(if (hasNextPage) mockTableResult else null)

    // Setup BigQuery
    whenever(mockBigQuery.query(any<QueryJobConfiguration>())).thenReturn(mockTableResult)
  }

  private fun setupMockBigQueryResponseWithPages(pages: List<List<FieldValueList>>) {
    // Setup schema
    val joinKeyField = Field.of("encrypted_join_key", StandardSQLTypeName.BYTES)
    val eventDataField = Field.of("encrypted_event_data", StandardSQLTypeName.BYTES)
    val schema = Schema.of(joinKeyField, eventDataField)

    // Create all TableResult mocks first
    val tableResults =
      pages.map { pageRows ->
        val pageResult: TableResult = mock()
        whenever(pageResult.getSchema()).thenReturn(schema)
        whenever(pageResult.getValues()).thenReturn(pageRows)
        pageResult
      }

    // Now chain them together with proper hasNextPage() setup
    tableResults.forEachIndexed { index, pageResult ->
      val hasNext = index < tableResults.size - 1
      val nextPage = if (hasNext) tableResults[index + 1] else null
      whenever(pageResult.hasNextPage()).thenReturn(hasNext)
      whenever(pageResult.getNextPage()).thenReturn(nextPage)
    }

    // Setup BigQuery to return first page
    whenever(mockBigQuery.query(any<QueryJobConfiguration>())).thenReturn(tableResults.first())
  }
