...
class DecryptAndMatchEventsTaskTest {
  ...
  private fun createTestEncryptedEventWithMultiplePayloads(
    ...
    val decryptedEventDataProto = decryptedEventData {
      plaintexts.forEach { eventData += it }
    }
    val encryptedEventData = AesGcmCryptoHelper.encrypt(decryptedEventDataProto.toByteString(), gKey)
  ...
  fun `execute handles mixed single and multiple event payloads`() = runBlockingTest {
    ...
    val allEvents = ByteString.copyFrom(
      listOf(
        encryptedEvent1.toDelimitedByteString(),
        encryptedEvent2.toDelimitedByteString()
      )
    )
