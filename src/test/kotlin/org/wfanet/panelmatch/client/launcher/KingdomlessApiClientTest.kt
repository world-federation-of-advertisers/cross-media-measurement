...
class KingdomlessApiClientTest {
  ...
  @Test
  fun claimExchangeStepReturnsStepsGoingBackToFirstExchangeDate() = runBlockingTest {
    val clock = FakeClock(startTime = CURRENT_TIME.plus(Duration.ofDays(3L)))
    val client = buildClient(RECURRING_EXCHANGE_ID_1 to WORKFLOW, clock = clock)

    assertThat(client.claimExchangeStep()?.exchangeDate).isEqualTo(CURRENT_DATE.plusDays(3L))
    assertThat(client.claimExchangeStep()?.exchangeDate).isEqualTo(CURRENT_DATE.plusDays(2L))
    assertThat(client.claimExchangeStep()?.exchangeDate).isEqualTo(CURRENT_DATE.plusDays(1L))
    assertThat(client.claimExchangeStep()?.exchangeDate).isEqualTo(CURRENT_DATE)
    assertThat(client.claimExchangeStep()?.exchangeDate).isNull()
  }

  @Test
  fun claimExchangeStepReturnsStepsGoingBackToLookbackWindowDaysAgo() = runBlockingTest {
