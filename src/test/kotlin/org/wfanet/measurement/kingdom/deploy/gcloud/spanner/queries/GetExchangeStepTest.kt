...
class GetExchangeStepTest : KingdomDatabaseTestBase() {
  ...
  private suspend fun readExchangeStepStructs(): List<Struct> =
    databaseClient
      ...
      .executeQuery(Statement.of("SELECT * FROM ExchangeSteps"))
      .toList()

  @Test
  fun `the Exchange Step with Data Provider exists`() = runBlocking {
    val exchangeStep =
      GetExchangeStepByDataProvider(
          ExternalId(EXTERNAL_DATA_PROVIDER_ID),
          listOf(ExchangeStep.State.READY, ExchangeStep.State.READY_FOR_RETRY)
        )
        .executeSingle(databaseClient.singleUse())
    val expected =
  ...
  fun `the Exchange Step with Model Provider exists`() = runBlocking {
    val exchangeStep =
      GetExchangeStepByModelProvider(
          ExternalId(EXTERNAL_MODEL_PROVIDER_ID),
          listOf(ExchangeStep.State.READY_FOR_RETRY)
        )
        .executeSingle(databaseClient.singleUse())
    val expected =
      ExchangeStep.newBuilder()
