...
class ModelShardsServiceTest {
  ...
  @Test
  fun `createModelShard returns NOT_FOUND with data provider name when data provider not found`() {
  ...
  @Test
  fun `createModelShard returns NOT_FOUND with model suite name when model suite not found`() {
    internalModelShardsMock.stub {
      onBlocking { createModelShard(any()) }
        .thenThrow(
    ...
    assertThat(exception.status.code).isEqualTo(Status.Code.NOT_FOUND)
    assertThat(exception.errorInfo?.metadataMap).containsEntry("model_suite", MODEL_SUITE_NAME)
  }

  @Test
  fun `createModelShard returns NOT_FOUND with model release name when model release not found`() {
    internalModelShardsMock.stub {
      onBlocking { createModelShard(any()) }
        .thenThrow(
    ...
    assertThat(exception.status.code).isEqualTo(Status.Code.NOT_FOUND)
    assertThat(exception.errorInfo?.metadataMap).containsEntry("model_release", MODEL_RELEASE_NAME)
  }

  @Test
  fun `deleteModelShard returns NOT_FOUND with data provider name when data provider not found`() {
    internalModelShardsMock.stub {
  ...
  @Test
  fun `deleteModelShard returns NOT_FOUND with model shard name when model shard not found`() {
    internalModelShardsMock.stub {
      onBlocking { deleteModelShard(any()) }
        .thenThrow(
    ...
    assertThat(exception.status.code).isEqualTo(Status.Code.NOT_FOUND)
    assertThat(exception.errorInfo?.metadataMap).containsEntry("model_shard", MODEL_SHARD_NAME)
  }

  @Test
  fun `deleteModelShard returns INVALID_ARGUMENT with model shard and provider name when argument invalid`() {
    internalModelShardsMock.stub {
      onBlocking { deleteModelShard(any()) }
        .thenThrow(
