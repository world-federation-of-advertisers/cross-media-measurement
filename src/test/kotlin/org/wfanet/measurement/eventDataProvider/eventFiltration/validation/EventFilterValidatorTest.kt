...
class EventFilterValidatorTest {
  private val TEMPLATE_PREFIX = "org.wfa.measurement.api.v2alpha.event_templates.testing"
  private val emptyEnv = { Env.newEnv() }
  ...
  private fun envWithTestTemplates(vararg vars: Decl): Env {
  ...
  private fun assertThatFailsWithCode(celExpression: String, code: Code, env: Env = emptyEnv()) {
    val e =
  ...
  fun `fails on inexistant template field`() {
    assertThatFailsWithCode(
      "bt.age.value == 1",
      Code.INVALID_CEL_EXPRESSION,
      envWithTestTemplates(bannerTemplateVar("bt"))
    )
  }

  @Test
  fun `can use valid template fields on comparison`() {
    EventFilterValidator.validate("vt.age.value == 1", envWithTestTemplates(videoTemplateVar("vt")))
  }
  ...
  fun `can use template with IN operator`() {
    EventFilterValidator.validate(
      "vt.age.value in [0, 1]",
      envWithTestTemplates(videoTemplateVar("vt"))
    )
  }
}
