...
class ReachAndFrequencyComputationsTest {
  ...
  @Test
  fun `computeFrequencyDistribution folds all users to freq 1 when higher buckets fail`() {
  ...
  fun `computeFrequencyDistribution with noise and small-cell suppression`() {
    ...
    val s = distribution.values.sum()
    assertThat(s == 0.0 || kotlin.math.abs(s - 1.0) < FLOAT_COMPARISON_TOLERANCE).isTrue()
  }
