...
class PrincipalAuthInterceptorTest {
  ...
  fun `sets verified token and scopes`() = runBlocking {
    ...
    serviceStub
      .withCallCredentials(openIdProvider.generateCredentials(principal.user.subject, scopes))
  ...
  fun `throws UNAUTHENTICATED when principal not found`() = runBlocking {
    ...
    val exception =
      assertFailsWith<StatusException> {
        serviceStub
          .withCallCredentials(
            openIdProvider.generateCredentials("user1@example.com", setOf("foo.bar", "foo.baz"))
          )
          .check(HealthCheckRequest.getDefaultInstance())
      }
  ...
  fun `throws UNAUTHENTICATED when token is invalid`() = runBlocking {
    val providerWithWrongJwks = OpenIdProvider(ISSUER, CLIENT_ID)
    val exception =
      assertFailsWith<StatusException> {
        serviceStub
          .withCallCredentials(
  ...
  companion object {
    ...
    private const val CLIENT_ID = "s6BhdRkqt3"
    private val openIdProvider = OpenIdProvider(ISSUER, CLIENT_ID)

    private val PROVIDERS_CONFIG = openIdProvidersConfig {
      providerConfigByIssuer[ISSUER] =
        OpenIdProvidersConfigKt.providerConfig {
          clientId = CLIENT_ID
