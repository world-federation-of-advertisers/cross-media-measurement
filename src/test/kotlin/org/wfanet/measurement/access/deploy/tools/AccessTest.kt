...
class AccessTest {
  private val permissionsServiceMock: PermissionsCoroutineImplBase = mockService {
    onBlocking { getPermission(any()) }.thenReturn(PERMISSION)
    onBlocking { listPermissions(any()) }
      .thenReturn(listPermissionsResponse { permissions += PERMISSION })
    onBlocking { checkPermissions(any()) }
      .thenReturn(checkPermissionsResponse { permissions += GET_BOOK_PERMISSION })
  }
  ...
  fun `permissions get calls GetPermission with valid request`() {
    val args = commonArgs + arrayOf("permissions", "get", GET_BOOK_PERMISSION)
    val output = callCli(args)
    ...
    assertThat(request).isEqualTo(getPermissionRequest { name = GET_BOOK_PERMISSION })
  ...
  fun `permissions list calls ListPermissions with valid request`() {
    ...
    assertThat(parseTextProto(output.reader(), ListPermissionsResponse.getDefaultInstance()))
      .isEqualTo(listPermissionsResponse { permissions += PERMISSION })
  }
  ...
  fun `permissions check calls CheckPermissions with valid request`() {
    ...
    assertThat(parseTextProto(output.reader(), CheckPermissionsResponse.getDefaultInstance()))
      .isEqualTo(checkPermissionsResponse { permissions += GET_BOOK_PERMISSION })
  }
  ...
  companion object {
    ...
    private const val WRITE_BOOK_PERMISSION = "permissisons/books.write"
    private val PERMISSION = permission {
      name = GET_BOOK_PERMISSION
      resourceTypes += SHELF_RESOURCE
    }
  }
}
