...
class EdpValidationPostProcessorTest {
  ...
  @Test
  fun `validate sends a deterministic name-based request id derived from the query`() {
    fakeCloudFunction.responseBody = resultResponse(10_000_000L)
  ...
  @Test
  fun `validate applies the default floor when minimum_impression_count is unset`() {
    // With minimum_impression_count unset, the post-processor falls back to the documented 1000
    // default, so a publisher count of 500 is below the floor and validation is SKIPPED rather than
    // passing the gate (which would happen if the unset proto default of 0 were used directly).
    val configs =
      configsFor(DATA_PROVIDER).copy {
        configs[0] =
          configs[0].copy { tolerance = tolerance.copy { clearMinimumImpressionCount() } }
      }
    fakeCloudFunction.responseBody = resultResponse(500L)

    val result = postProcessor(configs).validate(listOf(row(reportedImpressions = 500L)))

    assertThat(result).isEqualTo(EdpValidationPostProcessor.ReportValidationResult.SKIPPED)
  }

  @Test
  fun `validate honors an explicit minimum_impression_count of 0 as no floor`() {
    // An explicit 0 disables the floor (always validate), unlike unset. 500 publisher vs 500
    // reported is within tolerance, so the result is PASSED — proving the gate did not skip.
    val configs =
      configsFor(DATA_PROVIDER).copy {
        configs[0] = configs[0].copy { tolerance = tolerance.copy { minimumImpressionCount = 0 } }
      }
    fakeCloudFunction.responseBody = resultResponse(500L)
  ...
  @Test
  fun `metrics - zero expected count emits skipped_queries with no_expected_impressions`() {
    // Publisher count (2000) clears the 1000 floor, but a tiny VID sampling width rounds the scaled
    // expected count to zero, so the skip reason is distinct from the publisher-floor skip.
    fakeCloudFunction.responseBody = resultResponse(2000L)

    postProcessor().validate(listOf(row(reportedImpressions = 5L, vidSamplingWidth = 1e-9)))
...
private class FakeCloudFunction {
  ...
  /** Body bytes of the most recent request received. */
