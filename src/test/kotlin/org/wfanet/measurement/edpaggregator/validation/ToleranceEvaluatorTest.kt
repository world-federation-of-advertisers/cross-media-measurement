...
class ToleranceEvaluatorTest {
  ...
  @Test
  fun `evaluate throws when failure floor is below warning floor`() {
    val config = toleranceConfig {
      warning = toleranceBand {
        thresholdFraction = 0.02
        minimumAbsoluteDeviation = 500
      ...
      failure = toleranceBand {
        thresholdFraction = 0.10
        minimumAbsoluteDeviation = 100
      }
    }
    assertFailsWith<IllegalArgumentException> {
      ToleranceEvaluator.evaluate(reportedCount = 800, expectedCount = 1_000, config = config)
    }
  }
}
