...
class VidRankBuilderAppTest {
  ...
  /** Records every `BatchCreateVidLabelingJobs` request into [into], otherwise like the mock. */
  ...
  fun `runWork carries pass-through fields into the memoized VidLabelerParams template`() =
    runBlocking<Unit> {
      ...
      assertThat(labelerParams.memoizedParams.modelStorageParams.gcsProjectId)
