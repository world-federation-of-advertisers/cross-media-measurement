...
class EventGroupSyncTest {
  ...
  fun `records sync_attempts metric for all event groups`() {
    runBlocking {
      ...
      assertThat(firstPoint.attributes.get(AttributeKey.stringKey("data_provider_key")))
  ...
  fun `records sync_latency metric for each sync`() {
    runBlocking {
      ...
      assertThat(point.attributes.get(AttributeKey.stringKey("data_provider_key")))
  ...
  fun `creates EventGroupSync span with correct attributes`() {
    runBlocking {
      ...
      assertThat(syncSpan.attributes.get(AttributeKey.stringKey("data_provider_key")))
        .isEqualTo("dataProviders/test-edp-123")
      assertThat(syncSpan.attributes.get(AttributeKey.stringKey("source")))
        .isEqualTo("kingdom")
  ...
  fun `creates EventGroupSync Item span for each event group`() {
    runBlocking {
      ...
      itemSpans.forEach { span ->
        assertThat(span.attributes.get(AttributeKey.stringKey("data_provider_key")))
      ...
      val refIds = itemSpans.map {
        it.attributes.get(AttributeKey.stringKey("event_group_reference_id"))
      }.toSet()
  ...
  fun `metrics do not contain event_group_reference_id attribute`() {
    runBlocking {
      ...
      edpaMetrics.forEach { metric ->
        when (metric.name) {
          "edpa.event_group.sync_attempts", "edpa.event_group.sync_success" -> {
