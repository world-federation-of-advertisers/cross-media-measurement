...
class EventGroupSyncTest {
  ...
  private lateinit var testMetrics: EventGroupSyncMetrics
  ...
  fun initTelemetry() {
    ...
    testMetrics = EventGroupSyncMetrics(meterProvider.get("test"))
    openTelemetry =
  ...
  fun `records sync_attempts metric for all event groups`() {
    runBlocking {
      ...
      assertThat(syncAttemptsMetric!!.description)
        .isEqualTo("Number of Event Group sync attempts")
  ...
  fun `does not record success metrics on validation failure`() {
    runBlocking {
      ...
      assertThat(syncFailureMetric!!.description)
        .isEqualTo("Number of failed Event Group syncs")
  ...
  fun `creates EventGroupSync span with correct attributes`() {
    runBlocking {
      ...
      val spans = getSpans()
      val syncSpans = spans.filter { it.name == "edpa.event_group_sync" }
  ...
  fun `creates EventGroupSync Item span for each event group`() {
    runBlocking {
      ...
      val spans = getSpans()
      val itemSpans = spans.filter { it.name == "edpa.event_group_sync.item" }
  ...
  fun `sets ERROR status on Item span when event group sync fails`() {
    runBlocking {
      ...
      val exceptionEvents = itemSpan.events.filter {
        it.name == "exception"
      }
  ...
  fun `metrics do not contain event_group_reference_id attribute`() {
    runBlocking {
      ...
      edpaMetrics.forEach { metric ->
        when (metric.name) {
          "edpa.event_group.sync_attempts", "edpa.event_group.sync_success", "edpa.event_group.sync_failure" -> {
