...
private val INTERNAL_REQUISITION_METADATA: InternalRequisitionMetadata =
  internalRequisitionMetadata {
    ...
    cmmsRequisition = REQUISITION_METADATA_REQUEST.cmmsRequisition
    report = REQUISITION_METADATA_REQUEST.report
    blobUri = REQUISITION_METADATA_REQUEST.blobUri
    groupId = REQUISITION_METADATA_REQUEST.groupId
    cmmsCreateTime = REQUISITION_METADATA_REQUEST.cmmsCreateTime
...
class RequisitionMetadataServiceTest {
  ...
  fun `createRequisitionMetadata returns a RequisitionMetadata`() = runBlocking {
    ...
    val result = service.createRequisitionMetadata(request)

    assertThat(result.name).endsWith(REQUISITION_METADATA_ID.toString())
    assertThat(result.state).isEqualTo(RequisitionMetadata.State.STORED)
    ...
    verifyProtoArgument(
      ...
      .isEqualTo(
        internalCreateRequisitionMetadataRequest {
          ...
          requisitionMetadata =
            INTERNAL_REQUISITION_METADATA.toBuilder()
              .apply {
                clearRequestId()
                clearState()
              }
