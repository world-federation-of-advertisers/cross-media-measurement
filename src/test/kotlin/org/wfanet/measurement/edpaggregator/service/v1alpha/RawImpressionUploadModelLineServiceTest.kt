...
class RawImpressionUploadModelLineServiceTest {
  ...
  fun `createRawImpressionUploadModelLine throws INVALID_ARGUMENT for missing model line`() =
    runBlocking {
      val request = createRawImpressionUploadModelLineRequest { parent = UPLOAD_KEY.toName() }
  ...
  @Test
  fun `batchCreateRawImpressionUploadModelLines returns model lines successfully`() =
    runBlocking<Unit> {
      createParentUpload(DATA_PROVIDER_ID, RAW_IMPRESSION_UPLOAD_ID)
  ...
  fun `batchCreateRawImpressionUploadModelLines throws INVALID_ARGUMENT for empty requests`() =
    runBlocking {
      val request = batchCreateRawImpressionUploadModelLinesRequest { parent = UPLOAD_KEY.toName() }
  ...
  @Test
  fun `getRawImpressionUploadModelLine throws INVALID_ARGUMENT for malformed name`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        service.getRawImpressionUploadModelLine(
  ...
  @Test
  fun `listRawImpressionUploadModelLines returns model lines`() =
    runBlocking<Unit> {
      createParentUpload(DATA_PROVIDER_ID, RAW_IMPRESSION_UPLOAD_ID)
      val created =
        service.createRawImpressionUploadModelLine(
  ...
  fun `listRawImpressionUploadModelLines supports wildcard parent across uploads`() =
    runBlocking<Unit> {
      ...
      assertThat(response.rawImpressionUploadModelLinesList).containsExactly(created1, created2)
    }
  ...
  @Test
  fun `listRawImpressionUploadModelLines throws INVALID_ARGUMENT for empty parent`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        service.listRawImpressionUploadModelLines(listRawImpressionUploadModelLinesRequest {})
      }
    assertThat(exception.status.code).isEqualTo(Status.Code.INVALID_ARGUMENT)
    assertThat(exception.errorInfo)
      .isEqualTo(
  ...
  fun `markRawImpressionUploadModelLineFailed throws INVALID_ARGUMENT for empty etag`() =
    runBlocking {
      ...
      val exception =
        assertFailsWith<StatusRuntimeException> {
          service.markRawImpressionUploadModelLineFailed(
            markRawImpressionUploadModelLineFailedRequest { name = created.name }
          )
        }
      assertThat(exception.status.code).isEqualTo(Status.Code.INVALID_ARGUMENT)
    }
