...
class RequisitionFetcherTest {
  private val requisitionsServiceMock: RequisitionsGrpcKt.RequisitionsCoroutineImplBase =
    mockService {
      onBlocking { listRequisitions(any()) }.thenReturn(
  ...
  private fun createFetcher(
    ...
    metrics: RequisitionFetcherMetrics = RequisitionFetcherMetrics.instance,
  ): RequisitionFetcher {
  ...
  @Test
  fun `fetchAndStoreRequisitions stores multiple GroupedRequisitions with same groupId`() = runBlocking {
  ...
  @Test
  fun `fetchAndStoreRequisitions stores multiple GroupedRequisitions with different groupIds`() = runBlocking {
  ...
  fun `metrics - data_provider_name attribute present in metrics`() = runBlocking {
    ...
    assertThat(latencyMetric).isNotNull()
    val latencyPoint = latencyMetric!!.histogramData.points.first() as HistogramPointData
    assertThat(latencyPoint.attributes.asMap()).containsKey(
      io.opentelemetry.api.common.AttributeKey.stringKey("data_provider_name")
    )
  }
