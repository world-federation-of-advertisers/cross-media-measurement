...
class DirectResultNoisersTest {
  ...
  fun `deterministic noiser draws differ for a different frequency vector`() {
    fun reachDrawFor(frequencyData: IntArray) =
    ...
    assertThat(reachDrawFor(IntArray(100) { 1 })).isNotEqualTo(reachDrawFor(FREQUENCY_DATA))
  }
  ...
  fun `deterministic noiser stays within the truncation bound`() {
    ...
    // Reach draws at unit sensitivity, where the compiled params put the bound at 6.7571.
    val noised = noiser.noiseReach(1_000L)

    assertThat(noised).isAtLeast(1_000L - UNIT_SENSITIVITY_BOUND)
    assertThat(noised).isAtMost(1_000L + UNIT_SENSITIVITY_BOUND)
  }
  ...
  private fun build(directNoiseMechanism: DirectNoiseMechanism) =
    buildDirectResultNoiser(
