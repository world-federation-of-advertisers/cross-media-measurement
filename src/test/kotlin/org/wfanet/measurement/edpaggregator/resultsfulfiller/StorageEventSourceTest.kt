...
class StorageEventSourceTest {
  ...
  private fun createEventGroupDetails(eventGroupReferenceId: String, startDate: LocalDate, endDate: LocalDate, zoneId: ZoneId): GroupedRequisitions.EventGroupDetails {
    val startInstant = startDate.atStartOfDay(zoneId).toInstant()
    val endInstant = endDate.atStartOfDay(zoneId).toInstant()

    return GroupedRequisitions.EventGroupDetails.newBuilder().apply {
  ...
  fun `secondary constructor creates default path resolver internally`(): Unit = runBlocking {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("event-group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC"))
    )
  ...
  @Test
  fun `expandEventGroupDates throws exception when start and end date are equal`() {
    // Create an interval where start and end timestamps are the same
    val sameInstant = LocalDate.of(2025, 1, 15).atStartOfDay(ZoneId.of("UTC")).toInstant()
    val eventGroupDetailsList = listOf(
      GroupedRequisitions.EventGroupDetails.newBuilder().apply {
        eventGroupReferenceId = "group-1"
        addCollectionIntervals(
          interval {
            startTime = timestamp {
              seconds = sameInstant.epochSecond
              nanos = sameInstant.nano
            }
            endTime = timestamp {
              seconds = sameInstant.epochSecond + 1 // Add 1 second so after subtracting 1 it equals start
              nanos = sameInstant.nano
            }
          }
        )
    ...
    val exception = assertFailsWith<IllegalArgumentException> {
      StorageEventSource.expandEventGroupDates(eventGroupDetailsList, ZoneId.of("UTC"))
    }
    assertThat(exception.message).contains("Start date and end date are the same")
    assertThat(exception.message).contains("2025-01-15")
    assertThat(exception.message).contains("group-1")
  }

  @Test
  fun `expandEventGroupDates throws exception when start date is after end date`() {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 20), LocalDate.of(2025, 1, 15), ZoneId.of("UTC"))
    )

    val exception = assertFailsWith<IllegalArgumentException> {
      StorageEventSource.expandEventGroupDates(eventGroupDetailsList, ZoneId.of("UTC"))
    }
    assertThat(exception.message).contains("Start date")
    assertThat(exception.message).contains("after end date")
    assertThat(exception.message).contains("group-1")
  }
  ...
  fun `expandEventGroupDates expands multi-day interval`() {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC"))
    )
  ...
  @Test
  fun `expandEventGroupDates handles multiple event groups with valid intervals`() {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC")),
      createEventGroupDetails("group-2", LocalDate.of(2025, 1, 2), LocalDate.of(2025, 1, 4), ZoneId.of("UTC"))
    )

    val result = StorageEventSource.expandEventGroupDates(eventGroupDetailsList, ZoneId.of("UTC"))

    assertThat(result).hasSize(4) // group-1: Jan 1,2 + group-2: Jan 2,3 = 4 total
    assertThat(result.filter { it.eventGroupReferenceId == "group-1" }).hasSize(2)
    assertThat(result.filter { it.eventGroupReferenceId == "group-2" }).hasSize(2)
  }

  @Test
  fun `expandEventGroupDates throws exception for same-day intervals`() {
    // Test with end time at 3PM on the same day - should throw exception
    val eventGroupDetailsList = listOf(
  ...
  fun `generateEventBatches works with FakeEventReader`(): Unit = runBlocking {
    ...
    val eventGroupDetails = createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC"))
  ...
  fun `generateEventBatches assigns unique batch IDs`(): Unit = runBlocking {
    ...
    assertThat(batches).hasSize(2) // Jan 1-3 exclusive = two dates = two batches
  }
  ...
  fun `generateEventBatches uses path-specific EventReaders`(): Unit = runBlocking {
    val eventGroupDetails = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC")),
      createEventGroupDetails("group-2", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3), ZoneId.of("UTC"))
    )
