...
class StorageEventSourceTest {
  ...
  fun `generateEventBatches handles EventReader exception`(): Unit = runBlocking {
    ...
    // Note: Can't inject failing reader without factory pattern, but missing impression files will
    // cause failure
    ...
    assertFailsWith<Exception> { eventSource.generateEventBatches().toList() }
  }
