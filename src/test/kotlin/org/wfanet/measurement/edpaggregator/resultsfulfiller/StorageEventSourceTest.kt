...
class StorageEventSourceTest {
  ...
  fun `secondary constructor creates default path resolver internally`(): Unit = runBlocking {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("event-group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3))
    )
  ...
  fun `expandEventGroupDates throws exception when start and end date are equal`() {
    ...
    val eventGroupDetailsList = listOf(
      GroupedRequisitions.EventGroupDetails.newBuilder()
        .setEventGroupReferenceId("group-1")
        .addCollectionIntervals(
          interval {
            startTime = timestamp {
  ...
  fun `expandEventGroupDates throws exception when start date is after end date`() {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 20), LocalDate.of(2025, 1, 15))
    )
  ...
  fun `expandEventGroupDates handles multiple event groups with valid intervals`() {
    val eventGroupDetailsList = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3)),
      createEventGroupDetails("group-2", LocalDate.of(2025, 1, 2), LocalDate.of(2025, 1, 4))
    )
  ...
  fun `generateEventBatches works with FakeEventReader`(): Unit = runBlocking {
    ...
    val eventGroupDetails = createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3))
    ...
    assertThat(batches).hasSize(2) // Two dates (Jan 1 and Jan 2)
  ...
  fun `generateEventBatches handles multiple batches from single EventReader`(): Unit = runBlocking {
    ...
    // Each EventReader generates batches with sizes 3, 2, 1
    val batchSizes = batches.map { it.events.size }.sorted()
    assertThat(batchSizes).isEqualTo(listOf(1, 1, 2, 2, 3, 3))
  }
  ...
  fun `generateEventBatches uses path-specific EventReaders`(): Unit = runBlocking {
    val eventGroupDetails = listOf(
      createEventGroupDetails("group-1", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3)),
      createEventGroupDetails("group-2", LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 3))
    )
