...
class ResultsFulfillerTest {
  ...
  fun `fulfillRequisitions emits telemetry`() = runBlocking {
    ...
    val statusKey = AttributeKey.stringKey("status")
    ...
    val reportSpan =
      spans.first { it.name == "report_fulfillment" && it.attributes.size() > 0 }
    val reportIdAttr = AttributeKey.stringKey("report_id")
    val groupIdAttr = AttributeKey.stringKey("group_id")
    assertThat(reportSpan.attributes.get(reportIdAttr)).isEqualTo("reports/telemetry-report")
    val expectedGroupId = groupedRequisitions.groupId
    assertThat(reportSpan.attributes.get(groupIdAttr)).isEqualTo(expectedGroupId)
    ...
    val requisitionAttr = AttributeKey.stringKey("cmms_requisition")
    assertThat(requisitionSpan.attributes.get(requisitionAttr)).isEqualTo(REQUISITION_NAME)
    assertThat(requisitionSpan.attributes.get(reportIdAttr)).isEqualTo("reports/telemetry-report")
