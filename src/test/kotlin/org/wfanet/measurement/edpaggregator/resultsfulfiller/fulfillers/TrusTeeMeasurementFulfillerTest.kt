...
class TrusTeeMeasurementFulfillerTest {
  ...
  fun `fulfillRequisition with non-empty frequency vector with sufficient k-anonymity`() {
    runBlocking {
      ...
      assertThat(fulfilledRequisitions[0].header.hasTrusTee()).isTrue()
      val bodyData = fulfilledRequisitions[1].bodyChunk.data
      assertThat(bodyData.size()).isGreaterThan(0)
    }
  }
  ...
  fun `fulfillRequisition with empty frequency vector with insufficient k-anonymity`() {
    runBlocking {
      ...
      assertThat(fulfilledRequisitions[0].header.hasTrusTee()).isTrue()
      val bodyData = fulfilledRequisitions[1].bodyChunk.data
      assertThat(bodyData.size()).isEqualTo(EMPTY_FREQUENCY_VECTOR.dataCount)
    }
  }
