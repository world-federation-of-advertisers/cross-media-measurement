...
class StripedByteFrequencyVectorTest {
  ...
  @Test
  fun `mergeUnsafe combines two vectors correctly`() {
    val vector1 = StripedByteFrequencyVector(100)
    val vector2 = StripedByteFrequencyVector(100)
    
    // Add some counts to both vectors
    repeat(5) { vector1.incrementByIndex(10) }
    repeat(3) { vector2.incrementByIndex(10) }
    repeat(2) { vector1.incrementByIndex(20) }
    repeat(7) { vector2.incrementByIndex(20) }
    
    // Merge vector2 into vector1
    vector1.mergeUnsafe(vector2)
    
    val result = vector1.getArray()
    assertThat(result[10]).isEqualTo(8) // 5 + 3
    assertThat(result[20]).isEqualTo(9) // 2 + 7
    assertThat(result.filterIndexed { i, _ -> i != 10 && i != 20 }.all { it == 0 }).isTrue()
  }

  @Test
  fun `mergeUnsafe handles saturation correctly`() {
    val vector1 = StripedByteFrequencyVector(100)
    val vector2 = StripedByteFrequencyVector(100)
    
    // Fill vector1 to near capacity
    repeat(120) { vector1.incrementByIndex(5) }  // Should cap at 127
    repeat(50) { vector2.incrementByIndex(5) }   // Would overflow if added
    
    vector1.mergeUnsafe(vector2)
    
    val result = vector1.getArray()
    assertThat(result[5]).isEqualTo(127) // Should be saturated at max value
  }
