...
class EventProcessingIntegrationTest {
  ...
  fun `process multiple requisitions with different event groups`() = runBlocking {
    ...
    val freq1Sum = freq1!!.getArray().sum()
    ...
    val freq2Sum = freq2!!.getArray().sum()
  ...
  fun `process grouped requisitions with shared event groups`() = runBlocking {
    ...
    val eventSource = createEventSourceFromReader(eventReader, listOf(sharedEventGroup), testDate, testDate)
    val populationSpec = loadPopulationSpecFromFile(TEST_DATA_RUNTIME_PATH.toString(), true)
    val vidIndexMap = InMemoryVidIndexMap.build(populationSpec)
  ...
  fun `process single requisition with multiple event groups`() = runBlocking {
    ...
    val eventSource = createEventSourceFromReader(eventReader, listOf(eventGroup1, eventGroup2, eventGroup3), testDate, testDate)
    val populationSpec = loadPopulationSpecFromFile(TEST_DATA_RUNTIME_PATH.toString(), true)
    val vidIndexMap = InMemoryVidIndexMap.build(populationSpec)
    ...
    val allMalesFreq = result[multiGroupRequisition.name]!!.getArray().sum()
    val youngMalesFreq = result[youngMalesRequisition.name]!!.getArray().sum()
  ...
  fun `process requisitions with different measurement specs and VID sampling`() = runBlocking {
    ...
    // Verify VID sampling behavior
    assertThat(fullPopReach).isEqualTo(100L) // All VIDs

    // VID sampling should result in roughly the expected proportions
    // Allow some variance due to sampling algorithm implementation
    assertThat(firstHalfReach).isAtLeast(40L)  // ~50 VIDs
    assertThat(firstHalfReach).isAtMost(60L)
    assertThat(secondHalfReach).isAtLeast(40L) // ~50 VIDs
    assertThat(secondHalfReach).isAtMost(60L)
    assertThat(middleQuarterReach).isAtLeast(40L) // ~50 VIDs
    assertThat(middleQuarterReach).isAtMost(60L)
    assertThat(smallSampleReach).isAtLeast(5L) // ~10 VIDs
    assertThat(smallSampleReach).isAtMost(15L)

    // First half and second half should not significantly overlap
    // (this is a sampling property test)

    // Frequency cap test - reach should be independent of frequency cap for single-impression events
    assertThat(highFreqCapReach).isAtLeast(5L)
    assertThat(highFreqCapReach).isAtMost(15L)
  ...
  fun `process requisitions with complex CEL expressions`() = runBlocking {
    ...
    val ageGroups = listOf(Person.AgeGroup.YEARS_18_TO_34, Person.AgeGroup.YEARS_35_TO_54, Person.AgeGroup.YEARS_55_PLUS)
    val genders = listOf(Person.Gender.MALE, Person.Gender.FEMALE)
    val socialGrades = listOf(Person.SocialGradeGroup.A_B_C1, Person.SocialGradeGroup.SOCIAL_GRADE_GROUP_UNSPECIFIED)
    ...
    ageGroups.forEach { ageGroup ->
      genders.forEach { gender ->
        socialGrades.forEach { socialGrade ->
          repeat(5) {
            events.add(createLabeledImpressionWithSocialGrade(
              vid = vid++,
              ageGroup = ageGroup,
              gender = gender,
    ...
    val youngMalesOrSeniorFemalesReach = result["requisitions/young-males-or-senior-females"]!!.getReach()
  ...
  fun `process requisitions with empty and edge case results`() = runBlocking {
    ...
    assertThat(minimalWidthSamplingReach).isLessThan(3L) // Minimal width sampling

    // Verify minimal results
    assertThat(minimalSamplingReach).isLessThan(3L) // Very narrow sampling
    ...
    result.values.forEach { freq ->
      assertThat(freq.getArray().all { it >= 0 }).isTrue()
      assertThat(freq.getArray().sum() >= 0L).isTrue()
    }
  ...
  fun `process requisitions with overlapping time ranges across event groups`() = runBlocking {
    ...
    val eventSource = createEventSourceFromReader(eventReader, listOf(eventGroup1, eventGroup2, eventGroup3), day1, day3)
    val populationSpec = loadPopulationSpecFromFile(TEST_DATA_RUNTIME_PATH.toString(), true)
    val vidIndexMap = InMemoryVidIndexMap.build(populationSpec)
