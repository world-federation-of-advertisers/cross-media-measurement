...
class EventProcessingIntegrationTest {
  ...
  private lateinit var eventProcessingOrchestrator: EventProcessingOrchestrator<TestEvent>
  ...
  private class MultiGroupEventReader(private val readers: List<EventReader<TestEvent>>) :
    EventReader<TestEvent> {
    override suspend fun readEvents(): Flow<List<LabeledEvent<TestEvent>>> {
      return flow {
  ...
  private class TestEventSource(
    private val eventReader: EventReader<TestEvent>,
    ...
    override fun generateEventBatches(): Flow<EventBatch<TestEvent>> {
      return flow {
        ...
        eventReader.readEvents().collect { eventList: List<LabeledEvent<TestEvent>> ->
          if (eventList.isNotEmpty()) {
            ...
            emit(EventBatch<TestEvent>(eventList, minTime, maxTime))
          }
        }
      }
    }
  ...
  private suspend fun setupStorageWithEvents(
    ...
    val readers: List<EventReader<TestEvent>> =
