...
class FilterProcessorTest {
  ...
  private val typeRegistry: TypeRegistry = TypeRegistry.newBuilder()
    .add(TestEvent.getDescriptor())
    .build()

  /**
   * Helper function to create a DynamicMessage from a TestEvent.
   */
  ...
  fun `processBatch filters events with CEL expression`() { runBlocking {
    ...
    val filterProcessor = FilterProcessor(
      ...
      eventMessageDescriptor = testEventDescriptor,
    ...
    assertThat(result.map { it.vid }).containsExactly(1L, 3L)
  }}
  ...
  fun `processBatch with empty CEL expression matches all events`() { runBlocking {
    ...
    assertThat(result.map { it.vid }).containsExactly(1L, 2L, 3L)
  }}
  ...
  fun `processBatch filters by time range`() { runBlocking {
    ...
    assertThat(result[0].vid).isEqualTo(2L)
  }}
  ...
  fun `processBatch applies combined filters`() { runBlocking {
    ...
    assertThat(result).hasSize(1)
    assertThat(result[0].vid).isEqualTo(1L)
  }}
  ...
  fun `processBatch with complex CEL expression`() { runBlocking {
    ...
    assertThat(result).hasSize(2)
    assertThat(result.map { it.vid }).containsExactly(1L, 4L)
  }}
  ...
  fun `processBatch filters events at time range boundaries`() { runBlocking {
    ...
    assertThat(result).hasSize(3)
    assertThat(result.map { it.vid }).containsExactly(2L, 3L, 4L)
  }}
}