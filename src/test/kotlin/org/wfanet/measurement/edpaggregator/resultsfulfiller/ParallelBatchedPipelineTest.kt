...
class ParallelBatchedPipelineTest {
  ...
  private class TestEventSource(private val eventBatches: List<List<LabeledEvent<Message>>>) :
    EventSource {
  ...
  fun `processEventBatches processes empty flow successfully`() = runBlocking {
    val pipeline = ParallelBatchedPipeline(batchSize = 10, workers = 2)
  ...
  fun `processEventBatches with time interval filters`() = runBlocking {
    val pipeline = ParallelBatchedPipeline(batchSize = 5, workers = 1)
  ...
  fun `processEventBatches with different batch sizes`() = runBlocking {
    val smallBatchPipeline = ParallelBatchedPipeline(batchSize = 2, workers = 1)
    ...
    val events =
      (1..5).map { i ->
        LabeledEvent(
          ...
          message = testEvent { person = person { ageGroup = Person.AgeGroup.YEARS_18_TO_34 } } as Message,
          eventGroupReferenceId = "reference-id-1",
        )
      }
  ...
  fun `processEventBatches with multiple workers`() = runBlocking {
    val pipeline = ParallelBatchedPipeline(batchSize = 1, workers = 4)
  ...
  fun `multiple workers process batches concurrently`() = runBlocking {
    ...
    val events =
      (1..6).map { i ->
        LabeledEvent(
          timestamp = now,
          vid = i.toLong(),
          message = TestEvent.getDefaultInstance() as Message,
          eventGroupReferenceId = "reference-id-1",
        )
      }
    ...
    val baseSink = FrequencyVectorSink(
    ...
    val workerTrackingSink = spy(baseSink)
    doAnswer { invocation ->
      val batch = invocation.getArgument<EventBatch>(0)
      val workerName = Thread.currentThread().name
      batch.events.forEach { event ->
        workerProcessingLog.add("Worker $workerName processed VID ${event.vid}")
      }
      invocation.callRealMethod()
    }.`when`(workerTrackingSink).processBatch(any())

    val sinks = listOf(workerTrackingSink)
  ...
  fun `processEventBatches with multiple sinks processes all sinks per batch`() = runBlocking {
    ...
    doAnswer { invocation ->
      val batch = invocation.getArgument<EventBatch>(0)
      batch.events.forEach { event -> sink1Processed.add((event.vid / 10).toInt()) }
      invocation.callRealMethod()
    }.`when`(trackingSink1).processBatch(any())
    
    val trackingSink2 = spy(sink2)
    doAnswer { invocation ->
      val batch = invocation.getArgument<EventBatch>(0)
      batch.events.forEach { event -> sink2Processed.add((event.vid / 10).toInt()) }
      invocation.callRealMethod()
    }.`when`(trackingSink2).processBatch(any())
  ...
  fun `channel backpressure controls batch flow`() = runBlocking {
    ...
    doAnswer { invocation ->
      ...
      invocation.callRealMethod()
    }.`when`(slowProcessingSink).processBatch(any())
  ...
  fun `worker failure cancels pipeline and propagates exception`() = runBlocking {
    ...
    doAnswer { invocation ->
      ...
      invocation.callRealMethod()
    }.`when`(failingSink).processBatch(any())
