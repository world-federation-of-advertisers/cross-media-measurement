...
class ParallelBatchedPipelineTest {
  ...
  fun `processEventBatches processes empty flow successfully`() = runBlocking {
    ...
    val sinks =
      listOf(
        FrequencyVectorSink(
          frequencyVector = StripedByteFrequencyVector(realVidIndexMap.size.toInt()),
  ...
  fun `processEventBatches with time interval filters`() = runBlocking {
    ...
    val timeInterval =
      Interval.newBuilder()
        ...
        .setEndTime(
          com.google.protobuf.Timestamp.newBuilder().setSeconds(System.currentTimeMillis() / 1000)
        )
        .build()
