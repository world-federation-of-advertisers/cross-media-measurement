  ...
  fun `processEventBatches with StorageEventSource reads from encrypted storage files`() = runBlocking {
    ...
    val filterConfiguration = FilterConfiguration(
      filterSpec = FilterSpec(
        celExpression = "", // Empty expression matches all events
        collectionInterval = timeInterval,
        vidSamplingStart = 0L,  // Start from VID 0
        vidSamplingWidth = 1000000L,  // Cover all VIDs 
    ...
    println("  Max Frequency: ${frequencyVector.getMaxFrequency()}")
    
    val vidsWithFrequency = frequencyVector.getVids()
    println("  VIDs with frequency: ${vidsWithFrequency.size}")
    
    val frequencyDistribution = frequencyVector.getFrequencyDistribution()
    println("  Frequency distribution: $frequencyDistribution")
