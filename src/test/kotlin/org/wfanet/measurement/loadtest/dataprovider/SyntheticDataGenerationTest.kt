...
class SyntheticDataGenerationTest {
  ...
  fun `generateEvents returns a sequence of dynamic event messages`() {
    ...
    val parsedLabeledEvents =
      eventSequence
        .map {
          SyntheticDataGeneration.LabeledEvent(it.vid, TestEvent.parseFrom(it.event.toByteString()))
        }
        .toList()
    ...
    repeat(2) {
      for (vid in 0L until 25L) {
        expectedTestEvents.add(SyntheticDataGeneration.LabeledEvent(vid, expectedTestEvent))
      }
      for (vid in 25L until 50L) {
        expectedTestEvents.add(SyntheticDataGeneration.LabeledEvent(vid, expectedTestEvent2))
      }
    }
    for (vid in 50L until 75L) {
      expectedTestEvents.add(SyntheticDataGeneration.LabeledEvent(vid, expectedTestEvent3))
    }
    for (vid in 75L until 100L) {
      expectedTestEvents.add(SyntheticDataGeneration.LabeledEvent(vid, expectedTestEvent4))
    }
