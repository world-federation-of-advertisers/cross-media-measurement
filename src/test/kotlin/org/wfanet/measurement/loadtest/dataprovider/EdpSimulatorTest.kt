...
class EdpSimulatorTest {
  ...
  private val eventGroupsServiceMock: EventGroupsCoroutineImplBase = mockService {
    onBlocking { getEventGroup(any()) }
      .thenAnswer { invocation ->
        ...
        eventGroup {
          name = request.name
          eventGroupReferenceId = SIMULATOR_EVENT_GROUP_REFERENCE_ID_PREFIX
        }
      }
  }
  private val eventGroupMetadataDescriptorsServiceMock:
    ...
    mockService {
      onBlocking { createEventGroupMetadataDescriptor(any()) }
        .thenReturn(eventGroupMetadataDescriptor { name = EVENT_GROUP_METADATA_DESCRIPTOR_NAME })
    }
  ...
  @Test
  fun `createEventGroup creates EventGroup with synthetic data spec`() {
    val edpSimulator =
      EdpSimulator(
        EDP_DATA,
    ...
    runBlocking { edpSimulator.createEventGroup() }
    ...
    assertThat(metadata.metadata.typeUrl)
      .isEqualTo("type.googleapis.com/${SyntheticEventGroupSpec.getDescriptor().fullName}")
