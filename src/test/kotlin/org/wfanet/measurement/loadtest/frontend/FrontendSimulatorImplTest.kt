...
private val MEASUREMENT_SIGNING_KEY: SigningKeyHandle by lazy {
  ...
  SigningKeyHandle(
    ...
    readPrivateKey(
      FIXED_SERVER_KEY_DER_FILE.readByteString(), consentSignal509Cert.publicKey.algorithm)
  )
}
...
private val RF_MEASUREMENT = measurement {
  ...
  results += resultPair {
    ...
    encryptedResult = encryptResult(
      signResult(
        result,
        MEASUREMENT_SIGNING_KEY),
      loadPublicKey(FIXED_ENCRYPTION_PUBLIC_KEYSET).toEncryptionPublicKey()
    )
  }
  protocolConfig = protocolConfig {
    liquidLegionsV2 = LIQUID_LEGIONS_V2_PROTOCOL_CONFIG
  }
}
