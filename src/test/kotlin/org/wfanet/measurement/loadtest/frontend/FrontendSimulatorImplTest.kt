...
private const val RUN_ID = "run it"
  ...
  fun `get expected result from sketches successfully`() = runBlocking {
    frontendSimulator =
      FrontendSimulatorImpl(
        ...
        SketchStore(FileSystemStorageClient(temporaryFolder.root)),
        RUN_ID
      )
  ...
  companion object {
    ...
    fun writeSketchesToStore() =
      runBlocking<Unit> {
        ...
        val sketchStore = SketchStore(FileSystemStorageClient(temporaryFolder.root))
