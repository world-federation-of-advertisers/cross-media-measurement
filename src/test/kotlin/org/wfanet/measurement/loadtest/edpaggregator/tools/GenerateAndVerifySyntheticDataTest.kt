 ...
 * coalesces multiple [`SyntheticEventGroupSpec`][org.wfanet.measurement.api.v2alpha.event_group_metadata.testing.SyntheticEventGroupSpec]
...
class GenerateAndVerifySyntheticDataTest {
  ...
  fun `generate with multiple specs produces coalesced output for both event groups`() {
    ...
    for (date in EXPECTED_DATES) {
      for (eventGroupReferenceId in listOf(SPEC_A.eventGroupReferenceId, SPEC_B.eventGroupReferenceId)) {
