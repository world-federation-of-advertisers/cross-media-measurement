...
class CovariancesTest {
  ...
  fun `computeMeasurementCovariance throws IllegalArgumentException for reach when sketch params are not matched`() {
    ...
    val otherWeightedReachMeasurementVarianceParams =
      WeightedReachMeasurementVarianceParams(
        ...
        baseMethodology = LiquidLegionsSketchBaseMethodology(decayRate + 1, sketchSize)
      )
    ...
    val unionWeightedReachMeasurementVarianceParams =
      WeightedReachMeasurementVarianceParams(
        ...
        baseMethodology = LiquidLegionsV2BaseMethodology(decayRate + 2, sketchSize, 1e5)
      )
  ...
  companion object {
    fun computeErrorTolerance(estimate: Double, truth: Double): Double {
      return if (truth == 0.0 || estimate == 0.0) {
        ERROR_TOLERANCE_PERCENTAGE
      } else {
        abs(truth * ERROR_TOLERANCE_PERCENTAGE)
      }
    }
