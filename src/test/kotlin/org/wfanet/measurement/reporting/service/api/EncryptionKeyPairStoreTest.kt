...
class EncryptionKeyPairStoreTest {
  ...
  /**
   * Returns [publicKey] re-parsed and re-serialized through [TinkPublicKeyHandle] -- the same
   * normalization [InMemoryEncryptionKeyPairStore.fingerprint] applies -- which differs, byte for
   * byte, from [publicKey] while still parsing to the same key.
   *
   * This is a real instance of the bug being fixed, not a synthetic one: [PUBLIC_KEY_1] was
   * generated by an older Tink version, and the current Tink library's own serialization of that
   * identical key material already differs from the checked-in file's bytes.
   */
  private fun reserialize(publicKey: ByteString): ByteString {
    val reserialized = TinkPublicKeyHandle(publicKey).toByteString()
