...
class ModelLinesServiceTest {
  ...
  private val eventGroupsServiceMock: KingdomEventGroupsCoroutineImplBase = mockService {
    onBlocking { getEventGroup(any()) }.thenReturn(
      eventGroup {
  ...
  private val dataProvidersServiceMock: KingdomDataProvidersCoroutineImplBase = mockService {
    onBlocking { getDataProvider(any()) }.thenReturn(
      dataProvider {
        dataAvailabilityInterval = interval {
          startTime = timestamp {
            seconds = 100
  ...
  val grpcTestServerRule = GrpcTestServerRule {
    ...
    addService(KingdomModelSuitesService(InternalKingdomModelSuitesCoroutineStub(internalGrpcTestServerRule.channel))
                 .withMetadataPrincipalIdentities())
    addService(KingdomModelLinesService(InternalKingdomModelLinesCoroutineStub(internalGrpcTestServerRule.channel))
                 .withMetadataPrincipalIdentities())
    addService(KingdomModelRolloutsService(InternalKingdomModelRolloutsCoroutineStub(internalGrpcTestServerRule.channel))
                 .withMetadataPrincipalIdentities())
    addService(KingdomModelReleasesService(InternalKingdomModelReleasesCoroutineStub(internalGrpcTestServerRule.channel))
                 .withMetadataPrincipalIdentities())
  ...
  @get:Rule
  val serverRuleChain: TestRule = chainRulesSequentially(spannerDatabase, internalGrpcTestServerRule, grpcTestServerRule)
  ...
  fun initService() {
    runBlocking {
      ...
      modelProviderName = ModelProviderKey(externalIdToApiId(modelProvider.externalModelProviderId)).toName()
      val modelSuitesStub = KingdomModelSuitesCoroutineStub(grpcTestServerRule.channel)
      modelSuite =
        modelSuitesStub.withPrincipalName(modelProviderName).createModelSuite(
    ...
    modelLinesStub = KingdomModelLinesCoroutineStub(grpcTestServerRule.channel).withPrincipalName(modelProviderName)
  ...
  fun `listValidModelLines returns model lines in order`(): Unit = runBlocking {
    val eventGroupName = EventGroupKey(
      "1234",
      "1234"
    ).toName()
    ...
    whenever(eventGroupsServiceMock.getEventGroup(any()))
      .thenReturn(eventGroup {
        ...
        dataAvailabilityInterval = interval {
          startTime = futureTime.copy {
            seconds += 50
          }
          endTime = futureTime.copy {
            seconds += 100
          }
        }
      })
    ...
    var modelLine = modelLine {
      ...
      activeStartTime = futureTime.copy {
        seconds += 25
      }
  ...
  @Test
  fun `listValidModelLines does not return model line if time_interval not contained`(): Unit = runBlocking {
    val eventGroupName = EventGroupKey(
      "1234",
      "1234"
    ).toName()
  ...
  @Test
  fun `listValidModelLines does not return model line if eg data availability not contained`(): Unit = runBlocking {
    val eventGroupName = EventGroupKey(
      "1234",
      "1234"
    ).toName()
  ...
  @Test
  fun `listValidModelLines does not return model line if edp data availability not contained`(): Unit = runBlocking {
    val eventGroupName = EventGroupKey(
      "1234",
      "1234"
    ).toName()
  ...
  fun `listValidModelLines throws NOT_FOUND when event group not found`(): Unit = runBlocking {
    ...
    whenever(eventGroupsServiceMock.getEventGroup(any()))
      .thenThrow(EventGroupNotFoundException(eventGroupName).asStatusRuntimeException(Status.Code.NOT_FOUND))
  ...
  fun `listValidModelLines throws PERMISSION_DENIED when caller does not have permission`() {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL.copy { name = "principals/other-model-suite-user" }, SCOPES) {
          runBlocking {
            service.listValidModelLines(
              listValidModelLinesRequest {
                timeInterval = interval {
  ...
  @Test
  fun `listValidModelLines throws INVALID_ARGUMENT when time_interval start_time is invalid`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL, SCOPES) {
          runBlocking {
  ...
  @Test
  fun `listValidModelLines throws INVALID_ARGUMENT when time_interval end_time is invalid`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL, SCOPES) {
          runBlocking {
  ...
  @Test
  fun `listValidModelLines throws INVALID_ARGUMENT when time_interval start equals end`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL, SCOPES) {
          runBlocking {
  ...
  @Test
  fun `listValidModelLines throws INVALID_ARGUMENT when time_interval start greater than end`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL, SCOPES) {
          runBlocking {
  ...
  @Test
  fun `listValidModelLines throws INVALID_ARGUMENT when event_groups has invalid name`() = runBlocking {
    val exception =
      assertFailsWith<StatusRuntimeException> {
        withPrincipalAndScopes(PRINCIPAL, SCOPES) {
          runBlocking {
  ...
  companion object {
    ...
    private val ALL_PERMISSIONS =
      setOf(ModelLinesService.Permission.LIST)
