...
class BasicReportsServiceTest {
  ...
  fun `createBasicReport does not overwrite report_start when default exists`(): Unit =
    runBlocking {
      ...
      assertThat(createdBasicReport.reportingInterval.effectiveReportStart)
        .isEqualTo(BASIC_REPORT.reportingInterval.reportStart.copy { clearHours() })
    }

  @Test
  fun `createBasicReport doesn't require time when default exists`(): Unit = runBlocking {
    service =
      BasicReportsService(
        internalBasicReportsService,
        internalImpressionQualificationFiltersService,
  ...
  @Test
  fun `createBasicReport returns basic report when custom IQF has no spec`(): Unit = runBlocking {
    val measurementConsumerKey = MeasurementConsumerKey(CMMS_MEASUREMENT_CONSUMER_ID)
    val campaignGroupKey = ReportingSetKey(measurementConsumerKey, "1234")
  ...
  @Test
  fun `createBasicReport returns basic report when custom IQF spec has no filter`(): Unit =
    runBlocking {
      val measurementConsumerKey = MeasurementConsumerKey(CMMS_MEASUREMENT_CONSUMER_ID)
      val campaignGroupKey = ReportingSetKey(measurementConsumerKey, "1234")
  ...
  @Test
  fun `createBasicReport returns basic report when custom IQF term has a float value`(): Unit =
    runBlocking {
      val measurementConsumerKey = MeasurementConsumerKey(CMMS_MEASUREMENT_CONSUMER_ID)
      val campaignGroupKey = ReportingSetKey(measurementConsumerKey, "1234")
  ...
  @Test
  fun `createBasicReport throws INVALID_ARGUMENT when float value is set on a non-float field`():
    Unit = runBlocking {
    val measurementConsumerKey = MeasurementConsumerKey(CMMS_MEASUREMENT_CONSUMER_ID)
    val campaignGroupKey = ReportingSetKey(measurementConsumerKey, "1234")
  ...
  fun `createBasicReport throws PERMISSION_DENIED when caller does not have permission`() =
    runBlocking {
      ...
      val span =
        spanExporter.finishedSpanItems.single { it.name == "reporting.basic_report.create" }
  ...
  fun `createBasicReport throws ALREADY_EXISTS when basicReport already exists`() = runBlocking {
    ...
    val failureSpan =
      spanExporter.finishedSpanItems.single {
        it.name == "reporting.basic_report.create" &&
          it.attributes.get(ReportTraceAttributes.OUTCOME) == "failed"
      }
    assertThat(failureSpan.attributes.get(ReportTraceAttributes.BASIC_REPORT_NAME))
      .isEqualTo(createdBasicReport.name)
  ...
  @Test
  fun `createBasicReport throws INVALID_ARGUMENT when custom has spec with no media type`() =
    runBlocking {
      val measurementConsumerKey = MeasurementConsumerKey(CMMS_MEASUREMENT_CONSUMER_ID)
      val campaignGroupKey = ReportingSetKey(measurementConsumerKey, "1234")
      ...
      assertThat(exception)
        ...
        .isEqualTo(
          errorInfo {
            ...
            reason = Errors.Reason.REQUIRED_FIELD_NOT_SET.name
            metadata[Errors.Metadata.FIELD_NAME.key] =
              "basic_report.impression_qualification_filters[0].custom.filter_specs[0].media_type"
          }
        )
    }
  ...
  fun `getBasicReport throws NOT_FOUND when not found`() = runBlocking {
    ...
    val span = spanExporter.finishedSpanItems.single { it.name == "reporting.basic_report.get" }
    assertThat(span.attributes.get(ReportTraceAttributes.BASIC_REPORT_NAME)).isEqualTo(request.name)
    assertThat(span.attributes.get(ReportTraceAttributes.LIFECYCLE_STAGE))
      .isEqualTo("basic_report_api_fetch")
    assertThat(span.attributes.get(ReportTraceAttributes.OUTCOME)).isEqualTo("failed")
    assertThat(span.attributes.get(ReportTraceAttributes.ERROR_TYPE))
      .isEqualTo("StatusRuntimeException")
  }
  ...
  fun `listBasicReports returns healthy BasicReport and lists poisoned one in unreachable`(): Unit =
    runBlocking {
      ...
      val campaignGroupId = "campaign-1"
      val primitiveReportingSetId = "primitive-1"
      val healthyBasicReportId = "healthy-1"
      val poisonBasicReportId = "poison-1"
      ...
      val healthyInternalBasicReport =
        internalBasicReportsService.insertBasicReport(
          insertBasicReportRequest {
            basicReport = internalBasicReport {
              this.cmmsMeasurementConsumerId = cmmsMeasurementConsumerId
              externalBasicReportId = healthyBasicReportId
              externalCampaignGroupId = campaignGroupId
              campaignGroupDisplayName = INTERNAL_CAMPAIGN_GROUP.displayName
              details = basicReportDetails {
                title = "title"
      ...
      val poisonBasicReportToCreate = internalBasicReport {
        this.cmmsMeasurementConsumerId = cmmsMeasurementConsumerId
        externalBasicReportId = poisonBasicReportId
        externalCampaignGroupId = campaignGroupId
        details = basicReportDetails {
          title = "title"
          reportingInterval = internalReportingInterval {
      ...
      assertThat(response.basicReportsList)
        .containsExactly(
          basicReport {
            name =
              BasicReportKey(
                  cmmsMeasurementConsumerId = cmmsMeasurementConsumerId,
                  basicReportId = healthyBasicReportId,
                )
                .toName()
            campaignGroup =
              ReportingSetKey(
                  cmmsMeasurementConsumerId = cmmsMeasurementConsumerId,
                  reportingSetId = campaignGroupId,
                )
                .toName()
            campaignGroupDisplayName = INTERNAL_CAMPAIGN_GROUP.displayName
            effectiveCampaignGroup = campaignGroup
            ...
            createTime = healthyInternalBasicReport.createTime
            state = BasicReport.State.SUCCEEDED
          }
        )
      assertThat(response.unreachableList)
        .containsExactly(
          BasicReportKey(
              cmmsMeasurementConsumerId = cmmsMeasurementConsumerId,
              basicReportId = poisonBasicReportId,
            )
            .toName()
        )
    }
