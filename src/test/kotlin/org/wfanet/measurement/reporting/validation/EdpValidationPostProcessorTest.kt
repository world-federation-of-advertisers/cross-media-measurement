...
class EdpValidationPostProcessorTest {
  ...
  @Test
  fun `validate returns PASSED when cloud function call fails`() {
    fakeCloudFunction.responseStatus = 500

    // A cloud function error is logged and counted but never fails the report.
    val result = postProcessor().validate(listOf(row(reportedImpressions = 8_500_000L)))
  ...
  @Test
  fun `validate returns PASSED when response is skipped`() {
  ...
  @Test
  fun `validate skips DataProviders without configuration`() {
    val result =
      postProcessor()
        .validate(
