...
class ToleranceEvaluatorTest {
  ...
  private val defaultConfig = toleranceConfig {
    ...
    noiseConfidenceMultiplier = 4.0
  }

  @Test
  fun `evaluate returns PASS when counts match exactly`() {
    val result = ToleranceEvaluator.evaluate(
  ...
  @Test
  fun `evaluate uses noise-aware tolerance with VID sampling`() {
    val result = ToleranceEvaluator.evaluate(
      reportedCount = 950_000,
      publisherCount = 10_000_000,
      config = defaultConfig,
      vidSamplingWidth = 0.1,
    )
  ...
  @Test
  fun `evaluate handles zero publisher count`() {
    val result = ToleranceEvaluator.evaluate(
      reportedCount = 0,
      publisherCount = 0,
  ...
  @Test
  fun `evaluate returns FAIL when reported is non-zero but publisher is zero`() {
    val result = ToleranceEvaluator.evaluate(
    ...
    assertThat(result.verdict).isEqualTo(ToleranceEvaluator.Verdict.FAIL)
    assertThat(result.deviationFraction).isEqualTo(1.0)
  }
}
