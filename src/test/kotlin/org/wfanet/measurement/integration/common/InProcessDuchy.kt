 ...
 *
 * @param duchyName the name of this duchy
...
class InProcessDuchy(
  ...
  private val systemComputationsClient by lazy {
    SystemComputationsCoroutineStub(kingdomChannel).withDuchyId(duchyName)
  }
  private val systemComputationLogEntriesClient by lazy {
    SystemComputationLogEntriesCoroutineStub(kingdomChannel).withDuchyId(duchyName)
  }
  private val systemComputationParticipantsClient by lazy {
    SystemComputationParticipantsCoroutineStub(kingdomChannel).withDuchyId(duchyName)
  }
  private val systemRequisitionsClient by lazy {
    SystemRequisitionsCoroutineStub(kingdomChannel).withDuchyId(duchyName)
  }
  ...
  private val computationControlServer =
    GrpcTestServerRule(
      computationControlChannelName(duchyName),
  ...
  private val heraldRule = CloseableResource {
    GlobalScope.launchAsAutoCloseable {
      ...
      val protocolsSetupConfig =
        if (duchyName == LLV2_AGGREGATOR_NAME) {
    ...
    GlobalScope.launchAsAutoCloseable {
      val workerStubs =
        ALL_DUCHY_NAMES.associateWith {
          ...
          val stub = SystemComputationControlCoroutineStub(channel).withDuchyId(duchyName)
          stub
        }
      val liquidLegionsV2mill =
        LiquidLegionsV2Mill(
          millId = "$duchyName liquidLegionsV2mill",
          duchyId = duchyName,
