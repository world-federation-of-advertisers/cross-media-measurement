 ...
 * easily.
 */
abstract class InProcessLifeOfAMeasurementIntegrationTestBase {
  ...
  private suspend fun createAllResources() {
    ...
    mcResourceName = resourceSetup.createMeasurementConsumer(mc).name
    // Create all EDPs
  ...
  fun createResourcesAndStartMillsAndDataProviders() = runBlocking {
    ...
    edpSimulators.forEach {
      it.start(edpDisplayNameToResourceNameMap[it.displayName]!!, mcResourceName)
    }

    // Wait for 1s to make sure all EDPs finish creating eventGroups before the test starts.
    delay(1000)
  }

  @Test
  fun `create a measurement and check the result is equal to the expected result`() = runBlocking {
    // Assert that all EDPs have participated in the above measurement.
    assertThat(
      ...
      .hasSize(ALL_EDP_DISPLAY_NAMES.size)
