...
class DynamicClipperTest {
  ...
  @Test
  fun `runTwoPhaseDynamicClipping for impression returns expected dynamicClipResult with small maxThreshold`() {
    val histogramList = listOf(0L, 1L, 1L, 2L) // cumulativeHistogramList is listOf(4.0, 3.0, 2.0)
    val maxThreshold = 2

    // Set rho large enough to minimize noise.
    val dynamicClipper =
      DynamicClipper(histogramList, BIG_RHO, IMPRESSION_MEASUREMENT_TYPE, maxThreshold)
  ...
  @Test
  fun `runTwoPhaseDynamicClipping for impression returns expected dynamicClipResult with large maxThreshold`() {
    val histogramList = listOf(0L, 1L, 1L, 2L) // cumulativeHistogramList is listOf(4.0, 3.0, 2.0)
  ...
  @Test
  fun `runTwoPhaseDynamicClipping for impression result has expected threshold and error with small maxThreshold`() {
    val histogramList =
      listOf(0L, 1L, 1L, 1L, 1L) // cumulativeHistogramList is listOf(4.0, 3.0, 2.0, 1.0)
    val maxThreshold = 3
  ...
  @Test
  fun `runTwoPhaseDynamicClipping for impression result has expected threshold and error with another dataset`() {
    val histogramList =
      listOf(0L, 20L, 20L, 12L, 1L) // cumulativeHistogramList is listOf(53.0, 33.0, 13.0, 1.0)
    val maxThreshold = 3
  ...
  @Test
  fun `runTwoPhaseDynamicClipping for impression result has expected threshold and error with default max threshold`() {
    val histogramList = listOf(0L, 20L, 20L, 12L, 1L)

    // Set rho large enough to minimize noise.
    val dynamicClipper = DynamicClipper(histogramList, BIG_RHO, IMPRESSION_MEASUREMENT_TYPE)
    val dynamicClipResult = dynamicClipper.runTwoPhaseDynamicClipping()
  ...
  fun `frequencyHistogramMapToList works as expected`() {
    ...
    val histogramList = DynamicClipper.frequencyHistogramMapToList(frequencyHistogramMap)
  ...
  private companion object {
    private val IMPRESSION_MEASUREMENT_TYPE = DynamicClipMeasurementType.IMPRESSION
