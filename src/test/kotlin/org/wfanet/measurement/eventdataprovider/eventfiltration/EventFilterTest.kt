...
import wfanet.measurement.api.v2alpha.event_templates.testing.AgeRange
import wfanet.measurement.api.v2alpha.event_templates.testing.TestEvent
...
class EventFilterTest {
  ...
  fun `keeps event when condition matches`() {
    val program =
      EventFilter.compileProgram(
        " video_ad.age.value == 1",
        TestEvent.getDefaultInstance(),
      )
    val event = exampleEventWithAge()
    assert(EventFilter.matches(event, program))
  }

  @Test
  fun `filters even when condition does not match`() {
    val program =
      EventFilter.compileProgram(" video_ad.age.value != 1", TestEvent.getDefaultInstance())
    val event = exampleEventWithAge()
    assert(!EventFilter.matches(event, program))
  }
  ...
  fun `throws error when field is not filled`() {
    ...
    assertThatFailsWithCode(EventFilterException.Code.EVALUATION_ERROR) {
      EventFilter.matches(event, program)
    }
  }
