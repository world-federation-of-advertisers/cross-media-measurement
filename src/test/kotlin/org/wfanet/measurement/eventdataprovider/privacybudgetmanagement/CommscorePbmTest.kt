...
private const val FOLDER_PATH =
  "/google/src/cloud/uakyol/om_open_source/google3/experimental/users/uakyol/pbmForComscoreWithImmutable/cross-media-measurement/src/test/kotlin/org/wfanet/measurement/eventdataprovider/privacybudgetmanagement"
...
class CommscorePbmTest {
  ...
  private fun getStartOffset(elem: JsonElement): Long {
    try {
      ...
      // println("getStartOffset")
      // println(elem.getAsString().split("..").get(0).toLong())
      return 31 - elem.getAsString().split("..").get(0).toLong()
    }
  }
  ...
  private fun getEndOffset(elem: JsonElement): Long {
    try {
      ...
      // println("getEndOffset")
      // println(elem.getAsString().split("..").get(1).toLong())
      return 31 - elem.getAsString().split("..").get(1).toLong()
    }
  }
        ...
        value =
          RequisitionSpecKt.EventGroupEntryKt.value {
            collectionInterval = timeInterval {
              startTime =
                LocalDate.now()
                  .minusDays(getStartOffset(obj.get("days")))
                  .atStartOfDay()
                  .toInstant(ZoneOffset.UTC)
                  .toProtoTime()
              endTime =
                LocalDate.now()
                  .minusDays(getEndOffset(obj.get("days")))
                  .atStartOfDay()
                  .toInstant(ZoneOffset.UTC)
                  .toProtoTime()
            }
  ...
  fun `chargePrivacyBudget throws PRIVACY_BUDGET_EXCEEDED when given a large single charge`() {
    ...
    // backingStore.ledger.forEach { println(it.repetitionCount) }
  }
}
