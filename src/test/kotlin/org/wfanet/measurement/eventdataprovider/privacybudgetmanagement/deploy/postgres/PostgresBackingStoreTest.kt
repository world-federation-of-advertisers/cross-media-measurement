...
import org.wfanet.measurement.eventdataprovider.privacybudgetmanagement.AbstractPrivacyBudgetLedgerStoreTest
...
class PostgresBackingStoreTest : AbstractPrivacyBudgetLedgerStoreTest() {
  companion object {
    ...
    fun initDatabase() {
      ...
      val connection: Connection = embeddedPostgres.postgresDatabase.connection
  ...
  override fun createBackingStore(createSchema: Boolean): PrivacyBudgetLedgerBackingStore {
    val connection = embeddedPostgres.getDatabase(PG_USER_NAME, TEST_DB).connection
    if (createSchema) {
      connection.createStatement().use { statement: Statement ->
    ...
    // return always the same connection to simplify testing
    val createConnection = { connection }
