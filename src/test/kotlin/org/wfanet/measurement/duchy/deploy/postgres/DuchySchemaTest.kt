...
class DuchySchemaTest {
  ...
  fun `all duchy tables are created`(): Unit = runBlocking {
    ...
    val duchyTableNames = listOf(
      "computations", "requisitions", "computationstages", "computationblobreferences",
      "computationstageattempts", "computationstats", "heraldcontinuationtokens"
      )

    val resList = mutableListOf<String>()
    databaseClient.readTransaction().executeQuery(boundStatement(query)).consume(::translate).toList(resList)
