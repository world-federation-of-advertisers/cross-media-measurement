...
private val FULFILLED_RESPONSE = unaryFulfillRequisitionResponse { state = Requisition.State.FULFILLED }
...
/** Test for [UnaryRequisitionFulfillmentService]. */
@RunWith(JUnit4::class)
class UnaryRequisitionFulfillmentServiceTest {
  ...
  private lateinit var service: UnaryRequisitionFulfillmentService
  ...
  ): UnaryFulfillRequisitionRequest {
    return unaryFulfillRequisitionRequest {
      name = this@withContent.name
      requisitionFingerprint = this@withContent.requisitionFingerprint
      nonce = this@withContent.nonce
      data = bodyContent.asSequence().fold(ByteString.EMPTY, ByteString::concat)
    }
  }
}
