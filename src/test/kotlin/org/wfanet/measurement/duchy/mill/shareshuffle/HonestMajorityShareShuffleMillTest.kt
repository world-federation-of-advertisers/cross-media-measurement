...
private val NO_NOISE_HMSS_PARAMETERS =
  HonestMajorityShareShuffleKt.ComputationDetailsKt.parameters {
    maximumFrequency = 10
    ringModulus = RING_MODULUS
...
private val NO_NOISE_REACH_ONLY_HMSS_PARAMETERS =
  HonestMajorityShareShuffleKt.ComputationDetailsKt.parameters {
    maximumFrequency = 1
    ringModulus = RING_MODULUS
    noiseMechanism = NoiseMechanism.NONE
...
class HonestMajorityShareShuffleMillTest {
  ...
  private suspend fun getNoNoiseHmssComputationDetails(
    role: RoleInComputation
  ): ComputationDetails {
    return computationDetails {
      kingdomComputation =
      ...
      honestMajorityShareShuffle =
        HonestMajorityShareShuffleKt.computationDetails {
          this.role = role
          parameters = NO_NOISE_HMSS_PARAMETERS
          nonAggregators += listOf(DUCHY_ONE_ID, DUCHY_TWO_ID)
  ...
  private suspend fun getNoNoiseReachOnlyHmssComputationDetails(
    role: RoleInComputation
  ): ComputationDetails {
    return computationDetails {
      kingdomComputation =
      ...
      honestMajorityShareShuffle =
        HonestMajorityShareShuffleKt.computationDetails {
          this.role = role
          parameters = NO_NOISE_REACH_ONLY_HMSS_PARAMETERS
          nonAggregators += listOf(DUCHY_ONE_ID, DUCHY_TWO_ID)
  ...
  @Test
  fun `noNoiseShufflePhase passes NoiseMechanism NONE to crypto worker`() = runBlocking {
    val computationDetails =
      getNoNoiseHmssComputationDetails(RoleInComputation.FIRST_NON_AGGREGATOR)
  ...
  @Test
  fun `noNoiseAggregationPhase passes NoiseMechanism NONE to crypto worker`() = runBlocking {
    val computationDetails = getNoNoiseHmssComputationDetails(RoleInComputation.AGGREGATOR)
    val inputBlobPath1 =
      ComputationBlobContext(GLOBAL_ID, Stage.AGGREGATION_PHASE.toProtocolStage(), 0L)
    val inputBlobData1 =
      aggregationPhaseInput {
  ...
  @Test
  fun `noNoiseReachOnlyShufflePhase passes NoiseMechanism NONE to crypto worker`() = runBlocking {
    val computationDetails =
      getNoNoiseReachOnlyHmssComputationDetails(RoleInComputation.FIRST_NON_AGGREGATOR)
  ...
  @Test
  fun `noNoiseReachOnlyAggregationPhase passes NoiseMechanism NONE to crypto worker`() =
    runBlocking {
      val computationDetails =
        getNoNoiseReachOnlyHmssComputationDetails(RoleInComputation.AGGREGATOR)
      val inputBlobPath1 =
        ComputationBlobContext(GLOBAL_ID, Stage.AGGREGATION_PHASE.toProtocolStage(), 0L)
      val inputBlobData1 =
        aggregationPhaseInput {
