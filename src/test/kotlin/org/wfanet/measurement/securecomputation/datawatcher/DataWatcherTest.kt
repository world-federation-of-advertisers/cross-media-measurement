...
class DataWatcherTest() {
  ...
  fun `records processing_duration metric for control plane sink`() {
    runBlocking {
      ...
      val processingDurationMetric = metrics.find { it.name == "edpa.data_watcher.processing_duration" }
      ...
      assertThat(point.attributes.get(AttributeKey.stringKey("config_identifier")))
        .isEqualTo(configIdentifier)
      assertThat(point.attributes.get(AttributeKey.stringKey("sink_type")))
  ...
  fun `records queue_writes counter metric`() {
    runBlocking {
      ...
      assertThat(point.attributes.get(AttributeKey.stringKey("work_item_id"))).isEqualTo(workItemId)
      assertThat(point.attributes.get(AttributeKey.stringKey("queue"))).isEqualTo(topicId)
      assertThat(point.attributes.get(AttributeKey.stringKey("config_identifier")))
        .isEqualTo(configIdentifier)
    }
  }
  ...
  fun `records multiple queue_writes for multiple paths`() {
    runBlocking {
      ...
      val sumData = queueWritesMetric!!.longSumData
      // Should have 3 points (one per work item)
      assertThat(sumData.points).hasSize(3)
      val totalCount = sumData.points.sumOf { it.value }
  ...
  companion object {
    init {
      EdpaTelemetry.useNoopTelemetryForTests()
    }
  }
}
